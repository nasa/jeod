...
my $Max_Processes = <PSOUT>;
chomp $Max_Processes;
close PSOUT;

# On Mac OS, don't run more than 255 processes on top of already running processes,
# this is necessary so the nightly build script don't fork/spawn more jobs at runtime.
$Max_Processes += 255;
...
unless ( $Environment ) {
   ...
   my $Trick_host_cpu = `trick-gte TRICK_HOST_CPU`;
...
chomp ($Swig_version = `swig -version | grep -i version | sed -e 's/^[^0-9]*//'`);
my ($Swig_major_ver, $Swig_minor_ver) = split /(?=[.])/i, $Swig_version, 2;
chomp (my $cpp_standard = `perl $Jeod_home/bin/jeod/test_cpp11.pl`);
$Cpp11_supported = ($cpp_standard =~ /c\+\+1[147]/);
...
sub set_environment() {
   ...
   $ENV{'JEOD_SPICE_DIR'}          = "/usr/local/share/cspice" unless ( defined $ENV{'JEOD_SPICE_DIR'} );
   $ENV{'TRICK_VER'}               = '17.3.0';
   $ENV{'TRICK_HOME'}              = "/usr/local/trick/$ENV{'TRICK_VER'}" unless ( defined $ENV{'TRICK_HOME'} );
   ...
   $ENV{'TRICK_USER_LINK_LIBS'}    = '';
   my $trick_host_cpu              = (`$ENV{'TRICK_HOME'}/bin/trick-gte TRICK_HOST_CPU`);
   chomp $trick_host_cpu;
   $ENV{'TRICK_HOST_CPU'}          = $trick_host_cpu;
   ...
   if ( $Os_name eq "darwin" ) {
      my $fink_path = "/sw/bin";
      my $tex_path = "/usr/local/texlive/2014/bin/x86_64-darwin";
      $ENV{'PATH'} = "$tex_path:$fink_path:$ENV{'PATH'}";
   }
...
sub jeod_sim_level_tasks() {
   ...
   # Build the DE405 ephem files in advance due to SIM_mercury, which
   # spans 601 years and thus needs all of the data built.
   my @ephem_years=map {1600+20*$_} (0..21);
   my $years=join' ', @ephem_years;
   print "\nvariable years = $years\n";
   print "\nBuilding JPL 405 files\n\n";
   system ('cd ${JEOD_HOME}/models/environment/ephemerides/de4xx_ephem/data ; \
            make MODEL=405 YEARS="${years}" JPL-DE405.${TRICK_HOST_CPU}.bin');
   ...
   # Kill any remaining sims that are running over the timeout limit
   system ("bash -c '$Trick_home/bin/kill_sim >& /dev/null'");
...
sub find_sim_dir() {        # list of jeod sims
   ...
   if ($basename =~ /SIM\_.*/) {
      ...
      if ($Swig_major_ver >= 3 && $Cpp11_supported) {
         ...
         foreach my $d (@all_jeod_sims) {
            my $s_overrides = "$d/S_overrides.mk";
            if ((-f $s_overrides) &&
                (system ("grep -q c++11 $s_overrides") == 0)) {
                print "Skipping $d (requires SWIG 3.0 and c++11)\n";
            }
            else {
               push @jeod_sims, $d;
            }
...
sub compile() {
   ...
   if ( $Clean ) {
      ...
      system ("bash -c 'catalog -i >& /dev/null'");
      sleep 1;
      remove_trick_stuff;
...
sub wait_for_fork() {
   ...
   while ( $num_proc > $Max_Processes ) {
      foreach my $zombie_id (get_zombie_list($0)) {
         ...
         print "I am zombie_id = $zombie_id ...\n\n";
         waitpid($zombie_id, 0);
         $num_proc--;
      }
      sleep 10;
...
sub fork_run {   # run a run and if fails, reports it to run_error
   ...
   my $smain_command = "./S_main_$ENV{'TRICK_HOST_CPU'}.exe $rundir/$inputfile";
   my $pid = fork();
   ...
   if ( $pid == 0 ) {
      eval {
         ...
         my $output_file = "$rundir"."_run_out.txt";
         $output_file =~ s/\//_/g;
         system ("bash -c '$smain_command >&   $output_file'");
         alarm 0;
         unless ( check_sendhs($rundir) ) {
            open RUNERROR, ">>$Jeod_home/$error_file";
      ...
      if ( $@ ) {
         if ( $@ eq "timeout\n" ) {
            print "RUN $simpath/$rundir is running for too long! ...\n";
         } else {
            print "Somethine occured...\n";
         }
      }
      exit 0;
   } elsif ( $pid ) {
...
sub build_pdf {
   ...
   my $pdf_build = system ("bash -c 'make >& doc_out.txt'");
   ...
   if ($pdf_build) {
      ...
      system ("bash -c 'make install >& doc_install_out.txt'");
   }
...
sub clean_pdf {
   ...
   system ("bash -c 'make real_clean >& /dev/null'");
...
sub make_validation_data() {
   ...
   foreach my $run (@set_test_runs) {
      ...
      foreach my $file (@run_data) {
         push (@headers,$file) if ( $file =~ /\.header$/ );
         push (@headers,$file) if ( $file =~ /input/ );
         push (@ascii_binaries,$file) if ( $file =~ /\.trk$/ );
         push (@ascii_binaries,$file) if ( $file =~ /\.csv$/ );
         push (@ascii_binaries,$file) if ( $file =~ /\.out$/ );
      }
