...
######################################################################
#
# start_jeod3_build.pl: JEOD Nightly Build Process script for JEOD 3.x
#
######################################################################
...
my (
    ...
    $Local_Archive_dir,            # Directory to local archive nighly build status
    $Webserver,                    # Hostname of webserve (nexus)
    $Webserver_user,               # Username of webserver
    $Webserver_dir,                # Directory to webserver where nightly build archive is located
...
######################################################################
# Check if Build is necessary, and if yes, checkout the code
######################################################################
...
sub compile_sims() {
   ...
   if ( $Path ) {
      system ("jeod_auto_cm.pl -env -cp -path $Path");
   } else {
      system ("jeod_auto_cm.pl -env -cp");
   }
}
...
sub uncompile_sims() {
   if ( $Path ) {
      system ("jeod_auto_cm.pl -env -cp -clean -path $Path");
   } else {
      system ('jeod_auto_cm.pl -env -cp -clean');
   }
}
...
sub compile_and_run() {
   ...
   if ( $Path ) {
      system ("jeod_auto_cm.pl -env -cp -run -path $Path");
   } else {
      system ('jeod_auto_cm.pl -env -cp -run');
   }
}
...
sub run_sims() {
   ...
   if ( $Path ) {
      system ("jeod_auto_cm.pl -env -run -path $Path");
   } else {
      system ('jeod_auto_cm.pl -env -run');
   }
}
...
sub clean_runs() {
   if ( $Path ) {
      system ("jeod_auto_cm.pl -env -run -clean -path $Path");
   } else {
      system ('jeod_auto_cm.pl -env -run -clean');
   }
}
...
sub archive_build_status() {
   mkdir $Local_Archive_dir unless -d $Local_Archive_dir;
   mkdir $Local_Archive_dir . "/" . $Os_name unless -d "$Local_Archive_dir/$Os_name";

   my $current_build_dir = $Current_month . "-" . $Current_day . "-" . $Current_year;

   mkdir "$Local_Archive_dir/$Os_name/$current_build_dir" unless -d "$Local_Archive_dir/$Os_name/$current_build_dir";
   ...
   foreach my $file (@error_files) {
      copy($file, "$Local_Archive_dir/$Os_name/$current_build_dir/$file") or die "Cannot copy $file to $Local_Archive_dir/$Os_name/$current_build_dir\n";
   }

   system ("scp -r $Local_Archive_dir/$Os_name/$current_build_dir $Webserver_user\@$Webserver:$Webserver_dir/$Archive_dir_name/$Os_name/.");
}
######################################################################
