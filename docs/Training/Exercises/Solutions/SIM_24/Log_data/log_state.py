...
def log_state ( log_cycle ) :
  ...
  for ii in range(0,3) :
    ...
    dr_group.add_variable( "vehicle2.dyn_body.composite_body.state.trans.velocity[" + str(ii) +"]")
    dr_group.add_variable( "vehicle.dyn_body.composite_body.state.rot.ang_vel_this[" + str(ii) +"]")
    dr_group.add_variable( "vehicle2.dyn_body.composite_body.state.rot.ang_vel_this[" + str(ii) +"]")
    for jj in range(0,3) :
      dr_group.add_variable( "vehicle.dyn_body.composite_body.state.rot.T_parent_this[" + str(ii) +"][" + str(jj) +"]")
      dr_group.add_variable( "vehicle2.dyn_body.composite_body.state.rot.T_parent_this[" + str(ii) +"][" + str(jj) +"]") 
