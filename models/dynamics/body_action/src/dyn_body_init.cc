 ...
 */
void DynBodyInit::initialize(DynManager & dyn_manager)
{
    ...
    if(body_ref_frame == nullptr)
    {
        if(body_frame_id.empty())
        {
            MessageHandler::warn(__FILE__,
                                 __LINE__,
                                 "Incomplete assignment to dyn-body-init\n",
        ...
        body_ref_frame = find_body_frame(*dyn_subject, body_frame_id, "body_frame_id");
    }
    ...
    {
        reference_ref_frame = find_ref_frame(dyn_manager, reference_ref_frame_name, "reference_ref_frame_name");
    }
 ...
 */
RefFrameItems::Items DynBodyInit::initializes_what()
{
 ...
 */
void DynBodyInit::apply(DynManager & dyn_manager)
{
    ...
    MessageHandler::debug(__FILE__,
                          ...
                          action_identifier.c_str(),
                          body_ref_frame->get_name().c_str(),
                          RefFrameItems::to_string(set_items).c_str(),
 ...
 */
void DynBodyInit::report_failure()
{
    RefFrameItems::Items set_items = // The newly set state elements
        initializes_what();

    std::string temp = "unspecified frame";
    if(body_ref_frame != nullptr)
    {
        temp = body_ref_frame->get_name();
    }

    MessageHandler::error(__FILE__,
                          __LINE__,
                          BodyActionMessages::not_performed,
                          "%s:\n"
 ...
 */
void DynBodyInit::apply_user_inputs()
{
    RefFrame user_frame;                         /* --
    ...
    body_ref_frame->compute_relative_state(*reference_ref_frame, reverse_sense, user_frame.state);
    ...
    if(set_items.contains(RefFrameItems::Rate))
    {
        if(rate_in_parent)
        {
            Vector3::transform(user_frame.state.rot.T_parent_this, ang_velocity, user_frame.state.rot.ang_vel_this);
        }
        else
        {
    ...
    user_frame.compute_relative_state(*(dyn_subject->get_integ_frame()), state);
 ...
 */
void DynBodyInit::compute_rotational_state()
{
 ...
 * This method is obsolete. Use apply_user_inputs instead.
 */
void DynBodyInit::compute_translational_state()
{
    static bool reported = false;
 ...
 */
Planet * DynBodyInit::find_planet(const DynManager & dyn_manager,
 ...
 * \param[in] variable_name For error reporting
 */
DynBody * DynBodyInit::find_dyn_body(const DynManager & dyn_manager,
 ...
 * \param[in] variable_name For error reporting
 */
RefFrame * DynBodyInit::find_ref_frame(const DynManager & dyn_manager,
 ...
 * \param[in] variable_name For error reporting
 */
BodyRefFrame * DynBodyInit::find_body_frame(DynBody & frame_container,
                                            ...
                                            const std::string & variable_name)
{
    ...
    if(found_body_frame == nullptr)
    {
        MessageHandler::fail(__FILE__,
                             ...
                             action_identifier.c_str(),
                             body_frame_identifier.c_str(),
                             frame_container.name.c_str(),
                             variable_name.c_str());
