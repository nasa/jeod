 ...
 */
RefFrameItems::Items DynBodyInitRotState::initializes_what()
{
 ...
 */
bool DynBodyInitRotState::is_ready()
{
    ...
    return (body_frame == nullptr) || body_frame->initialized_items.contains(required_items);
}
 ...
 */
void DynBodyInitRotState::initialize(DynManager & dyn_manager)
{
    // Check that the state enum value is valid.
    if((state_items != Both) && (state_items != Attitude) && (state_items != Rate))
    {
 ...
 * \param[in,out] dyn_manager Dynamics manager
 */
void DynBodyInitRotState::apply(DynManager & dyn_manager)
{
