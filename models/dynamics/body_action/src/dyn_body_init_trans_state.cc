 ...
 */
RefFrameItems::Items DynBodyInitTransState::initializes_what()
{
 ...
 */
bool DynBodyInitTransState::is_ready()
{
    ...
    return (body_frame == nullptr) || body_frame->initialized_items.contains(required_items.get());
}
 ...
 */
void DynBodyInitTransState::initialize(DynManager & dyn_manager)
{
    // Check that the state enum value is valid.
    if((state_items != Both) && (state_items != Position) && (state_items != Velocity))
    {
 ...
 * \param[in,out] dyn_manager Dynamics manager
 */
void DynBodyInitTransState::apply(DynManager & dyn_manager)
{
