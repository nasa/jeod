...
NedDerivedState::~NedDerivedState()
{
    ...
    if(local_dm != nullptr)
    {
        local_dm->remove_ref_frame(ned_state.ned_frame);
    }
 ...
 */
void NedDerivedState::set_use_alt_pfix(const bool use_alt_pfix_in)
{
 ...
 */
void NedDerivedState::initialize(DynBody & subject_body, DynManager & dyn_manager)
{
    ...
    ned_state.ned_frame.set_name(subject_body.name.get_name(), reference_name, "ned");
 ...
 * Update the state.
 */
void NedDerivedState::update()
{
    // Update the NED frame based on the vehicle's state relative to the planet.
    subject->composite_body.compute_relative_state(*pfix_ptr, pfix_rel_state);
 ...
 */
void NedDerivedState::compute_ned_frame(const RefFrameTrans & rel_trans)
{
