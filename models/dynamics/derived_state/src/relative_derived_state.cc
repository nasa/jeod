 ...
 */
void RelativeDerivedState::initialize(DynBody & subject_body, DynManager & dyn_manager)
{
 ...
 * \param[in,out] dyn_manager Dynamics manager
 */
void RelativeDerivedState::initialize(DynManager & dyn_manager)
{
    ...
    else
    {
        ...
        subject_frame = dynamic_cast<BodyRefFrame *>(dyn_manager.find_ref_frame(subject_frame_name));
    }
 ...
 * Update the state.
 */
void RelativeDerivedState::update()
{
 ...
 * for RelKin manager
 */
void RelativeDerivedState::set_activation_flag(bool raf)
{
    // true = on; false = off
    ...
    { // If relstate turned off, unsubscribe subject and target frames
