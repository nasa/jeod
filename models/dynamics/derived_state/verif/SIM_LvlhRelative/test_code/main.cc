...
// Check curvilinear calculations
  ...
  double **computedCurviPosAinB = new double *[numRows];
  double **computedCurviVelAinB = new double *[numRows];
  double **computedCurviAngVelAinB=new double *[numRows];
  double **computedCurviPosBinA = new double *[numRows];
  double **computedCurviVelBinA = new double *[numRows];
  double **computedCurviAngVelBinA=new double *[numRows];
...
velocityA = positionA+3;
    ...
    if (relValues[ii][numCols-2] == 1.0) {
      thetaDot = dot(hA, normalToPlaneOfB)/mag2(perp) -
                 mag(hB)/mag2(positionB);
...
// Compute curviPos|VelBinA
    ...
    if (relValues[ii][numCols-1] == 1.0) {
      thetaDot = dot(hB, normalToPlaneOfA)/mag2(perp) -
                     mag(hA)/mag2(positionA);
