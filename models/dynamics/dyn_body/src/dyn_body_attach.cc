...
// point of view.
bool
DynBody::attach_validate_parent (
...
const
{
    ...
    else if( !dyn_manager->is_dyn_body_registered(&parent) )
    {
        ...
        if (generate_message) {
            MessageHandler::warn (
                    __FILE__, __LINE__, DynBodyMessages::not_dyn_body,
...
const
{
   ...
   if ((! is_valid) && generate_message) {
      MessageHandler::warn (
         ...
         "%s\n"
         "Attachment of body '%s' to '%s' was not performed.",
         reason.c_str(), child.name.c_str(), name.c_str());
   }
 ...
 */
bool DynBody::add_mass_body_validate (
   const MassBody & child, bool generate_message)
const
{
    bool is_valid = true;
    ...
    else if( child.get_root_body() != nullptr )
    {
        std::string root_name = child.get_root_body()->name.get_name();
        DynBody* parent_check = dyn_manager->find_dyn_body(root_name.c_str());
   ...
   DynBody & child )
{
    ...
    if(!success) {
        MessageHandler::warn (
                ...
                "%s\n"
                "Attachment of '%s' to '%s' was not performed.\n",
                reason.c_str(), child_point_name, this_point_name);
        return success;
    }
    ...
    // within its own body)
    Vector3::transform( child_point->T_parent_this,
            child_point->position,
            child_struct_wrt_child_point.position );
    Vector3::negate( child_struct_wrt_child_point.position );
    Matrix3x3::transpose( child_point->T_parent_this,
            child_struct_wrt_child_point.T_parent_this );
    child_point->Q_parent_this.conjugate(
            child_struct_wrt_child_point.Q_parent_this);
    ...
    Vector3::transform(
            T_tmp,
            child_struct_wrt_child_point.position,
            child_struct_wrt_parent_struct.position);
   ...
   DynBody & child )
{
    ...
    // Validate
    if(   !attach_validate_child( child, true )
       || !child.attach_validate_parent(*this,true) )
    {
        success = false;
        return success;
    }
    ...
    else
    {
        ...
        MessageHandler::inform (
                 ...
                 child.name.c_str(), child_root->name.c_str(),
                 child.name.c_str(), name.c_str(),
                 child_root->name.c_str(), name.c_str() );

        // Compute the affine transformation from the root structural frame
        // to child body's structural frame.
        child.mass.structure_point.compute_state_wrt_pred (
                child_root->mass.structure_point,
        ...
        Matrix3x3::product_left_transpose (
                child_struct_wrt_root_struct.T_parent_this,
                T_pstr_to_cstr,
                T_pstr_to_rstr);

        Vector3::copy ( xyz_cstr_wrt_pstr,
                        xyz_rstr_wrt_pstr );
        ...
        attach_update_properties( xyz_rstr_wrt_pstr,
                                  T_pstr_to_rstr,
                                  *child_root );
        success = true;
    }
   ...
   MassBody & child)
{
    ...
    else
    {
        ...
        MessageHandler::inform (
                 ...
                 child.name.c_str(), child.links.parent()->name.c_str(),
                 child.name.c_str(), name.c_str(),
                 child_root->name.c_str(), name.c_str() );

        // Compute the affine transformation from the root structural frame
        // to child body's structural frame.
        child.structure_point.compute_state_wrt_pred (
                child_root->structure_point,
                child_struct_wrt_root_struct);
        ...
        mass.attach_update_properties( xyz_rstr_wrt_pstr,
                                       T_pstr_to_rstr,
                                       *child_root );
...
void
DynBody::attach_establish_links (
   DynBody & parent)
{
   ...
   if (integ_frame != dyn_parent->integ_frame) {
      set_integ_frame (*(dyn_parent->integ_frame));
   }
}
   ...
   DynBody & child)
{
   ...
   {
      mass.attach_update_properties (
         offset_pstr_cstr_pstr, T_pstr_cstr, child.mass);
   }
   ...
   // this body.
      ...
      process_dynamic_attachment (offset_pstr_cstr_pstr, T_pstr_cstr,
                                  *root_body, child);
   ...
   DynBody & child_body)
{
   ...
   Vector3::sum (linear_momentum_child_composite,
                 linear_momentum_parent_composite,
                 linear_momentum);
   ...
   Vector3::copy (root_body.composite_body.state.trans.velocity,
                  parent_vel_inrtl);
   Vector3::copy (child_body.composite_body.state.trans.velocity,
                  child_vel_inrtl);
   ...
   mass.attach_update_properties (
      offset_pstr_cstr_pstr, T_pstr_cstr, child_body.mass);
   ...
   Vector3::transform_transpose (root_body.structure.state.rot.T_parent_this,
                                 cm_delta,
                                 cm_delta_inrtl);

   Vector3::incr (cm_delta_inrtl,
   ...
   Vector3::diff (child_vel_inrtl,
                  root_body.composite_body.state.trans.velocity,
                  child_vel_relative);

   Vector3::scale (child_vel_relative,
                   child_body.mass.composite_properties.mass,
                   child_lin_mom_relative);

   Vector3::diff (parent_vel_inrtl,
                  root_body.composite_body.state.trans.velocity,
                  parent_vel_relative);

   Vector3::scale( parent_vel_relative,
                   mass_parent,
                   parent_lin_mom_relative );
   ...
   Vector3::cross (child_pos_rel_newcm_inrtl,
                   child_lin_mom_relative,
                   child_ang_mom_vel);
   ...
   Vector3::cross (parent_lin_mom_relative,
                   cm_delta_inrtl,
                   parent_ang_mom_vel);
   ...
   Vector3::transform_transpose (
      child_body.composite_body.state.rot.T_parent_this,
      child_ang_mom_cbdy,
      child_ang_mom_inrtl);
   ...
   Vector3::transform (root_body.composite_body.state.rot.T_parent_this,
                       angular_momentum);
   ...
   Vector3::incr (parent_ang_mom_pbdy,
                  angular_momentum);
   ...
   root_body.set_state_source_internal (RefFrameItems::Pos_Att,
                                        root_body.core_body);
   root_body.set_state_source_internal (RefFrameItems::Vel_Rate,
