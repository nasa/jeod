...
bool DynBody::detach(DynBody & other_body)
{
    ...
    if(parent == nullptr || detacher == nullptr)
    {
        success = false;
    ...
    for(it = parent->dyn_children.begin(); it != parent->dyn_children.end(); ++it)
    {
        if(*it == detacher)
        {
...
bool DynBody::detach()
{
    ...
    else
    {
        ...
        MessageHandler::inform(__FILE__,
                               __LINE__,
                               DynBodyMessages::invalid_technique,
                               "\nDuring detach process:\n"
...
bool DynBody::remove_mass_body(MassBody & child)
{
    // Look up references
...
void DynBody::detach_mass_body_frames(MassBody & subbody)
{
    ...
    {
        pt_frame = find_body_frame(std::string(mass_point->get_name()));
        if(pt_frame != nullptr)
        {
            veh_pt = std::find(vehicle_points.begin(), vehicle_points.end(), pt_frame);
...
void DynBody::detach_mass_internal( // Return: -- Void
    MassBody & child)               /* Inout: -- The newly detached child */
{
    ...
    root_body->set_state_source_internal(RefFrameItems::Pos_Vel_Att_Rate, root_body->core_body);
