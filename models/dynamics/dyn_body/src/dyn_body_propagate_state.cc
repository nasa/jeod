...
void DynBody::compute_ref_point_transform(const BodyRefFrame & source_frame,
                                          const MassPoint ** const ref_point,
                                          MassPointState & rel_state)
{
    const MassPoint * source_point;
    const MassPoint * target_point;
    const auto * source_body = dynamic_cast<const DynBody *>(source_frame.get_owner());
    ...
    if(integrated_frame == &structure)
    {
        ...
        if(*ref_point != source_point)
        {
            *ref_point = source_point;
            source_point->compute_state_wrt_pred(mass.structure_point, rel_state);
        }
    }
    ...
    else if(integrated_frame == &composite_body)
    {
        ...
        if(*ref_point != source_point)
        {
            *ref_point = source_point;
            ...
            source_point->compute_state_wrt_pred(*target_point, rel_state);
    ...
    else
    {
        MessageHandler::fail(__FILE__,
                             ...
                             "Body '%s' has an invalid integration frame.",
                             name.c_str());
    }
}
...
void DynBody::compute_derived_state_forward(const BodyRefFrame & source_frame,
                                            ...
                                            BodyRefFrame & derived_frame) const
{
    ...
    Vector3::transform(rel_state.T_parent_this,
                       source_frame.state.rot.ang_vel_this,
    ...
    Vector3::copy(source_frame.state.trans.position, derived_frame.state.trans.position);
    Vector3::transform_transpose_incr(source_frame.state.rot.T_parent_this,
                                      rel_state.position,
                                      derived_frame.state.trans.position);

    // v_A->C:A = v_A->B:A + v_B->C:A
    //   = v_A->B:A + T_A->B^T * (w_A->B:B X r_B->C:B)
    Vector3::copy(source_frame.state.trans.velocity, derived_frame.state.trans.velocity);
    Vector3::cross(source_frame.state.rot.ang_vel_this, rel_state.position, wxr);
    Vector3::transform_transpose_incr(source_frame.state.rot.T_parent_this, wxr, derived_frame.state.trans.velocity);
...
void DynBody::compute_state_elements_forward(const BodyRefFrame & source_frame,
                                             const MassPoint & rel_state,
...
void DynBody::compute_derived_state_reverse(const BodyRefFrame & source_frame,
                                            const MassPoint & rel_state,
                                            BodyRefFrame & derived_frame) const
{
    double wxr[3];
    ...
    Vector3::copy(source_frame.state.trans.position, derived_frame.state.trans.position);
    Vector3::transform_transpose_decr(derived_frame.state.rot.T_parent_this,
                                      rel_state.position,
                                      derived_frame.state.trans.position);
    ...
    Vector3::copy(source_frame.state.trans.velocity, derived_frame.state.trans.velocity);
    Vector3::cross(derived_frame.state.rot.ang_vel_this, rel_state.position, wxr);
    Vector3::transform_transpose_decr(derived_frame.state.rot.T_parent_this, wxr, derived_frame.state.trans.velocity);
...
// Compute selected aspects of the derived state.
void DynBody::compute_state_elements_reverse(const BodyRefFrame & source_frame,
                                             const MassPoint & rel_state,
                                             const RefFrameItems & state_items,
                                             BodyRefFrame & derived_frame) const
{
...
void DynBody::update_integrated_state()
{
    ...
    else if((rate_source != nullptr) && (attitude_source == integrated_frame))
    {
        ...
        Vector3::transform_transpose(rel_state.T_parent_this,
                                     rate_source->state.rot.ang_vel_this,
    ...
    // Convert to the integrated frame if possible and note that rate is
    // initialized. Attitude is needed to perform this conversion.
    else if((position_source != nullptr) && (attitude_source == integrated_frame))
    {
        ...
        // r_A->C:A = r_A->B:A + r_B->C:A
        //   = r_A->B:A - T_A->C^T * r_C->B:C
        Vector3::copy(position_source->state.trans.position, integrated_frame->state.trans.position);
        Vector3::transform_transpose_decr(integrated_frame->state.rot.T_parent_this,
                                          rel_state.position,
    ...
    else if((velocity_source != nullptr) && (attitude_source == integrated_frame) && (rate_source == integrated_frame))
    {
        double wxr[3];
        ...
        Vector3::copy(velocity_source->state.trans.velocity, integrated_frame->state.trans.velocity);
        Vector3::cross(integrated_frame->state.rot.ang_vel_this, rel_state.position, wxr);
        Vector3::transform_transpose_decr(integrated_frame->state.rot.T_parent_this,
                                          wxr,
...
void DynBody::propagate_state_from_structure()
{
    ...
    if(structure.initialized_items.is_full())
    {
        ...
        // for this object.
        compute_derived_state_forward(structure, mass.composite_properties, composite_body);

        compute_derived_state_forward(structure, mass.core_properties, core_body);
        ...
        for(auto child : dyn_children)
        {
            ...
            compute_derived_state_forward(structure, child->mass.structure_point, child->structure);
    ...
    else
    {
        ...
        // Propagate the structure state to the composite and core body states
        // for this object.
        compute_state_elements_forward(structure, mass.composite_properties, propagated_states, composite_body);
        compute_state_elements_forward(structure, mass.core_properties, propagated_states, core_body);
        ...
        {
            // Propagate the structure state to the child body's structure state.
            compute_state_elements_forward(structure, child->mass.structure_point, propagated_states, child->structure);
...
void DynBody::propagate_state_from_composite()
{
    // Use the full propagation methods if the state is full.
    if(composite_body.initialized_items.is_full())
    {
        ...
        // for this object.
        compute_derived_state_reverse(composite_body, mass.composite_properties, structure);
        compute_derived_state_forward(composite_body, mass.core_wrt_composite, core_body);
        ...
        for(auto child : dyn_children)
        {
            ...
            compute_derived_state_forward(composite_body, child->mass.composite_wrt_pbdy, child->composite_body);
    ...
    else
    {
        ...
        // Propagate the composite state to the structure and core body states
        // for this object.
        compute_state_elements_reverse(composite_body, mass.composite_properties, propagated_states, structure);
        compute_state_elements_forward(composite_body, mass.core_wrt_composite, propagated_states, core_body);
        ...
        {
            // Propagate the structure state to the child body's structure state.
            compute_state_elements_forward(composite_body,
                                           child->mass.composite_wrt_pbdy,
                                           propagated_states,
...
void DynBody::compute_vehicle_point_states(RefFrameItems::Items set_items)
{
    ...
    else
    {
        for(auto point : vehicle_points)
        {
            ...
            compute_state_elements_forward(structure, *(point->mass_point), items, *point);
        }
    }
}
