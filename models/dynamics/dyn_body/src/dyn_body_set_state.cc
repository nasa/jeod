   ...
   unsigned int line)
{
   if (frame.get_owner() != dyn_body) {
      MessageHandler::fail (
         file, line, DynBodyMessages::invalid_frame,
         "Body '%s' is not the owner of reference frame '%s'.",
         dyn_body->name.c_str(), frame.get_name());
   }
}
   ...
   BodyRefFrame & subject_frame)
{
   ...
   if ((set_items & RefFrameItems::Pos) != 0) {
      ...
      Vector3::copy (state.trans.position,
                     subject_frame.state.trans.position);
   }
   ...
   if ((set_items & RefFrameItems::Vel) != 0) {
      ...
      Vector3::copy (state.trans.velocity,
                     subject_frame.state.trans.velocity);
   }
   ...
   if ((set_items & RefFrameItems::Att) != 0) {
      ...
      state.rot.Q_parent_this.left_quat_to_transformation (
         subject_frame.state.rot.T_parent_this);
   }
   ...
   if ((set_items & RefFrameItems::Rate) != 0) {
      ...
      Vector3::copy (state.rot.ang_vel_this,
                     subject_frame.state.rot.ang_vel_this);
   ...
   BodyRefFrame & frame)
{
   ...
   if ((source_body != this) &&
       (source_body->get_root_body() != this)) {
      MessageHandler::fail (
         __FILE__, __LINE__, DynBodyMessages::invalid_frame,
         "Reference frame '%s', is owned by body '%s', "
         "which is not a child body of '%s'.",
         frame.get_name(), source_body->name.c_str(), name.c_str());
   ...
   BodyRefFrame & subject_frame)
{
   ...
   subject_frame.state.rot.Q_parent_this.left_quat_to_transformation (
      subject_frame.state.rot.T_parent_this);
}
   ...
   BodyRefFrame & subject_frame)
{
   ...
   subject_frame.state.rot.Q_parent_this.left_quat_from_transformation (
      subject_frame.state.rot.T_parent_this);
}
...
// Set the source of aspects of the state.
void
DynBody::set_state_source_internal (
   RefFrameItems::Items items,
   BodyRefFrame & frame)
{
