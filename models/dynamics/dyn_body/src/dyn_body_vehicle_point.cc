...
void DynBody::add_mass_point(const MassPointInit & mass_point_init)
{
...
const BodyRefFrame * DynBody::find_vehicle_point(const std::string & pt_name) const
{
    ...
    if(!pt_name.empty())
    {
        std::string pt_suffix = name.suffix(pt_name);

        // Search for the point.
        auto name_compare = [&search_offset, &pt_suffix](const BodyRefFrame * point)
        {
            return point->get_name().compare(search_offset, std::string::npos, pt_suffix) == 0;
        };

        const auto iter = std::find_if(vehicle_points.begin(), vehicle_points.end(), name_compare);
 ...
 */
void DynBody::compute_vehicle_point_derivatives(const BodyRefFrame & vehicle_pt, FrameDerivs & pt_derivs)
{
    ...
    Vector3::transform_transpose(root_rot.T_parent_this, cm_to_point.position, cm_to_point_inrtl);
    ...
    Vector3::transform(grav_interaction.grav_grad, cm_to_point_inrtl, delta_acc_grav);

    // Compute accelerations.
    Vector3::diff(accel_rot_inrtl, delta_acc_grav, delta_acc_non_g);
    Vector3::sum(root_derivs.non_grav_accel, delta_acc_non_g, pt_derivs.non_grav_accel);
    Vector3::sum(root_derivs.trans_accel, accel_rot_inrtl, pt_derivs.trans_accel);
    Vector3::transform(cm_to_point.T_parent_this, root_derivs.rot_accel, pt_derivs.rot_accel);
