...
void
StructureIntegratedDynBody::collect_forces_and_torques ()
{
   ...
   // Child body: Propagate forces, torques to the parent body.
    ...
    else
    {
        ...
        {
            effector_wrench_co_m = effector_wrench.transform_to_point (
        ...
        if (rotational_dynamics)
        {
            ...
            Vector3::incr (effector_wrench_co_m.get_torque(),
                           collect.extern_torq_struct);
        ...
        if (translational_dynamics) {
            ...
            Vector3::incr (effector_wrench_co_m.get_force(),
                           collect.extern_forc_struct);
...
void
StructureIntegratedDynBody::collect_local_forces_and_torques ()
{
    ...
    if (translational_dynamics) {
        accumulate_forces (
            collect.collect_effector_forc, collect.effector_forc);
        accumulate_forces (
            collect.collect_environ_forc, collect.environ_forc);
        accumulate_forces (
            collect.collect_no_xmit_forc, collect.no_xmit_forc);
    }
    ...
    {
        accumulate_torques (
            collect.collect_effector_torq, collect.effector_torq);
        accumulate_torques (
            collect.collect_environ_torq, collect.environ_torq);
        accumulate_torques (
            collect.collect_no_xmit_torq, collect.no_xmit_torq);
    }
...
void
StructureIntegratedDynBody::PropagateForcesAndTorques ()
{
    ...
    if (translational_dynamics && rotational_dynamics)
    {
        StructureIntegratedDynBody* struct_parent =
            dynamic_cast<StructureIntegratedDynBody*>(dyn_parent);
        struct_parent->effector_wrench +=
            effector_wrench.transform_to_parent(mass.structure_point);
    }
    ...
    if (translational_dynamics) {
        ...
        Vector3::incr (effector_force_parent_structure,
                       dyn_parent->collect.effector_forc);
        Vector3::incr (environ_force_parent_structure,
    ...
    if (rotational_dynamics) {
        ...
        Vector3::cross_incr (
            parent_co_m_to_child_co_m, effector_force_parent_structure,
            effector_torque_parent_structure);
        Vector3::cross_incr (
            parent_co_m_to_child_co_m, environ_force_parent_structure,
            environ_torque_parent_structure);

        // Transmit the torques to the parent dyn body.
        Vector3::incr (effector_torque_parent_structure,
                       dyn_parent->collect.effector_torq);
        Vector3::incr (environ_torque_parent_structure,
...
void
StructureIntegratedDynBody::compute_inertial_torque ()
{
    ...
    Vector3::transform( mass.composite_properties.T_parent_this,
                        structure.state.rot.ang_vel_this,
                        ang_vel_body);
...
void
StructureIntegratedDynBody::compute_rotational_acceleration ()
{
    ...
    Vector3::diff (
        collect.extern_torq_body, collect.inertial_torq,
        torque_body);

    // Solve the rotational EOM for body accelerations.
    Vector3::transform( mass.composite_properties.inverse_inertia,
                        torque_body,
                        derivs.rot_accel );
...
void
StructureIntegratedDynBody::compute_translational_acceleration()
{
    ...
    Vector3::transform_transpose (
        structure.state.rot.T_parent_this, collect.extern_forc_struct,
        collect.extern_forc_inrtl);

    // Compute accelerations at the composite center of mass.
    Vector3::scale( collect.extern_forc_inrtl,
    ...
    Vector3::cross (
        mass.composite_properties.position, structure.state.rot.ang_vel_this,
        omega_cross_r);
    Vector3::cross (
        structure.state.rot.ang_vel_this, omega_cross_r,
...
// Compute the inertial-referenced translational acceleration vector.
void
StructureIntegratedDynBody::complete_translational_acceleration ()
{

    // Compute translational acceleration at the composite center of mass.
    Vector3::sum (
        derivs.non_grav_accel, grav_interaction.grav_accel,
        derivs.trans_accel);
    ...
    Vector3::cross (
        mass.composite_properties.position, struct_derivs.rot_accel,
        inertial_accel_struct_omega_dot);
    Vector3::sum (
        inertial_accel_struct_omega, inertial_accel_struct_omega_dot,
        inertial_accel_struct);
    Vector3::transform_transpose (
        structure.state.rot.T_parent_this, inertial_accel_struct,
        inertial_accel_inrtl);

    // Compute accelerations at the structural origin.
    Vector3::sum (
        derivs.non_grav_accel, inertial_accel_inrtl,
        struct_derivs.non_grav_accel);
    Vector3::sum (
        struct_derivs.non_grav_accel, grav_interaction.grav_accel,
