...
er7_utils::IntegratorResult StructureIntegratedDynBody::trans_integ(double dyn_dt, unsigned int target_stage)
{
    // Integrate the translational state.
    return trans_integrator.integrate(dyn_dt,
                                      target_stage,
                                      struct_derivs.trans_accel,
                                      structure.state.trans.velocity,
                                      structure.state.trans.position);
}

// Integrate the rotational state of a StructureIntegratedDynBody.
er7_utils::IntegratorResult StructureIntegratedDynBody::rot_integ(double dyn_dt, unsigned int target_stage)
{
