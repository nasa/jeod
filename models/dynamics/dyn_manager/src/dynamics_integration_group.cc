 ...
 */
DynamicsIntegrationGroup::DynamicsIntegrationGroup(JeodIntegrationGroupOwner & owner,
 ...
 */
void DynamicsIntegrationGroup::register_base_contents()
{
 ...
 */
DynamicsIntegrationGroup * DynamicsIntegrationGroup::create_group(JeodIntegrationGroupOwner & owner,
                                                                  er7_utils::IntegratorConstructor & integ_cotr,
                                                                  JeodIntegratorInterface & integ_inter,
                                                                  JeodIntegrationTime & time_mngr) const
{
    return JEOD_ALLOC_CLASS_OBJECT(DynamicsIntegrationGroup, (owner, integ_cotr, integ_inter, time_mngr));
}
 ...
 */
void DynamicsIntegrationGroup::register_group(DynManager & dyn_manager)
{
 ...
 */
void DynamicsIntegrationGroup::initialize_group(DynManager &)
{
    ...
    if(bodies_integrated_separately)
    {
        ...
        {
            DynBody * body = *it;
            body->create_body_integrators(*integ_constructor, *integ_controls, *jeod_time_manager);
        }
    }
 ...
 */
void DynamicsIntegrationGroup::add_dyn_body(DynBody & dyn_body)
{
    bool found = false;
    ...
    {
        // Create the integrators for this body.
        dyn_body.create_body_integrators(*integ_constructor, *integ_controls, *jeod_time_manager);

        // Add any integrable objects associated with the dyn_body.
        JeodPointerVector<er7_utils::IntegrableObject>::type associated_objects = dyn_body.get_integrable_objects();

        for(std::vector<er7_utils::IntegrableObject *>::const_iterator it = associated_objects.begin();
 ...
 */
void DynamicsIntegrationGroup::delete_dyn_body(DynBody & dyn_body)
{
 ...
 */
void DynamicsIntegrationGroup::prepare_for_integ_loop(double sim_endtime)
{
 ...
 */
void DynamicsIntegrationGroup::gravitation(DynManager & dyn_manager, GravityManager & gravity_manager)
{
    ...
    for(std::vector<DynBody *>::const_iterator it = dyn_bodies.begin(); it != dyn_bodies.end(); ++it)
    {
        ...
        if(body->is_root_body())
        {
            ...
            gravity_manager.gravitation(body->composite_body, body->grav_interaction);
        }
    }
}
 ...
 */
void DynamicsIntegrationGroup::collect_derivatives()
{
 ...
 */
void DynamicsIntegrationGroup::reset_body_integrators()
{
 ...
 */
er7_utils::IntegratorResult DynamicsIntegrationGroup::integrate_bodies(double cycle_dyndt, unsigned int target_stage)
{
    ...
    {
        status = integrate_container(cycle_dyndt, target_stage, integrable_objects);
    }
    ...
    for(std::vector<DynBody *>::const_iterator it = dyn_bodies.begin(); it != dyn_bodies.end(); ++it)
    {
        ...
        if(body->is_root_body())
        {
            ...
            integ_merger.merge_integrator_result(body->integrate(cycle_dyndt, target_stage), status);
        }
    }
