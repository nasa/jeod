 ...
 */
void DynManager::initialize_dyn_bodies()
{
    ...
    for(std::list<BodyAction *>::const_iterator it = body_actions.begin(); it != body_actions.end(); ++it)
    {
 ...
 */
void DynManager::initialize_dyn_body(DynBody & body)
{
 ...
 * \param[in,out] body Body to be initialized
 */
void DynManager::perform_mass_body_initializations(MassBody * body)
{
 ...
 * and apply those that are immediately ready to be applied.
 */
void DynManager::perform_mass_attach_initializations()
{
 ...
 * \param[in,out] body Body to be initialized
 */
void DynManager::perform_dyn_body_initializations(DynBody * body)
{
    // Initialize the queued actions that derive from DynBodyInit.

    for(auto action : body_actions)
    {
        ...
        if(dynamic_cast<DynBodyInit *>(action) != nullptr)
        {
            ...
            if((body == nullptr) || (action->is_same_subject_body(body->mass)))
            {
                // Initialize the action.
                action->initialize(*this);
            }
    ...
    if(more_inits_to_do)
    {
        ...
        for(auto action : body_actions)
        {
            auto * body_init = dynamic_cast<DynBodyInit *>(action);
 ...
 */
void DynManager::check_for_uninitialized_states()
{
    ...
    for(std::vector<DynBody *>::const_iterator it = dyn_bodies.begin(); it != dyn_bodies.end(); ++it)
    {
        ...
        if(!missing_states.is_empty())
        {
            ...
            MessageHandler::error(__FILE__,
                                  ...
                                  body->name.c_str(),
                                  missing_states.to_string().c_str());
        }
    }
    ...
    {
        MessageHandler::fail(__FILE__, __LINE__, DynManagerMessages::inconsistent_setup, "Uninitialized states");
    }
}
