 ...
 */
bool DynManager::is_integ_group_registered(const DynamicsIntegrationGroup * integ_group) const
{
    return std::find(integ_groups.begin(), integ_groups.end(), integ_group) != integ_groups.end();
}
 ...
 */
void DynManager::add_integ_group(DynamicsIntegrationGroup & integ_group)
{
