...
class MassBody {
  ...
  JEOD_MAKE_SIM_INTERFACES(MassBody)
...
public:
   ...
   static void compute_point_mass_inertia (
      double mass, const double r_pt[3], double inertia[3][3]);
   ...
   bool get_mass_properties_initialized() {return mass_properties_initialized;}
   ...
   virtual bool attach_validate (const MassBody & parent, bool generate_message)
   const;
   ...
   // FIXME: Virtual?
   void reattach (double offset[3], double T_pstr_cstr[3][3]);


   // Mass points methods

   const MassPoint * find_mass_point (const char * pt_name) const;

   virtual void add_mass_point (const MassPointInit & mass_point_init);

   std::size_t mass_points_size (void) const;
   ...
   virtual const MassBody * get_parent_body (void) const;
   virtual const MassBody * get_root_body (void) const;


   // Mass update methods

   void set_update_flag (void);
   virtual void update_mass_properties (void);
...
protected:
   ...
   virtual bool attach_validate_parent (
      const MassBody & parent, bool generate_message) const;
   ...
   // Test validity from connectivity point of view (sent to child)
   virtual bool detach_validate_parent (
      const MassBody * parent, bool generate_message) const;

   // Test validity from properties point of view (sent to parent)
   virtual bool detach_validate_child (
      const MassBody & child, bool generate_message) const;
   ...
   virtual MassBody * get_parent_body_internal (void);
   virtual MassBody * get_root_body_internal (void);


   // Mass update methods

   void calc_composite_cm (void);
   void calc_composite_inertia (void);
...
private:
    ...
    * Not implemented.
    */
   MassBody (const MassBody& rhs);

   /**
    * Not implemented.
    */
   void operator = (const MassBody& rhs);
};
