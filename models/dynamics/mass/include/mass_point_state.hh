...
{
    JEOD_MAKE_SIM_INTERFACES(jeod, MassPointState)
...
public:
    // NOTES:
    // - initialize_mass_point() does the exact same thing, but is a virtual
    //   function, and thus should not be called from the constructor EXCEPT
    // - initialize_mass_point() resets the quaternion to an identity.
    //   The default constructor for the quaternion already did just that.
    MassPointState() = default;
 ...
 */
inline void MassPointState::update_point(const double pt_location[3])
{
 ...
 */
inline void MassPointState::update_orientation(const double transformation[3][3])
{
 ...
 */
inline void MassPointState::update_orientation(const Quaternion & left_quat)
{
 ...
 */
inline void MassPointState::compute_quaternion()
{
    Q_parent_this.left_quat_from_transformation(T_parent_this);
}
 ...
 */
inline void MassPointState::compute_transformation()
{
    // Save the new structure-to-point transformation quaternion.
    Q_parent_this.left_quat_to_transformation(T_parent_this);
}
