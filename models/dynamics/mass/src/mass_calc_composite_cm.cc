 ...
 */
void MassBody::calc_composite_cm()
{
    ...
    Vector3::scale(core_properties.position, core_properties.mass, mass_times_com);
    ...
    for(auto * child_link : TreeLinksChildrenRange<MassBodyLinks>(links))
    {
        ...
        Vector3::scale_incr(child->composite_wrt_pstr.position, child->composite_properties.mass, mass_times_com);
    }
    ...
    if(mass > 0.0)
    {
        ...
        Vector3::scale(mass_times_com, composite_properties.inverse_mass, composite_properties.position);
    }
    else
    {
