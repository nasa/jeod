 ...
 */
void MassBody::calc_composite_inertia()
{
    ...
    compute_point_mass_inertia(core_properties.mass, core_wrt_composite.position, offset_inertia);

    // Compute the contribution to the composite inertia by the core mass.
    Matrix3x3::add(offset_inertia, core_properties.inertia, composite_properties.inertia);
    ...
    for(auto * child_link : TreeLinksChildrenRange<MassBodyLinks>(links))
    {
        ...
        Matrix3x3::transpose_transform_matrix(child->composite_wrt_pbdy.T_parent_this,
