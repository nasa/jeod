 ...
 */
void MassPoint::initialize_mass_point()
{
    detach();
 ...
 */
void MassPoint::compute_relative_state(const MassPoint & ref_point, MassPointState & rel_state) const
{
    ...
    else
    {
        ...
        for(auto * link : TreeLinksDescentRange<const MassPointLinks>(ref_point.links, common_node_index + 1))
        {
 ...
 */
void MassPoint::compute_state_wrt_pred(const MassPoint & ref_point, MassPointState & rel_state) const
{
 ...
 * \param[out] rel_state Point state wrt ref. pt.
 */
void MassPoint::compute_state_wrt_pred(unsigned int ref_point_index, MassPointState & rel_state) const
{
    ...
    else
    {
        ...
        for(auto * link :
            TreeLinksAscendRange<const MassPointLinks>(links, links.path_length() - 1, ref_point_index + 1))
        {
            const MassPoint & node = link->container();

            // r_A->C:A = r_A->B:A + T_A:B^T * r_B:C:B
            Vector3::transform_transpose(node.T_parent_this, rel_state.position, r_temp);
 ...
 * \param[out] rel_state The relative state
 */
void MassPoint::compute_pred_rel_state(const MassPoint & ref_point, MassPointState & rel_state) const
{
    unsigned ref_point_idx = links.find_path_index(ref_point.links);
 ...
 */
void MassPoint::compute_pred_rel_state(unsigned int ref_point_index, MassPointState & rel_state) const
{
    ...
    else
    {
            ...
            TreeLinksAscendRange<const MassPointLinks>(links, links.path_length() - 1, ref_point_index + 1))
        {
            ...
            Vector3::transform_transpose_decr(rel_state.T_parent_this, node.position, rel_state.position);
        }
    }
}
