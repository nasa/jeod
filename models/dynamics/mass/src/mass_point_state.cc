...
/**
 * Default constructor; constructs a MassPointState object.
 */
MassPointState::MassPointState (
   void)
{
 ...
 */
void
MassPointState::incr_left (
   const MassPointState & s_ab)
{
   // Relevant equations:
   //   T_A:C = T_B:C * T_A:B
   //   x_A:C = x_A:B + T_A:B^T * x_B:C
 ...
 */
void
MassPointState::incr_right (
   const MassPointState & s_bc)
{
   // Relevant equations:
   //   T_A:C = T_B:C * T_A:B
   //   x_A:C = x_A:B + T_A:B^T * x_B:C
   ...
   if (!Numerical::compare_exact(Q_parent_this.scalar,1.0)) {
      ...
      Vector3::transform_transpose_incr (T_parent_this, s_bc.position,
                                         position);
   ...
   const MassPointState & s_bc)
{
   ...
   //   x_A:B = x_A:C - T_A:B^T * x_B:C
   if (!Numerical::compare_exact(Q_parent_this.scalar,1.0)) {
      Vector3::transform_transpose_decr (T_parent_this, s_bc.position,
                                         position);
   }
   // Shortcuts for the case T_A:B is identity.
   else {
