 ...
 * \param[in] offset Desired offset from parent struct frame to attached child's struct frame, expressed in parent
 * struct frame\n Units: M \param[in] T_pstr_cstr Desired transformation matrix from parent struct frame to attached
 * child's struct frame
 */
bool MassBody::reattach(double offset[3], double T_pstr_cstr[3][3])
{
