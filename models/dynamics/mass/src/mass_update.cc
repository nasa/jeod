   ...
   void)
{
   ...
   if (links.is_atomic()) {
      ...
      Matrix3x3::copy (core_properties.inertia,
   ...
   // Calculate the composite properties for a composite body.
      ...
      for (auto* link : TreeLinksChildrenRange<MassBodyLinks>(links)) {
         ...
         Vector3::diff (child.composite_wrt_pstr.position,
                        composite_properties.position,
                        r_cm_cm_str);
         Vector3::transform (composite_properties.T_parent_this, r_cm_cm_str,
      ...
      Vector3::diff (core_properties.position, composite_properties.position,
                     r_cm_cm_str);
      Vector3::transform (composite_properties.T_parent_this, r_cm_cm_str,
   ...
   if (links.is_root()) {
      if (compute_inverse_inertia && (composite_properties.mass > 0.0)) {
         Matrix3x3::invert_symmetric (
            composite_properties.inertia, composite_properties.inverse_inertia);
      } else {
   ...
   // structure.
      ...
      Vector3::transform_transpose (
         structure_point.T_parent_this, composite_properties.position,
