...
{
    JEOD_MAKE_SIM_INTERFACES(jeod, PlanetRotation)
...
protected: // protected member variables
     ...
     */
    double RADTODEG{180.0 / M_PI}; //!< trick_units(degree/rad)

    /**
     * Inverse of RADTODEG
     */
    double DEGTORAD{M_PI / 180.0}; //!< trick_units(rad/degree)

    /**
     * Convert from degrees to arcseconds
     */
    double DEGTOSEC{60.0 * 60.0}; //!< trick_units(arcsecond/degree)

    /**
     * Inverse of DEGTOSEC
     */
    double SECTODEG{1.0 / (60.0 * 60.0)}; //!< trick_units(degree/arcsecond)

    /**
     * Convert from julian day to seconds
     */
    double JULIANDAYTOSEC{86400.0}; //!< trick_units(s/day)

    /**
     * Inverse of JULIANDAYTOSEC
     */
    double SECTOJULIANDAY{1.0 / 86400.0}; //!< trick_units(day/s)

    /**
     * Convert from julian centuries to julian days
     */
    double JULIANCENTTODAY{36525.0}; //!< trick_units(--)

    /**
     * Inverse of JULIANCENTTODAY
     */
    double DAYTOJULIANCENT{1.0 / 36525.0}; //!< trick_units(--)

public: // public member functions
    PlanetRotation() = default;
    virtual ~PlanetRotation() = default;
    PlanetRotation & operator=(const PlanetRotation &) = delete;
    PlanetRotation(const PlanetRotation &) = delete;
    ...
    /*
      Purpose:
        (Initializes the invoking object from an initialization object.)
     */
    virtual void initialize(PlanetRotationInit * init JEOD_UNUSED)
    {
        return;
    }
