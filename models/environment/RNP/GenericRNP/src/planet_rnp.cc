...
void PlanetRNP::update_rnp()
{
    ...
    if(rnp_type == FullRNP)
    {
        ...
        else
        {
            ...
            // internal NP matrix
            Matrix3x3::product_transpose_transpose(nutation->rotation, precession->rotation, NP_matrix);
        }
    }
 ...
 */
void PlanetRNP::update_axial_rotation()
{
    // Update the rotation, first checking if it has been set to something
    if(rotation == nullptr)
    {
...
void PlanetRNP::propagate_rnp()
{
    ...
    if(enable_polar == true)
    {
        ...
        {
            Matrix3x3::product_transpose_transpose(polar_motion->rotation, rotation->rotation, scratch_matrix);
        }
    }
    else
    {
