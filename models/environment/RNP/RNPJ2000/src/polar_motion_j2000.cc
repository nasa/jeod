 ...
 * PlanetRotation
 */
void PolarMotionJ2000::update_rotation()
{
    ...
    if(!override_table)
    {
        ...
        if(time >= polar_mjd[last_table_index])
        {
            ...
            if(warn_table != true)
            {
                MessageHandler::warn(__FILE__,
                                     __LINE__,
                                     RNPMessages::polar_motion_table_warning,
        ...
        { // need to interpolate xp and yp out of the tables
            warn_table = false;
 ...
 */
void PolarMotionJ2000::initialize(PlanetRotationInit * init)
{
    auto * pm_init = dynamic_cast<PolarMotionJ2000Init *>(init);
