...
const double METAtmosphere::gauss_altitudes[9] = {90.0, 105.0, 125.0, 160.0, 200.0, 300.0, 500.0, 1500.0, 2500.0};
//  TODO 1970/71 inconsistency
...
//****************************************************************************
METAtmosphereThermal::METAtmosphereThermal(const double & exo_temp, const double & altitude)
...
//****************************************************************************
METAtmosphere::METAtmosphere(const double & trunc_julian_time_in)
    : trunc_julian_time(trunc_julian_time_in),
      state(),
...
*****************************************************************************/
void METAtmosphereThermal::update()
{
    // T_125 represents T_x in the Jacchia papers.
    // See eq(9) in either Jacchia paper.
    //  TODO 1970/71 inconsistency
    //       1970:
    T_125 = 444.3807 + (0.02385 * T_exosphere) - (392.8292 * exp(-0.0021357 * T_exosphere));
    //       1971:
...
*****************************************************************************/
double METAtmosphereThermal::compute_temperature(double altitude_km_in)
{
    ...
    if(dz <= 0.0)
    {
        ...
        return T_125 + dT * ((k_1 * dz) + (k_3 * dz_3) + (k_4 * dz_4));
    }
    ...
    return T_125 + coeff_A * std::atan2(k_1 * dT * dz * (1.0 + (4.5E-6 * dz_2_5)), coeff_A);
}
...
****************************************************************************/
void METAtmosphere::update_atmosphere(const PlanetFixedPosition * pfix_pos, AtmosphereState * ext_state)
{
 ...
 */
//****************************************************************************
void METAtmosphere::update_atmosphere(const PlanetFixedPosition * pfix_pos, METAtmosphereStateVars * ext_state)
{
    if(ext_state == nullptr)
    {
        MessageHandler::error(__FILE__,
 ...
 */
//****************************************************************************
void METAtmosphere::update_atmosphere(const PlanetFixedPosition * pfix_pos)
{
    if(pfix_pos == nullptr)
    {
        MessageHandler::error(__FILE__,
    ...
    state.pressure = (state.density * 1000.0 / state.mol_weight) * R_gas_constant * state.temperature;
}
...
*****************************************************************************/
void METAtmosphere::modify_densities()
{
    ...
    else if(altitude_km > base_fairing_height)
    {                      // i.e. between 440km and 500km
...
******************************************************************************/
void METAtmosphere::compute_solar_angles()
{
    //****************************************************************************
    ...
    solar_declination_angle = std::asin(std::sin(celestial_longitude) * std::sin(dec_angle_const));
    ...
    double right_ascension_point = greenwich_mean_position * deg_to_rad + longitude;
...
******************************************************************************/
void METAtmosphere::compute_exospheric_temperature()
{
    ...
    if(std::abs(latitude) > M_PI)
    {
        MessageHandler::warn(__FILE__,
                             __LINE__,
                             AtmosphereMessages::numerical_warning,
    ...
    //  TODO 1970/71 inconsistency
    //       1970:
    const double C1 = 383.0;
    const double C2 = 3.32;
    const double C3 = 1.80;
    //       1971:
    // const double C1 = 379.0;
    // const double C2 = 3.24;
    // const double C3 = 1.30;
    ...
    //        gamma =   43 degrees
    //        p     =    6 degrees
    const double beta = -0.6457718;
    const double gamma = 0.7504916;
    const double p = 0.1047198;
    ...
    double tau = solar_hour_angle + beta + p * std::sin(solar_hour_angle + gamma);
    ...
    //****************************************************************************
    const double D1 = 28.0;
    const double D2 = 0.03;
    const double D3 = 1.0;
    const double D4 = 100.0;
    ...
    if(geo_index_type == ATMOS_MET_GI_KP)
    { // geo_index represents K_p
        geomagnetic_variation = D1 * geo_index + D2 * std::exp(geo_index);
    }
    else
    { // geo_index represents a_p
        geomagnetic_variation = D3 * geo_index + D4 * (1.0 - std::exp(D5 * geo_index));
    }
    ...
    double tau0 = std::pow((1.0 + sin(M_PI * 2 * fraction_of_year + E7)) / 2, E8);
    ...
    state.exo_temp = solar_activity_variation * diurnal_variation + geomagnetic_variation + semiannual_variation;
}
...
void METAtmosphere::jacchia()
{
    ...
    else
    {
        ...
        temperature_ceiling_A = thermal.compute_temperature(barometric_equation_ceiling);
    }
    ...
    double integral_Mg_RT = apply_gauss_quadrature(0, integration_ceiling_A) / R_gas_constant;
    ...
    state.density = 2.1926E-5 * (state.mol_weight / temperature_ceiling_A) * std::exp(-integral_Mg_RT);
    ...
    species.num_density[1] = actual_particle_density *
                             (((state.mol_weight / species.nominal_mol_weight) * (1.0 + species.frac[1])) - 1.0);
    species.num_density[2] = 2.0 * actual_particle_density * (1.0 - state.mol_weight / species.nominal_mol_weight);
    ...
    if(altitude_km > barometric_equation_ceiling)
    {
        ...
        double integral_g_RT = apply_gauss_quadrature(1, altitude_km) / R_gas_constant;
        ...
        {
            species.num_density[ii] *= temp_ratio * std::exp(-species.mol_weight[ii] * integral_g_RT);
        }
    ...
    if(altitude_km > 500.0)
    {
        ...
        double integral_g_RT = apply_gauss_quadrature(6, altitude_km) / R_gas_constant;
    ...
    auto less_than_one = [](const double & num_density)
    {
        return num_density < 1.0;
    };

    std::replace_if(species.num_density, species.num_density + 6, less_than_one, 1.0);
...
*****************************************************************************/
void METAtmosphere::compute_seasonal_latitude_variation()
{
...
*****************************************************************************/
void METAtmosphere::compute_seasonal_lat_variation_He()
{
    ...
    double delta_He_num_density = species.num_density[4] * (std::pow(10.0, delta_log_He_density) - 1.0);
    // apply changes
...
*****************************************************************************/
void METAtmosphere::atmos_MET_FAIR5()
{
...
*****************************************************************************/
double METAtmosphere::compute_mol_wt(double altitude_km_in)
{
    ...
    double offset = std::max(altitude_km_in - reference_altitude, 90.0 - reference_altitude);
...
*****************************************************************************/
double METAtmosphere::apply_gauss_quadrature(int altitude_index_start, double altitude_end)
{
    ...
    if(altitude_index_start == 0 && altitude_end > barometric_equation_ceiling * 1.0001)
    {
        MessageHandler::error(__FILE__,
                              __LINE__,
                              "Invalid call.\n",
    ...
    // whole range.
    for(unsigned int ii = altitude_index_start; ii < 8 && gauss_altitudes[ii] < altitude_end; ++ii)
    {
        ...
        for(unsigned int kk = 0; kk < gauss_order; ++kk)
        {
            ...
            double alt_eval_point = alt_lo + half_cell_height * (1.0 + GaussQuadrature::gauss_xvalues[gauss_order][kk]);
            ...
            double value_eval_point = grav / thermal.compute_temperature(alt_eval_point);
            if(include_mu)
            {
                value_eval_point *= compute_mol_wt(alt_eval_point);
            }
            // weight this value and add it to the integral over the cell.
            integral_cell += GaussQuadrature::gauss_weights[gauss_order][kk] * value_eval_point;
        }
