...
% for ii in range(meta.num_items):
  % if ii == (meta.num_items-1):
    {
       .offset = ${meta.coef_start_index[ii]},
       .nterms = ${meta.num_coef_per_component[ii]},
       .npoly  = ${meta.num_coef_sets[ii]}
    }
  % else:
    {
       .offset = ${meta.coef_start_index[ii]},
...
% for ii in range(meta.number_segments):
    {
        .num_recs = ${data[ii].num_records},
        .start_epoch = ${data[ii].record_start_epoch[0]},
        .stop_epoch = ${data[ii].record_stop_epoch[-1]}
...
% for ii in range(data[segIdx].num_records):
    {
  % for jj in range(0, meta.ncoeff, 3):
     % if (meta.ncoeff - jj) == 1:
