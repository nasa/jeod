...
{
    JEOD_MAKE_SIM_INTERFACES(jeod, De4xxEphemItem)
...
public:
    ...
    // Member functions
    De4xxEphemItem() = default;
    ~De4xxEphemItem() = default;
    De4xxEphemItem(const De4xxEphemItem &) = delete;
    De4xxEphemItem & operator=(const De4xxEphemItem &) = delete;
...
protected:
     ...
     */
    Status status{Deselected}; //!< trick_units(--)

    /**
     * The node index number, per the De4xxEphemBodies numbering scheme.
     */
    unsigned int index{UINT_MAX}; //!< trick_units(--)
};
 ...
 */
class De4xxEphemeris : public EphemerisInterface,
                       public RefFrameOwner
{
    JEOD_MAKE_SIM_INTERFACES(jeod, De4xxEphemeris)
...
public:
    ...
    // Destructor
    ~De4xxEphemeris() override;

    De4xxEphemeris(const De4xxEphemeris &) = delete;
    De4xxEphemeris & operator=(const De4xxEphemeris &) = delete;

    // S_define-level interfaces

    // Initialize the model
    void initialize_model(const TimeManager & time_manager,
    ...
    {
        file.file_spec.set_model_directory(dirIn);
    }
    ...
    {
        return file.file_spec.get_model_directory();
    }
