...
De4xxEphemeris::De4xxEphemeris()
{
    ...
    body_to_file_idx = JEOD_ALLOC_PRIM_ARRAY(De4xxBase::number_jeod_items(999), int);
    body_to_file_idx[De4xxBase::De4xx_Ephem_Sun] = De4xxBase::De4xx_File_Sun;
    body_to_file_idx[De4xxBase::De4xx_Ephem_Mercury] = De4xxBase::De4xx_File_Mercury;
    body_to_file_idx[De4xxBase::De4xx_Ephem_Venus] = De4xxBase::De4xx_File_Venus;
    body_to_file_idx[De4xxBase::De4xx_Ephem_Mars] = De4xxBase::De4xx_File_Mars;
    body_to_file_idx[De4xxBase::De4xx_Ephem_Jupiter] = De4xxBase::De4xx_File_Jupiter;
    body_to_file_idx[De4xxBase::De4xx_Ephem_Saturn] = De4xxBase::De4xx_File_Saturn;
    body_to_file_idx[De4xxBase::De4xx_Ephem_Uranus] = De4xxBase::De4xx_File_Uranus;
    body_to_file_idx[De4xxBase::De4xx_Ephem_Neptune] = De4xxBase::De4xx_File_Neptune;
    body_to_file_idx[De4xxBase::De4xx_Ephem_Pluto] = De4xxBase::De4xx_File_Pluto;
    body_to_file_idx[De4xxBase::De4xx_Ephem_Moon] = De4xxBase::De4xx_File_Moon;
    body_to_file_idx[De4xxBase::De4xx_Ephem_EMbary] = De4xxBase::De4xx_File_EMbary;

    body_to_file_idx[De4xxBase::De4xx_Ephem_ENutation] = De4xxBase::De4xx_File_ENutation;
    body_to_file_idx[De4xxBase::De4xx_Ephem_LLibration] = De4xxBase::De4xx_File_LLibration;
    ...
    item_data[De4xxBase::De4xx_Ephem_LLibration].name = lunar_orientation.get_name();
    item_data[De4xxBase::De4xx_Ephem_LLibration].index = De4xxBase::De4xx_Ephem_LLibration;
 ...
 */
void De4xxEphemeris::deactivate()
{
    active = false;
}
 ...
 * @return Timestamp\n Units: day
 */
double De4xxEphemeris::timestamp() const
{
    return update_time;
}
 ...
 * @return Name
 */
std::string De4xxEphemeris::get_name() const
{
    return ident;
}
 ...
 */
void De4xxEphemeris::initialize_time(const TimeManager & time_manager, const std::string & time_type)
{
    // Get pointers to the ephemeris time and dynamic time time objects.
    if(time_type == "TDB")
    {
        time_tt = dynamic_cast<const TimeStandard *>(time_manager.get_time_ptr("TDB"));
    }
    else
    {
        if(time_type != "TT")
        ...
        time_tt = dynamic_cast<const TimeStandard *>(time_manager.get_time_ptr("TT"));
    }
 ...
 */
void De4xxEphemeris::initialize_file()
{
    ...
    // Construct the identifier for this model.
    {
        ...
        ident = NamedItem::construct_name("DE", denum.str(), "LE", lenum.str());
    }
}
 ...
 * \param[in,out] ephem_manager Ephemerides manager
 */
void De4xxEphemeris::initialize_items(EphemeridesManager & ephem_manager)
{
    ...
    if(item_data[De4xxBase::De4xx_Ephem_Earth].status != item_data[De4xxBase::De4xx_Ephem_Moon].status)
    {
        MessageHandler::fail(__FILE__,
                             __LINE__,
                             EphemeridesMessages::inconsistent_setup,
    ...
    // EphemeridesManager.
    if((item_data[De4xxBase::De4xx_Ephem_SSbary].status != De4xxEphemItem::Deselected) &&
       (ephem_manager.find_ref_frame(solar_system_barycenter_frame.get_name()) == nullptr))
    {
        ephem_manager.add_integ_frame(solar_system_barycenter_frame);
    }

    if((item_data[De4xxBase::De4xx_Ephem_EMbary].status != De4xxEphemItem::Deselected) &&
       (ephem_manager.find_ref_frame(earth_moon_barycenter_frame.get_name()) == nullptr))
    {
    ...
    if(item_data[De4xxBase::De4xx_Ephem_LLibration].status != De4xxEphemItem::Deselected)
    {
 ...
 * \param[in,out] ephem_manager Ephemerides manager
 */
void De4xxEphemeris::ephem_initialize(EphemeridesManager & ephem_manager JEOD_UNUSED)
{
    // Nothing to do if the model is inactive.
    if(!active)
    {
 ...
 * @return Void
 */
unsigned int De4xxEphemeris::activate_nodes()
{
    ...
    for(uint32_t ii = 0; ii < De4xxBase::number_jeod_items(file.file_spec.get_model_number()); ++ii)
    {
        ...
        if(item.status != De4xxEphemItem::Deselected)
        {
            ...
            if((item.enabled_item == nullptr) || (!item.enabled_item->is_active()))
            {
 ...
 */
unsigned int De4xxEphemeris::activate_em_nodes(unsigned int tot_active)
{
    static const De4xxBase::De4xxEphemBodies embodies[3] = {De4xxBase::De4xx_Ephem_Earth,
    ...
    embcount = 0;
    for(auto embodie : embodies)
    {
        De4xxEphemItem & item = item_data[embodie];
    ...
    if((embcount == 2) ||
       ((embcount == 1) && (item_data[De4xxBase::De4xx_Ephem_EMbary].status <= De4xxEphemItem::Inactive) &&
        (tot_active > 1)))
    {
        for(auto embodie : embodies)
        {
...
void De4xxEphemeris::determine_root_node()
{
    ...
    else if((tot_active == 3) && (item_data[De4xxBase::De4xx_Ephem_EMbary].status > De4xxEphemItem::Inactive) &&
            (item_data[De4xxBase::De4xx_Ephem_Earth].status > De4xxEphemItem::Inactive) &&
            (item_data[De4xxBase::De4xx_Ephem_Moon].status > De4xxEphemItem::Inactive))
    {
 ...
 * \param[in,out] ephem_manager Ephemerides manager
 */
void De4xxEphemeris::ephem_activate(EphemeridesManager & ephem_manager JEOD_UNUSED)
{
    unsigned int tot_active;
    ...
    for(int ii = De4xxBase::De4xx_Ephem_Sun; ii <= De4xxBase::De4xx_Ephem_EMbary; ++ii)
    {
        auto file_id = (De4xxBase::De4xxFileEntries)body_to_file_idx[ii];
        if(file_id < file.io.metaData->number_file_items)
        {
            file.item[file_id].active = (item_data[ii].status == De4xxEphemItem::Active);
        }
    }

    // Librations = lunar orientation.
    file.item[De4xxBase::De4xx_File_LLibration].active = (item_data[De4xxBase::De4xx_Ephem_LLibration].status ==
 ...
 * \param[in,out] ephem_manager Ephemerides manager
 */
void De4xxEphemeris::ephem_build_tree(EphemeridesManager & ephem_manager)
{
    // Nothing to do if the model is inactive.
    if(!active)
    {
    ...
    for(uint32_t ii = 0; ii < De4xxBase::number_trans_points(file.file_spec.get_model_number()); ++ii)
    {
        if((item_data[ii].status != De4xxEphemItem::Deselected) && (item_data[ii].item == item_data[ii].enabled_item))
        {
    ...
    if((root_item != nullptr) && (root_item->item == root_item->enabled_item))
    {
    ...
    for(uint32_t ii = 0; ii < De4xxBase::number_trans_points(file.file_spec.get_model_number()); ++ii)
    {
        ...
        if(item.status == De4xxEphemItem::Active)
        {
            ...
            if((ii == De4xxBase::De4xx_Ephem_Earth) || (ii == De4xxBase::De4xx_Ephem_Moon))
            {
 ...
 */
void De4xxEphemeris::ephem_update()
{
    ...
    if((!active) || ((!force_update) && (Numerical::compare_exact(update_time, time_dyn->seconds))))
    {
        return;
    }
    ...
    if((root_item != nullptr) && (root_item->item == root_item->enabled_item))
    {
        auto * point = dynamic_cast<EphemerisPoint *>(root_item->item);
    ...
    if(nactive_items > 0)
    {
        ...
        for(unsigned int ii = De4xxBase::De4xx_Ephem_Sun; ii <= De4xxBase::De4xx_Ephem_EMbary; ++ii)
        {
            ...
            {
                auto * point = dynamic_cast<EphemerisPoint *>(item_data[ii].item);
        ...
        if((item_data[De4xxBase::De4xx_Ephem_Earth].status == De4xxEphemItem::Active) &&
           (item_data[De4xxBase::De4xx_Ephem_Moon].status == De4xxEphemItem::Active))
        {
            De4xxFileItem & moon_ephem = file.item[body_to_file_idx[De4xxBase::De4xx_Ephem_Moon]];

            points[De4xxBase::De4xx_Ephem_Earth].update_scaled(moon_ephem.state[0],
                                                               moon_ephem.state[1],
                                                               -file.header.be_em_dist_ratio,
                                                               update_time);

            points[De4xxBase::De4xx_Ephem_Moon].update_scaled(moon_ephem.state[0],
                                                              moon_ephem.state[1],
                                                              file.header.bm_em_dist_ratio,
                                                              update_time);
        }
        ...
        if(item_data[De4xxBase::De4xx_Ephem_LLibration].status == De4xxEphemItem::Active)
        {
            De4xxFileItem & file_angles = file.item[De4xxBase::De4xx_File_LLibration];
            lunar_orientation.update(file_angles.state[0], file_angles.state[1], update_time);
        }
    }
}
 ...
 */
bool De4xxEphemeris::time_is_in_range() const
{
 ...
 */
void De4xxEphemeris::propagate_lunar_rnp()
{
    if((!active) || (item_data[De4xxBase::De4xx_Ephem_LLibration].status != De4xxEphemItem::Active))
    {
        return;
    }
