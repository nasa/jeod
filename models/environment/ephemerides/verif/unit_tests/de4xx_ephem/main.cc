...
// Internal functions
static bool vtransit_test (
   TimeManager & time_manager, EphemeridesManager & ephem_manager);

static bool dataset_test (
   const De4xxEphemeris & de4xx_ephem,
   TimeManager & time_manager,
   EphemeridesManager & ephem_manager);

static void pleph (
   double pv[2][3], EphemeridesManager & ephem_manager, int itgt, int ictr);
   ...
   char ** argv)
{
   ...
   passed_all = vtransit_test (time_manager, ephem_manager) &&
                passed_all;
   ...
   passed_all = dataset_test (de4xx_ephem, time_manager, ephem_manager) &&
                passed_all;
   ...
   std::printf (
      ...
      "========================================"
      "========================================\n\n\n",
      de_filename, color.pass_fail(passed_all).c_str());

   return passed_all ? 0 : 1;
}
   ...
   EphemeridesManager & ephem_manager)
{
   ...
   RefFrame & sun_inertial =
      *(ephem_manager.find_ref_frame ("Sun.inertial"));
   RefFrame & venus_inertial =
      *(ephem_manager.find_ref_frame ("Venus.inertial"));
   RefFrame & earth_inertial =
      *(ephem_manager.find_ref_frame ("Earth.inertial"));
   RefFrame & embary_inertial =
      *(ephem_manager.find_ref_frame ("EMBary.inertial"));
   ...
   rest2010 = 1.519903134179310e11 * 0.19693370052752313 +
              1.519903326785795e11 * 0.8030662994724768;
   vest2010 = 29.33992080990719e3 * 0.19693370052752313 +
              29.33991846404838e3 * 0.8030662994724768;
   ...
   // Interpolating between these times yields
   rest2021 = 1.519903764836356E+11 * 0.19693370052752313
            + 1.519903957431765e+11 * 0.8030662994724768 ;
   vest2021 = 29.33992081471865E+03 * 0.19693370052752313
            + 29.33991846885831E+03 * 0.8030662994724768 ;
   ...
   std::printf ("\nEarth-Moon barycenter position and velocity check:\n"
                ...
                "    relative error                      = %s\n\n",
                rest, robs, color.pass_fail(rpass, "%.3g", rerr).c_str(),
                vest, vobs, color.pass_fail(vpass, "%.3g", verr).c_str());
   ...
   thetaexp  = 626.9;
   thetaobs  = acos (Vector3::dot (venus_earth_hat, sun_earth_hat)) *
               180.0 / M_PI * 3600.0;
  ...
  std::printf ("\n\n"
   ...
   for (itgt = 0; itgt < 13; itgt++) {
      for (ictr = 0; ictr < 13; ictr++) {
         nrel_checked[itgt][ictr][0] =
         nrel_checked[itgt][ictr][1] = 0;
      }
   }
   for (itgt = 0; itgt < 2; itgt++) {
      nang_checked[itgt][0] =
      nang_checked[itgt][1] = 0;
   }
   nchecked = nbad = 0;
   ...
   while (fgets (buf, sizeof(buf), testdata) != NULL) {
      ...
      // Scan the line.
      if (sscanf (buf+16, "%lf %d %d %d %lf",
                  &jdate, &itgt, &ictr, &icoord, &val) != 5) {
         fprintf (stderr, "Bad line '%s'\n", buf);
         continue;
      }

      // Check for valid indices.
      if ((itgt < 1) || (itgt > 15) || (ictr < 0) || (ictr > 15) ||
          (icoord < 1) || (icoord > 6)) {
         fprintf (stderr, "Bad line '%s'\n", buf);
         continue;
      }
   ...
   for (itgt = 0; itgt < 2; itgt++) {
      std::printf ("%-9s | %5d %5d\n",
                   ang_name[itgt],
                   nang_checked[itgt][0], nang_checked[itgt][1]);
   }
   ...
   std::printf ("\n\nEphemeris test dataset test results:\n"
                ...
                "  # Failed: %d\n"
                "  Status  : %s\n\n",
                nchecked-nbad, nbad, color.pass_fail(passed_all).c_str());
   ...
   int ictr)
{
   ...
   // Target 15 is lunar orientation ("libration")
      ...
      EphemerisZXZOrientation * lunar_angles =
         dynamic_cast<EphemerisZXZOrientation*> (
            ephem_manager.find_ephem_angle ("Moon.pfix"));
