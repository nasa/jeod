 ...
 */
void GravityControls::initialize_control(GravityManager & grav_man)
{
 ...
 */
void GravityControls::reset_control(BaseDynManager & dyn_manager)
{
    if(body == nullptr || body->inertial == nullptr)
    {
        MessageHandler::inform(__FILE__,
                               __LINE__,
                               GravityMessages::null_pointer,
 ...
 */
void GravityControls::gravitation(const double integ_pos[3],
                                  ...
                                  double Pot[1])
{
    ...
    GravityIntegFrame & grav_source_frame = body->frames[integ_frame_idx]; // Grav frame for this integ frame
    const RefFrame & integ_frame = *(grav_source_frame.ref_frame);         // Integration frame

    // Compute position of integ. frame origin wrt the planet center.
    integ_frame.compute_position_from(*(body->inertial), grav_source_frame.pos);
    ...
    if(!spherical)
    {
        calc_nonspherical(integ_pos, posn, grav_source_frame, body_grav_accel, dgdx, Pot[0]);
    }
    ...
    {
        calc_spherical(integ_pos, posn, grav_source_frame, body_grav_accel, dgdx, Pot[0]);
    }
}
 ...
 */
void GravityControls::gravitation(const RefFrame & point_of_interest,
                                  unsigned int integ_frame_idx,
                                  double body_grav_accel[3],
                                  double dgdx[3][3],
                                  double & pot)
{
    ...
    // Compute position of the vehicle CoM wrt the planet center.
    Vector3::diff(point_of_interest.state.trans.position, grav_source_state.trans.position, rel_pos);

    // Compute contributions from non-spherical gravity if requested.
    if(!spherical)
    {
        calc_nonspherical(point_of_interest.state.trans.position,
                          rel_pos,
                          grav_source_frame,
                          body_grav_accel,
                          dgdx,
    ...
    if(relativistic)
    {
        ...
        Vector3::diff(point_of_interest.state.trans.velocity, grav_source_state.trans.velocity, rel_vel);
        calc_relativistic(point_of_interest, rel_pos, rel_vel, relativistic_accel);
    ...
    if(!perturbing_only && !skip_spherical)
    {
        ...
        calc_spherical(point_of_interest.state.trans.position, rel_pos, grav_source_frame, body_grav_accel, dgdx, pot);
    }
}

void GravityControls::calc_spherical(const double integ_pos[3],
                                     const double posn[3],
                                     ...
                                     double & pot)
{
    ...
    if(grav_source_frame.is_third_body)
    {
        ...
        if(battin_method)
        {
            ...
            // Therefor the sign in this calculation (+) is opposite of the one found in the documentation (-).
            ...
            if(q > -0.38)
            {
                ...
                Vector3::scale(-mu_local / (rho_3rd * (1.0 + q)),
                               Vector3::scale_decr(grav_source_frame.pos, q, Vector3::copy(integ_pos, acc_local)));
            }
            ...
            else
            {
                ...
                Vector3::scale_incr(grav_source_frame.pos, mu_local / rho_3rd, acc_local);
            }
        }
 ...
 */
void GravityControls::calc_relativistic(const RefFrame & point_of_interest,
                                        ...
                                        double perturbing_accel[3])
{
    ...
    for(auto other_body : all_bodies)
    {
    ...
    accel1_scale += 2.0 * Vector3::vmagsq(rel_vel) // 1+gamma = 2
