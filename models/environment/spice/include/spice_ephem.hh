 ...
 */
class SpiceEphemeris : public EphemerisInterface,
                       public RefFrameOwner,
                       public SimpleCheckpointable
{
    JEOD_MAKE_SIM_INTERFACES(jeod, SpiceEphemeris)
...
public:
    ...
    void initialize_model(const TimeManager & time_manager, EphemeridesManager & ephem_manager);
...
private:
     ...
     */
    std::string ident{"SPICE"}; //!< trick_units(--)
    ...
    SpiceEphemPoint * create_new_ephem_point(std::string object_name, const std::string & spice_name);
