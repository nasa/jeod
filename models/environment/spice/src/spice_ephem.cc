 ...
 */
void SpiceEphemeris::deactivate()
{
    inactive = true;
}
 ...
 * @return Timestamp\n Units: day
 */
double SpiceEphemeris::timestamp() const
{
    return update_time;
}
 ...
 * @return Name
 */
std::string SpiceEphemeris::get_name() const
{
    return ident;
}
 ...
 */
void SpiceEphemeris::initialize_model(const TimeManager & time_manager, EphemeridesManager & ephem_manager)
{
 ...
 */
void SpiceEphemeris::simple_restore()
{
    return; // Nothing yet.
}
 ...
 * \param[in] time_manager Time manager
 */
void SpiceEphemeris::initialize_time(const TimeManager & time_manager)
{
 ...
 */
void SpiceEphemeris::load_spice_files()
{
 ...
 */
void SpiceEphemeris::process_spk()
{
    ...
    for(unsigned int ii = 0; ii < planet_names.size(); ++ii)
    {
        ...
        for(std::list<int>::const_iterator it = spk_object_ids.begin(); it != spk_object_ids.end(); ++it)
        {
            if(obj_id == *it)
            {
        ...
        loaded_spk.push_back(create_new_ephem_point(planet_names[ii], search_name));
    }
}
 ...
 */
void SpiceEphemeris::process_orientations()
{
    ...
    {
        planetary_orientations.push_back(create_new_ephem_orientation(orientation_names[ii]));
    }
}
 ...
 */
void SpiceEphemeris::populate_item(EphemerisItem & item, const std::string & object_name)
{
    item.set_name(object_name, item.default_suffix());
    item.set_owner(this);
    item.enable();
}
 ...
 */
SpiceEphemPoint * SpiceEphemeris::create_new_ephem_point(std::string object_name, const std::string & spice_name)
{
 ...
 */
SpiceEphemOrientation * SpiceEphemeris::create_new_ephem_orientation(std::string jeod_name)
{
    // Dynamically allocate and populate new object.
    SpiceEphemOrientation * new_orientation = JEOD_ALLOC_CLASS_OBJECT(SpiceEphemOrientation, ());
 ...
 */
void SpiceEphemeris::initialize_items()
{
    ...
    if(root_item == nullptr)
    {
        MessageHandler::fail(__FILE__,
                             __LINE__,
                             EphemeridesMessages::null_pointer,
 ...
 */
void SpiceEphemeris::introduce_item(EphemerisItem & item)
{
 ...
 */
std::string SpiceEphemeris::spice_2_jeod(std::string spice_name)
{
 ...
 * @return name of pfix frame that SPICE associates with the given body. Handles high precision frames for Earth and
 * Moon unless IAU is specified by user \param[in] jeod_name Name of the JEOD body.
 */
std::string SpiceEphemeris::jeod_2_spice_pfix(std::string jeod_name)
{
 ...
 */
void SpiceEphemeris::name_barycenter_frames()
{
    ...
    for(unsigned int ii = 4; ii < 10; ++ii)
    {
        ...
        barycenter_frames[ii].set_name(jeod_name);
    }
}
 ...
 */
void SpiceEphemeris::add_barycenter(int id)
{
    ...
    loaded_spk.push_back(create_new_ephem_point(jeod_name, spice_name));

    // Register associated frame with the ephemeris manager.
    if(ephem_mngr_local->find_ref_frame(frame_name) == nullptr)
    {
 ...
 */
void SpiceEphemeris::create_barycenters()
{
 ...
 */
void SpiceEphemeris::determine_root_node()
{
 ...
 */
void SpiceEphemeris::ephem_initialize(EphemeridesManager &)
{
 ...
 * \param[in,out] ephem_manager Ephemerides manager
 */
void SpiceEphemeris::ephem_activate(EphemeridesManager &)
{
    // This EphemerisInterface does not track EphemerisItem existence and
    // activation/selection as separate concepts, since it only creates
    // EphemerisItems when specified to do so by the user. Thus, this method
 ...
 */
void SpiceEphemeris::add_descendants_r(SpiceEphemPoint * parent)
{
    for(unsigned int ii = 0; ii < loaded_spk.size(); ++ii)
    {
        ...
        if(loaded_spk[ii]->get_spice_id() != 0)
        {
            ...
            if(loaded_spk[ii]->get_parent_id() == parent->get_spice_id())
            {
                ...
                MessageHandler::debug(__FILE__,
                                      ...
                                      EphemeridesMessages::debug,
                                      "Adding %s to %s.\n",
                                      loaded_spk[ii]->get_target_frame()->get_name().c_str(),
                                      parent->get_target_frame()->get_name().c_str());

                // Do the add for the current frame.
                ephem_mngr_local->add_frame_to_tree(*(loaded_spk[ii]->get_target_frame()), parent->get_target_frame());
 ...
 * \param[in,out] ephem_manager Ephemerides manager
 */
void SpiceEphemeris::ephem_build_tree(EphemeridesManager & ephem_manager)
{
    // Nothing to do if the model is inactive.
    if(inactive)
    {
    ...
    MessageHandler::debug(__FILE__,
                          ...
                          EphemeridesMessages::debug,
                          "Adding root %s.\n",
                          root_item->get_target_frame()->get_name().c_str());
 ...
 */
void SpiceEphemeris::ephem_update()
{
    // Nothing to do if
    // - the model isn't active or
    // - neither time nor the tree has changed.
    if((inactive) || ((!force_update) && (Numerical::compare_exact(update_time, *dyn_seconds))))
    {
        return;
    }
 ...
 */
SpiceEphemPoint * SpiceEphemeris::find_spice_id(int id_to_find)
{
 ...
 */
int SpiceEphemeris::find_parent_id(int obj_id)
{
 ...
 */
void SpiceEphemeris::update_trans()
{
 ...
 */
void SpiceEphemeris::update_rot()
{
    for(unsigned int ii = 0; ii < planetary_orientations.size(); ++ii)
    {
 ...
 */
void SpiceEphemeris::mute_spice_errors()
{
    ...
    strncpy(new_value, "return", MAX_NAME_LENGTH);
    ...
    strncpy(new_value, "null", MAX_NAME_LENGTH);
    ...
    strncpy(new_value, "explain", MAX_NAME_LENGTH);
