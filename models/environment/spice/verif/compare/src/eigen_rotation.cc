...
*/
void Orientation::compute_matrix_from_eigen_rotation(double eigen_angle,         // In:  r  Rotation angle
                                                     const double eigen_axis[3], // In:  -- Rotation axis, unit vector
                                                     double trans[3][3]) // Out: -- Resultant transformation matrix
{
    ...
    trans[0][1] = trans[1][0] = omcp * eigen_axis[0] * eigen_axis[1];
    trans[0][2] = trans[2][0] = omcp * eigen_axis[0] * eigen_axis[2];
    trans[1][2] = trans[2][1] = omcp * eigen_axis[1] * eigen_axis[2];
...
*/
void Orientation::compute_eigen_rotation_from_matrix(const double trans[3][3], // In:  -- Transformation matrix
                                                     double * eigen_angle,     // Out: r  Resultant rotation angle
                                                     double eigen_axis[3])     // Out: -- Resultant rotation axis
{
    ...
    dmag = std::sqrt(diff[0] * diff[0] + diff[1] * diff[1] + diff[2] * diff[2]);
    ...
    else if(sinphi >= -cosphi)
    {
        ...
        double dmag_inv = 1.0 / dmag;
        eigen_axis[0] = dmag_inv * diff[0];
        eigen_axis[1] = dmag_inv * diff[1];
        eigen_axis[2] = dmag_inv * diff[2];
    }
