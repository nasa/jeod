...
int main(int arg_c, char ** arg_v)
{
    ...
    double vmag = sqrt(eigen_axis[0] * eigen_axis[0] + eigen_axis[1] * eigen_axis[1] + eigen_axis[2] * eigen_axis[2]);
    ...
    for(double & eigen_axi : eigen_axis)
    {
        eigen_axi /= vmag;
    }
    ...
    Orientation::compute_matrix_from_eigen_rotation(eigen_angle, eigen_axis, mat);
    ...
    Orientation::compute_eigen_rotation_from_matrix(mat, &new_eigen_angle, new_eigen_axis);
    ...
    for(double & new_eigen_axi : new_eigen_axis)
    {
        new_eigen_axi *= vmag;
    }
