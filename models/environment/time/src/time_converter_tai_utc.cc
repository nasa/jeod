 ...
 */
void
TimeConverter_TAI_UTC::initialize (
   ...
   const int int_dir)
{
   ...
   if (int_dir == 1) {
      ...
      if (utc_ptr->is_initialized()) {
         ...
         if ((index > 0) &&
             ((when_vec[index] - utc_ptr->trunc_julian_time) >
              ((val_vec[index] - val_vec[index - 1]) / 86400.0)
             )) {
            initialize_leap_second();
         }
      }
 ...
 */
void
TimeConverter_TAI_UTC::initialize_leap_second (
   void)
{
   ...
   if (trunc_julian_time < when_vec[0]) {
      ...
      if (tai_ptr->time_manager->dyn_time.scale_factor >= 0) {
         ...
         next_when = trunc_julian_time +
                     (trunc_julian_time - when_vec[last_index]);
      ...
      for (index = 0;
           trunc_julian_time >= when_vec[index + 1];
           ++index) {
         ;
      }
 ...
 */
void
TimeConverter_TAI_UTC::convert_a_to_b (
   void)
{
   ...
   if (!off_table_end && utc_ptr->true_utc) {
      ...
      if (tai_ptr->time_manager->dyn_time.scale_factor >= 0) {
        ...
        /*  assign these here.  It is redundant if the loop runs more than once,
            ...
            is more efficient. */
            a_to_b_offset = -val_vec[index] / 86400.0;
            utc_ptr->set_time_by_trunc_julian (tai_ptr->trunc_julian_time  +
                                               a_to_b_offset);
            // utc_ptr->trunc_julian_time = tai_ptr->trunc_julian_time + a_to_b_offset;
 ...
 */
void
TimeConverter_TAI_UTC::convert_b_to_a (
   void)
{
   if (!off_table_end && utc_ptr->true_utc) {
   ...
   tai_ptr->set_time_by_trunc_julian (utc_ptr->trunc_julian_time  -
                                      a_to_b_offset);
 ...
 */
void
TimeConverter_TAI_UTC::verify_table_lookup_ends (
   void)
{

   if (!utc_ptr->true_utc) { // table is not monitored
      return;
   }
