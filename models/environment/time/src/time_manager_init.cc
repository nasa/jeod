 ...
 */
void
TimeManagerInit::initialize_manager (
 ...
 */
void
TimeManagerInit::verify_times_setup (
   void)
{
   ...
   if (initializer_index == -2) { /* initializer undefined */
      if (time_manager->num_types > 1) { // more than one time-type
         MessageHandler::fail (
   ...
   for (int ii = 0; ii < time_manager->num_types; ++ii) {
      for (int jj = ii + 1; jj < time_manager->num_types;
           ++jj) {
         // Check for multiple instances of any time type.
         if ((typeid(*time_manager->time_vector[ii]) ==
              typeid(*time_manager->time_vector[jj])) &&
             time_manager->time_vector[ii]->must_be_singleton()) {
            MessageHandler::fail (
   ...
   void)
{
   ...
   for (unsigned int ii = 0; ii < time_manager->converter_vector.size();
      ...
      if (index_a < 0) {
         ...
         if ((converter_ptrs_index[conv_indx_ab] >= 0) ||
             (converter_ptrs_index[conv_indx_ba] >= 0)) {
            MessageHandler::fail (
               __FILE__, __LINE__, TimeMessages::redundancy_error, "\n"
 ...
 */
void
TimeManagerInit::verify_converter_setup (
   void)
{
   ...
   if ((tai_index > 0) && (utc_index > 0) && (ut1_index > 0)) {
      ...
      int tai_utc_conv_index = converter_ptrs_index[ utc_index +
                                                     tai_index *
                                                     time_manager->num_types];
      int tai_ut1_conv_index = converter_ptrs_index[ ut1_index +
                                                     tai_index *
                                                     time_manager->num_types];
      if ((tai_utc_conv_index > 0) && tai_ut1_conv_index) {
         // finally check whether the flags are set differently in the two
         // converter classes
         TimeConverter_TAI_UTC * temp_conv1 =
            dynamic_cast<TimeConverter_TAI_UTC*>
            (time_manager->
             converter_vector[
                tai_utc_conv_index ]);
         TimeConverter_TAI_UT1 * temp_conv2 =
            dynamic_cast<TimeConverter_TAI_UT1*>
            (time_manager->
             converter_vector[
                tai_ut1_conv_index ]);

         if (temp_conv1->override_data_table !=
             temp_conv2->override_data_table) {
            MessageHandler::fail (
               __FILE__, __LINE__, TimeMessages::invalid_setup_error, "\n"
 ...
 */
void
TimeManagerInit::create_init_tree (
   void)
{
   ...
   // Dyn time is not the time initialization source
   if (initializer_index != dyn_time_index) {
      ...
      if (init_ude_ptr != nullptr) {
         ...
         } // else found the standard to convert the UDE to a nonarbitrary form
         else {
            status[ upd_index ] = 2; // from 1st gen initializer time, so 2
            num_added_total     = 3;
         }
      }
      // else not initializing from a UDE
      else {
   ...
   // else dyn_time is the initializer
   else {
   ...
   while (num_added_total < time_manager->num_types) {
      if (num_added_pass > 0) {
         ...
         for (int ii = 0; ii < time_manager->num_types; ++ii) {
            if (status[ii] == 0) {
               time_manager->time_vector[ ii ]->
               add_type_initialize (seeking_status, this);
            }
         }
         ...
         for (int ii = 0; ii < time_manager->num_types;
              ++ii) {
            if (status[ii] == 0) {
               MessageHandler::fail (
                  __FILE__, __LINE__, TimeMessages::invalid_setup_error, "\n"
 ...
 */
void
TimeManagerInit::initialize_time_types (
   void)
{
   // First, take care of the head of the tree
   time_manager->time_vector[ initializer_index ]->
   initialize_initializer_time(this);
 ...
 */
void
TimeManagerInit::create_update_tree (
   void)
{
   ...
   while (num_added_total < time_manager->num_types) {
      if (num_added_pass > 0) {
         ...
         for (int ii = 0; ii < time_manager->num_types; ++ii) {
            if (status[ii] == 0) {
               time_manager->time_vector[ii]->add_type_update (
                  seeking_status, this);
            }
         }
      }
 ...
 */
void
TimeManagerInit::organize_update_list()
{
    ...
    {
        if( ii == order[ii] ) // if properly placed, continue
        {
            continue;
        }
        ...
        std::swap( time_manager->time_vector[ii],
                   time_manager->time_vector[order[ii]] );
 ...
 */
int
TimeManagerInit::get_conv_ptr_index (
   const int index_in)
 ...
 */
int
TimeManagerInit::get_conv_dir_init (
   const int index)
 ...
 */
int
TimeManagerInit::get_conv_dir_upd (
   const int index)
 ...
 */
void
TimeManagerInit::increment_status (
   const int index_slave,
   const int index_master)
{
