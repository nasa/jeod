 ...
 */
void TimeStandard::set_time_by_seconds(const double new_seconds)
{
 ...
 */
void TimeStandard::set_time_by_days(const double new_days)
 ...
 */
void TimeStandard::set_time_by_trunc_julian(const double new_tjt)
{
 ...
 */
double TimeStandard::julian_date_at_epoch()
{
 ...
 */
void TimeStandard::add_type_initialize(const int seeking_status, TimeManagerInit * time_manager_init)
{
    ...
    {
        time_manager->get_time_ptr(parent_index)->add_type_initialize(seeking_status, time_manager_init);
    }
 ...
 */
void TimeStandard::calculate_calendar_values()
{
    ...
    if(julian_day != prev_julian_day)
    {
        ...
        auto n_100 = static_cast<int>(r_400 / 36524.3); /* -- number of whole 100-year periods */
        ...
        auto n_1 = static_cast<int>(r_4 / 365.3); /*  -- number of whole years */
        ...
        auto m = static_cast<int>(r_4a / 30.585);
        ...
        calendar_year = 1600 + 400 * n_400 + 100 * n_100 + 4 * n_4 + ((m + 1) / 12);
    }
}
 ...
 */
void TimeStandard::calendar_update(double simtime)
{
 ...
 *      truncated_julian_time are all present in the same class.
 */
void TimeStandard::convert_from_calendar()
{
    ...
    trunc_julian_time = calendar_day + floor(30.585 * m) - (2 * n_1) - 31 + (1461 * n_4) + (36524 * n_100) +
                        (146097 * n_400) - 134493 + (calendar_hour / 24.0) + (calendar_minute / 1440.0) +
...
//  General case
//  General case
void TimeStandard::initialize_initializer_time(TimeManagerInit * time_manager_init)
{
    ...
    switch(time_manager_init->sim_start_format)
    {
        case TimeEnum::undefined:
            ...
            // else
            if(std::fpclassify(calendar_year + calendar_month + calendar_day + calendar_minute + calendar_hour +
                               calendar_second) == FP_ZERO)
            {
                if((std::fpclassify(seconds) == FP_ZERO) && (std::fpclassify(days) == FP_ZERO))
                {
                    MessageHandler::fail(__FILE__,
                ...
                { // secs !=0, cal =0, days =0 || secs / 86400
                    days = seconds / 86400;
                }
                trunc_julian_time = tjt_at_epoch + days;
                break;
        ...
        case TimeEnum::clock:
            MessageHandler::fail(__FILE__,
                                 ...
                                 TimeMessages::invalid_data_error,
                                 "\n"
                                 "Clock is an invalid format for the intializer time.\n");
            break;
        default:
            MessageHandler::fail(__FILE__,
                                 __LINE__,
 ...
 * \param[in] time_manager_init The TM initializer.
 */
void TimeStandard::initialize_from_parent(TimeManagerInit * time_manager_init)
{
    ...
    if((parent_index < 0) || (parent_index > time_manager->num_types))
    {
        MessageHandler::fail(__FILE__,
                             __LINE__,
    ...
    int conv_ptr_index = time_manager_init->get_conv_ptr_index(conv_index);
    TimeConverter * converter_ptr = time_manager->get_converter_ptr(conv_ptr_index);
 ...
 */
double TimeStandard::seconds_of_year()
{
    ...
    if(calendar_year == year_of_last_soy)
    { // year has not changed since last update
        return (seconds - seconds_at_year_start);
    }
    else if(((calendar_year - year_of_last_soy) == 1) && (calendar_month <= 2))
    {
        // year has ticked over, but less than 2 full months into new year
        double seconds_of_year_value = ((calendar_month - 1) * 31 + (calendar_day - 1)) * 86400 + calendar_hour * 3600 +
                                       calendar_minute * 60 + calendar_second;
