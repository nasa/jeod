...
TimeUDE::TimeUDE (
  ...
  epoch_format                (TimeEnum::undefined),
  initial_value_format        (TimeEnum::undefined),
  epoch_defined_in_name       (""),
  epoch_initializing_value    (0.0),
  initializing_data_present   (false),
  epoch_data_present          (false),
 ...
 */
void
TimeUDE::convert_epoch_to_update (
   JeodBaseTime * epoch_ptr,
   ...
   TimeManagerInit * time_manager_init)
{
   ...
   if (conv_dir == 0) { // impossible error, bad initialization
      MessageHandler::fail (
         __FILE__, __LINE__, TimeMessages::incomplete_setup_error, "\n"
   ...
   TimeConverter * converter_ptr = time_manager->get_converter_ptr (
      time_manager_init->get_conv_ptr_index (conv_index));
 ...
 */
void
TimeUDE::initialize_from_parent (
   TimeManagerInit * time_manager_init)
{
   ...
   if (!initializing_data_present && !epoch_data_present) {
      MessageHandler::warn (
         ...
         "Simulation will continue with these values", name.c_str());
   }
            ...
            if (!converter_ptr->is_initialized()) {
               if (!epoch_ptr->is_initialized() || !update_from_ptr->is_initialized()) {
                  MessageHandler::fail (
                     ...
                     name.c_str(), epoch_ptr->name.c_str(), update_from_ptr->name.c_str(),
                     name.c_str(), epoch_ptr->name.c_str(), update_from_ptr->name.c_str());
               }
   ...
   conv_index = update_index * time_manager->num_types + index;
   conv_dir   = time_manager_init->get_conv_dir_init (conv_index);
   if (conv_dir == 0) { // converter does not exist.  Redundant check,
      // impossible error.
      MessageHandler::fail (
         __FILE__, __LINE__, TimeMessages::incomplete_setup_error, "\n"
         "In initializing the epoch of time type %s, it was necessary to \n"
   ...
   if (revert_parent) { // parent currently has epoch value in it, change back to
 ...
 */
void
TimeUDE::initialize_initializer_time (
   TimeManagerInit * time_manager_init)
{
   ...
   if (time_manager_init->sim_start_format != initial_value_format) {
      if (time_manager_init->sim_start_format == TimeEnum::undefined) {
         MessageHandler::inform (
            __FILE__, __LINE__, TimeMessages::duplicate_methods, "\n"
   ...
   if (!updates_from_dyn) {
      ...
      initialized = true;
      converter_ptr->initialize (update_from_ptr,
                                 this,
                                 conv_dir);
      ...
      if ((std::fpclassify(init_secs) != FP_ZERO) || (std::fpclassify(init_days) != FP_ZERO)) {
         ...
         conv_dir   = time_manager_init->get_conv_dir_init (conv_index);
         if (conv_dir == 0) { // converter is not valid.
            MessageHandler::fail (
               __FILE__, __LINE__, TimeMessages::incomplete_setup_error, "\n"
   ...
   else { // time does update from Dyn, trivial case, just initialize converter.
      initialized = true;
      converter_ptr->initialize (update_from_ptr,
                                 this,
                                 conv_dir);
 ...
 */
void
TimeUDE::set_epoch_times (
   JeodBaseTime * epoch_ptr)

{
   TimeStandard * epoch_ptr_std = dynamic_cast<TimeStandard *> (epoch_ptr);
   TimeUDE * epoch_ptr_ude      = dynamic_cast<TimeUDE *> (epoch_ptr);
 ...
 */
void
TimeUDE::set_epoch_dyn (
   TimeDyn * epoch_ptr)
{
   if ((epoch_year != 0) || (epoch_month != 0) || (epoch_day != 0) ||
       (epoch_hour != 0) || (epoch_minute != 0)) {
      MessageHandler::fail (
         __FILE__, __LINE__, TimeMessages::invalid_setup_error, "\n"
   ...
   if (epoch_value_is_set_number) {
      ...
      case TimeEnum::undefined:
         MessageHandler::fail (
            ...
            epoch_defined_in_name.c_str(), name.c_str());
         break;
      ...
      case TimeEnum::truncated_julian:
         MessageHandler::fail (
            ...
            epoch_defined_in_name.c_str(), name.c_str(), epoch_defined_in_name.c_str());
         break;
 ...
 */
void
TimeUDE::set_epoch_std (
   TimeStandard * epoch_ptr)
{
   switch (epoch_format) {
   case TimeEnum::undefined:
      MessageHandler::fail (
   ...
   void)
{
   ...
   if ((clock_day != 0) || (clock_hour != 0) ||
       (clock_minute != 0) || (std::fpclassify(clock_second) != FP_ZERO)) {
      clock_set = true;
      count_set++;
   }
   ...
   // Too much initializing data
   ...
   else { // And this porridge is just right:  1 and only 1 value set.
      ...
      // or set by days, or ...
         ...
         seconds =  clock_day * 86400 + clock_hour * 3600 +
                   clock_minute * 60 + clock_second;
         days = seconds / 86400;
      }
 ...
 */
void
TimeUDE::set_epoch_initializing_value (
   ...
   void)
{
   ...
   if (epoch_index == -1) { // epoch_defined_in_name not found
      MessageHandler::fail (
         __FILE__, __LINE__, TimeMessages::invalid_setup_error, "\n"
   ...
   if ((epoch_month != 0) && (epoch_day != 0)) { // 0th month and 0th day are nonsense
      epoch_value_is_set_calendar = true;
   }

   if (((epoch_day != 0) || (epoch_hour != 0) || (epoch_minute != 0) ||
        (std::fpclassify(epoch_second) != FP_ZERO)) && ((epoch_year == 0) && (epoch_month == 0))) {
      epoch_value_is_set_clock = true;
   }

   if (epoch_value_is_set_number &&
       (epoch_value_is_set_calendar || epoch_value_is_set_clock)) {
      MessageHandler::fail (
         __FILE__, __LINE__, TimeMessages::redundancy_error, "\n"
   ...
   if (epoch_value_is_set_number ||
       epoch_value_is_set_clock ||
       epoch_value_is_set_calendar) {
      epoch_data_present = true;
   }

   // SANITY CHECKS

   if (epoch_index == -2) { // epoch type is undefined
   ...
   void)
{
   ...
   if (init_index == -1) { // init_from_name not found
      MessageHandler::warn (
         __FILE__, __LINE__, TimeMessages::redundancy_error, "\n"
         "For time %s:\n"
         ...
         name.c_str(), initialize_from_name.c_str() );
   }
