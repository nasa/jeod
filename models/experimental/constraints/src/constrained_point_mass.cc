...
void
ConstrainedPointMass::set_constraint_position (
...
void
ConstrainedPointMass::compute_constraint_position_root ()
{
    Vector3::incr (
        constraint_frame->origin_position_root,
...
void
ConstrainedPointMass::compute_constraint_position_com (
    const VehicleProperties& vehicle_properties)
{
    Vector3::diff (
        constraint_position_root,
        vehicle_properties.get_structure_to_body_offset(),
        constraint_position_com);
}


void
ConstrainedPointMass::compute_non_grav_accel (
    const VehicleNonGravState& non_grav_state)
{
    ...
    Vector3::transform (
        constraint_frame->T_root_constraint,
        non_grav_accel_root,
        non_grav_accel_constraint_frame);

    // Compute effective force.
    Vector3::scale (
        non_grav_accel_constraint_frame, mass, force_constraint_frame);
}
