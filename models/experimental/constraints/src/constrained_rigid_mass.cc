...
ConstrainedRigidMass::ConstrainedRigidMass(const ConstraintFrame * constraint_frame_in)
    : ConstrainedPointMass(constraint_frame_in)
{
}

void ConstrainedRigidMass::setup_constraint(const VehicleProperties & vehicle_properties,
                                            const VehicleNonGravState & non_grav_state)
{
    ...
    Vector3::transform(inertia, omega_constraint_frame, ang_mom_constraint_frame);
    Vector3::transform(inertia, omega_dot_constraint_frame, torque_constraint_frame);
    Vector3::cross_incr(omega_constraint_frame, ang_mom_constraint_frame, torque_constraint_frame);
}
