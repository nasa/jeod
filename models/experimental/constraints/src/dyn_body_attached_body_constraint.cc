...
void
DynBodyAttachedBodyConstraint::setup_constraint(
    ...
    const VehicleNonGravState& non_grav_state)
{
    ...
    for (unsigned ii = 0; ii < 3; ++ii)
    {
        force_constraints[ii].set_right_hand_side (
            -constrained_mass.force_constraint_frame[ii]);
        torque_constraints[ii].set_right_hand_side (
            -constrained_mass.torque_constraint_frame[ii]);
    }
}


void
DynBodyAttachedBodyConstraint::set_constraint_values (
