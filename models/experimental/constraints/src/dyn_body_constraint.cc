...
void
DynBodyConstraint::attach_to_solver ()
{
...
void
DynBodyConstraint::detach_from_solver ()
{
    for (unsigned ii = 0; ii < n_dimensions; ++ii)
    {
...
void
DynBodyConstraint::update_component_attachment (
...
void
DynBodyConstraint::setup_component_constraint (
    const VehicleProperties& vehicle_properties,
...
void
DynBodyConstraint::set_r_h_s (
    VectorView<double>& rhs_slice) const
{
    for (unsigned ii = 0; ii < n_dimensions; ++ii)
    {
...
void
DynBodyConstraint::set_cross_coeff (
    const VehicleProperties& vehicle_properties,
    ...
    MatrixView<double>& a_slice) const
{
    for (unsigned ii = 0; ii < n_dimensions; ++ii)
    {
        ...
        {
            a_slice(ii, jj) = this_component.get_cross_coeff(
                vehicle_properties,
                *other.get_component(jj));
        }
    }
}
...
void
DynBodyConstraint::set_self_coeff (
    const VehicleProperties& vehicle_properties,
    MatrixView<double>& a_slice) const
{
...
void
DynBodyConstraint::set_constraint_values (
...
void
DynBodyConstraint::compute_constraint_response (
    const VehicleProperties& vehicle_properties,
    const VehicleNonGravState& non_grav_state)
{
    for (unsigned ii = 0; ii < n_dimensions; ++ii)
    {
        get_component(ii)->compute_constraint_response (
...
DynBody*
DynBodyConstraint::get_dyn_body()
{
