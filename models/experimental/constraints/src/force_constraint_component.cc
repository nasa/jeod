...
ForceConstraintComponent::ForceConstraintComponent(ConstraintFrame * constraint_frame_in,
...
void ForceConstraintComponent::setup_constraint(const VehicleProperties & vehicle_properties,
                                                const VehicleNonGravState &)
{
    ...
    constraint_wrench.set_force(constraint_direction_root, constrained_mass->constraint_position_root);

    // Compute the vehicle's translational acceleration in response to this
    // constraint at the vehicle's center of mass.
    Vector3::scale(constraint_direction_root, vehicle_properties.get_inverse_mass(), constraint_com_accel_root);

    // Compute the vehicle's rotational acceleration in response to this
    // constraint.
    double torque[3];
    Vector3::cross(constrained_mass->constraint_position_com, constraint_direction_root, torque);
...
double ForceConstraintComponent::get_cross_coeff(const VehicleProperties & vehicle_properties,
                                                 const ConstraintComponent & other) const
{
    double accel[3];

    other.get_translational_response(vehicle_properties, constrained_mass->constraint_position_com, accel);

    return constrained_mass->mass * Vector3::dot(constraint_direction_root, accel);
}

SolverTypes::DecayedVector3T ForceConstraintComponent::get_translational_response(const VehicleProperties &,
                                                                                  SolverTypes::ConstVector3T point,
                                                                                  SolverTypes::Vector3T response) const
{
    Vector3::incr(constraint_com_accel_root, Vector3::cross(constraint_omega_dot_root, point, response));

    return response;
}

void ForceConstraintComponent::set_constraint_value(double constraint_value)
{
