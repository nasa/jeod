...
PendulumConstraintComponent::PendulumConstraintComponent(ConstraintFrame * constraint_frame_in,
...
void PendulumConstraintComponent::create_integrators(const er7_utils::IntegratorConstructor & generator,
...
void PendulumConstraintComponent::reset_integrators()
{
    pendulum_model->update_pendulum_model();
    constrained_mass->mass = pendulum_model->get_pendulum_mass();
    pendulum_model->get_hinge_point(constrained_mass->constraint_position_constraint_frame);
...
void PendulumConstraintComponent::attach_to_solver(DynBodyConstraintsSolver *, DynBody * dyn_body_in)
{
    dyn_body = dyn_body_in;
    dyn_body->add_integrable_object(*this);
}

void PendulumConstraintComponent::detach_from_solver(DynBodyConstraintsSolver *, DynBody *)
{
    if(dyn_body != nullptr)
    {
...
void PendulumConstraintComponent::activate()
{
...
void PendulumConstraintComponent::deactivate()
{
    mode = PendulumConstraintMode::Inactive;
}

bool PendulumConstraintComponent::check_for_reset()
{
    ...
    else if(((mode == PendulumConstraintMode::Vertical) || (mode == PendulumConstraintMode::Inverted)) &&
    ...
    else if((mode == PendulumConstraintMode::Horizontal) && (cos_sq_theta > cos_sq_theta_cartesian))
    {
...
void PendulumConstraintComponent::transition_to_horizontal()
{
    ...
    theta_dot = Vector3::dot(bob_velocity, constraint_to_spherical_transform[1]) / pendulum_length;
    phi_dot = Vector3::dot(bob_velocity, constraint_to_spherical_transform[2]) / (pendulum_length * sin_theta);
    compute_rho_hat_dot();
}

void PendulumConstraintComponent::set_pendulum_length(double new_length)
{
...
void PendulumConstraintComponent::setup_constraint(const VehicleProperties & vehicle_properties,
                                                   const VehicleNonGravState & non_grav_state)
{
    ...
    ForceConstraintComponent::setup_constraint(vehicle_properties, non_grav_state);

    Vector3::transform(constraint_frame->T_root_constraint, non_grav_state.omega_struct, omega_constraint_frame);
    ...
    right_hand_side = constrained_mass->mass * ((Vector3::vmagsq(bob_velocity) + Vector3::vmagsq(omega_cross_pos) -
                                                 2.0 * Vector3::dot(omega_cross_vel, bob_position)) /
                                                pendulum_length) -
                      Vector3::dot(constrained_mass->force_constraint_frame, constraint_direction_constraint_frame);
}

er7_utils::IntegratorResult PendulumConstraintComponent::integrate(double dyn_dt, unsigned int target_stage)
{
    if(mode == PendulumConstraintMode::Horizontal)
    {
...
void PendulumConstraintComponent::construct_constraint_to_spherical_transform()
{
...
er7_utils::IntegratorResult PendulumConstraintComponent::integrate_spherical(double dyn_dt, unsigned int target_stage)
{
    er7_utils::IntegratorResult result = bob_integrator.integrate(dyn_dt,
                                                                  target_stage,
                                                                  &theta_dot_dot,
                                                                  &theta_dot,
                                                                  &theta);
...
er7_utils::IntegratorResult PendulumConstraintComponent::integrate_cartesian(double dyn_dt, unsigned int target_stage)
{
    er7_utils::IntegratorResult result = bob_integrator.integrate(dyn_dt,
                                                                  target_stage,
                                                                  bob_accel,
...
void PendulumConstraintComponent::compute_rho_hat_dot()
{
...
void PendulumConstraintComponent::compute_bob_state_spherical()
{
    Vector3::scale(constraint_to_spherical_transform[0], pendulum_length, bob_position);
    Vector3::scale(rho_hat_dot, pendulum_length, bob_velocity);

    set_constraint_direction(constraint_to_spherical_transform[0]);
}

void PendulumConstraintComponent::compute_bob_state_cartesian()
{
    ...
    bob_velocity[2] = -(bob_position[0] * bob_velocity[0] + bob_position[1] * bob_velocity[1]) / bob_position[2];
...
void PendulumConstraintComponent::compute_constraint_response(const VehicleProperties &,
                                                              const VehicleNonGravState & non_grav_state)
{
    ...
    if(mode == PendulumConstraintMode::Horizontal)
    {
        double omega_rho = Vector3::dot(constraint_to_spherical_transform[0], omega_constraint_frame);
        double accel_theta = Vector3::dot(constraint_to_spherical_transform[1],
                                          constrained_mass->non_grav_accel_constraint_frame);
        double omega_theta = Vector3::dot(constraint_to_spherical_transform[1], omega_constraint_frame);
        double omega_dot_theta = Vector3::dot(constraint_to_spherical_transform[1], omega_dot_constraint_frame);
        double accel_phi = Vector3::dot(constraint_to_spherical_transform[2],
                                        constrained_mass->non_grav_accel_constraint_frame);
        double omega_phi = Vector3::dot(constraint_to_spherical_transform[2], omega_constraint_frame);
        double omega_dot_phi = Vector3::dot(constraint_to_spherical_transform[2], omega_dot_constraint_frame);

        theta_dot_dot = -(damping_factor * theta_dot + accel_theta / pendulum_length +
                          omega_rho * (omega_theta - 2.0 * phi_dot * sin_theta) + omega_dot_phi -
                          phi_dot * phi_dot * sin_theta * cos_theta);
        phi_dot_dot = -(damping_factor * phi_dot +
                        (accel_phi / pendulum_length + omega_rho * (omega_phi + 2.0 * theta_dot) - omega_dot_theta +
    ...
    else
    {
        ...
        double * non_grav_accel_constraint_frame = constrained_mass->non_grav_accel_constraint_frame;

        Vector3::cross(omega_dot_constraint_frame, bob_position, omega_dot_cross_pos);
        Vector3::cross(omega_constraint_frame, omega_cross_pos, omega_cross_omega_cross_pos);

        for(int ii = 0; ii < 2; ++ii)
        {
            // Calculation is done in two steps for numerical reasons.
            // The first part will typically be smaller than the second.
            bob_accel[ii] = -(damping_factor * bob_velocity[ii]            // Drag
    ...
    Vector3::scale(constrained_mass->mass * damping_factor, damping_torque_constraint_frame);
