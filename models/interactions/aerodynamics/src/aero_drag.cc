...
// behavior
   if (use_default_behavior == true) {
      default_behavior->aerodrag_force (rel_vel_mag, rel_vel_struct_hat,
                                        &param, mass, aero_force, aero_torque);
      return;
   ...
   for  (i_p = 0; i_p < aero_surface_ptr->facets_size; ++i_p) {
      ...
      aero_surface_ptr->aero_facets[i_p]->aerodrag_force (rel_vel_mag,
                                                          rel_vel_struct_hat,
                                                          &param,
                                                          center_grav);
 ...
 */
void
AerodynamicDrag::set_aero_surface(
   AeroSurface& to_set)
{
