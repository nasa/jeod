   ...
   double center_grav[3])
{
   ...
   double sin_alpha; /* --
      alpha is the angle of attack, the angle between the plate and the relative
       ...
       that angle.  */
   double cos_alpha; /* --
      cosine of angle alpha (see sin_alpha)  */
   double one_p_epsilon; /* --
      epsilon is the fraction of incident molecules that "bounce"; this is
       1+epsilon */
   double one_m_epsilon; /* --
      1-epsilon (see one_p_epsilon)  */
   double s; /* --
   ...
   double s_2; /* --
      s^2 (see definition of s)  */
   double s_sinalpha; /* --
      s * sin(alpha) (see defintion of s and of sin_alpha)  */
   double s_sa2; /* --
      (s_sinalpha)^2  */
   double exp_ssa2; /* --
       e^(-s_sa2) */
   double erf_ssa; /* --
       Error function, erf of s_sinalpha; erf(s_sinalpha) */
   double local_temp_reflect; /* --
      temperature of the reflected molecules  */
   double temp_ratio; /* --
      ratio of the temperature of the reflected molecules to that of the incident
       molecules */
   double force_base; /* N
   ...
   if (calculate_drag_coef == true) {
      ...
      s = rel_vel_mag / sqrt (2.0 * aero_drag_param_ptr->gas_const *
      ...
      case AeroDragEnum::Calc_coef:
         ...
         one_p_epsilon =  1 + epsilon;
         one_m_epsilon =  1 - epsilon;
         s_2           = s * s;
         s_sinalpha    = s * sin_alpha;
         s_sa2         = s_sinalpha * s_sinalpha;
         exp_ssa2      = exp (-s_sa2);

         erf_ssa = erf (s_sinalpha);

         local_temp_reflect = (one_m_epsilon) * temperature +
