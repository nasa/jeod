...
void
ContactSurfaceFactory::create_surface (
   ...
   InteractionSurface* inter_surface)
{
   ...
   for (unsigned int ii = 0; ii < surface->facets.size (); ++ii) {
      ...
      if ((ii == 0) ||
          (strcmp (surface->facets[ii - 1]->get_mass_body_ptr()->name.c_str(),
                   surface->facets[ii]->get_mass_body_ptr()->name.c_str()) == 0)) {
 ...
 */
void
ContactSurfaceFactory::add_facet_params (
   FacetParams* to_add)
{
