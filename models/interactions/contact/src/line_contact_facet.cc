   ...
   length (0.0)
{
   ...
   Vector3::initialize (contact_point);
}
   ...
   Contact * contact)
{
   ...
   if (temp_line == NULL) {
      ...
      contact_pair->interaction_distance = (max_dimension + temp_point->max_dimension) * contact->contact_limit_factor;
 ...
 */
void LineContactFacet::calculate_contact_point(
   double nvec[3])
{
   double end_point[3];
   double sign = 1.0;
   ...
   if (Vector3::vmag(unit) > 1e-10) {
      ...
      contact_point[0] = contact_point[0] + sign *
      sqrt(Numerical::square(x) *
 ...
 */
void LineContactFacet::calculate_torque (
   double *tmp_force)
{
   double cm[3];
   double vec[3];
   ...
   Vector3::diff(vehicle_point->mass_point->position,
                 vehicle_body->mass.composite_properties.position,
                 cm);

   // rotate facet contact point to vehicle structure frame
   Vector3::transform_transpose (vehicle_point->state.rot.T_parent_this,
                                 contact_point, vec);
   Vector3::transform (vehicle_body->structure.state.rot.T_parent_this,
                       vec, tmp_torque);
