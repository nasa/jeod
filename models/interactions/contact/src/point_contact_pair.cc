   ...
   void)
{
   ...
   Vector3::transform(rel_state.rel_state.rot.T_parent_this,
                      rel_state.rel_state.trans.position, nvec);
 ...
 */
void PointContactPair::initialize_pair (
   ...
   ContactFacet *target_facet)
{
   ...
   if (target_facet != NULL) {
      ...
      ContactUtils::copy_const_char_to_char (point_subject->vehicle_point->get_name (),
