 ...
 */
void GravityTorque::initialize(DynBody & subject)
{
...
void GravityTorque::update()
{
    ...
    if(subject_body == nullptr)
    {
        MessageHandler::fail(__FILE__,
                             __LINE__,
                             GravityTorqueMessages::initialization_error,
    ...
    Matrix3x3::transform_matrix(subject_body->composite_body.state.rot.T_parent_this,
                                subject_body->grav_interaction.grav_grad,
                                g);

    // torque in body frame
    torque[0] = g[1][2] * (inertia[2][2] - inertia[1][1]) - g[0][2] * inertia[0][1] + g[0][1] * inertia[0][2] -
                inertia[1][2] * (g[2][2] - g[1][1]);

    torque[1] = g[0][2] * (inertia[0][0] - inertia[2][2]) + g[1][2] * inertia[0][1] - g[0][1] * inertia[1][2] -
                inertia[0][2] * (g[0][0] - g[2][2]);

    torque[2] = g[0][1] * (inertia[1][1] - inertia[0][0]) - g[1][2] * inertia[0][2] + g[0][2] * inertia[1][2] -
                inertia[0][1] * (g[1][1] - g[0][0]);

    // Transform to structure.
    Vector3::transform_transpose(subject_body->mass.composite_properties.T_parent_this, torque);
}
