...
void verif_grav_torque(const PointMass & ptmass,
                       ...
                       const double grav_pot)
{
    ...
    for(int ii = 0; ii < ptmass.num_pts; ii++)
    {
        ...
        double x = r * cos(ptmass.pt[ii].posn[0] * deg2rad) * cos(ptmass.pt[ii].posn[1] * deg2rad);
        double y = r * cos(ptmass.pt[ii].posn[0] * deg2rad) * sin(ptmass.pt[ii].posn[1] * deg2rad);
        ...
        Vector3::scale_incr(del_pos, G_scale, pt_accel);
    }
    ...
    for(unsigned int ii_sc = 0; ii_sc < 3; ii_sc++)
    {
        ...
        for(int ii = 0; ii < ptmass.num_pts; ii++)
        {
            ...
            double G_scale = ptmass.G * ptmass.pt[ii].mass * sc_mass[ii_sc][0] / (r_mag * r_mag * r_mag);
            Vector3::scale_incr(r_scpt_pt, G_scale, F);
        }
