 ...
 */
void RadiationDefaultSurface::initialize()
{
    ...
    if((rad_coeff > 0) && (albedo > 0))
    { // hence diffuse must be > 0 also
    ...
    if(rad_coeff > 0)
    { // defined by rad_coeff, albedo and diffuse not defined
 ...
 */
void RadiationDefaultSurface::incident_radiation(const double flux_mag,
                                                 ...
                                                 const bool calculate_forces)
{
    ...
    double tempvec1[3];
    // absorption
    Vector3::scale(flux_struc_hat, areaxflux * (1.0 - albedo), tempvec1);
    ...
    // Diffuse reflection
    Vector3::scale(flux_struc_hat, ref_flux * diffuse * 13.0 / 9.0, tempvec1);
    Vector3::incr(tempvec1, F_diffuse);

    // Specular reflection
    Vector3::scale(flux_struc_hat, ref_flux * (1.0 - diffuse), tempvec1);
 ...
 * necessary
 */
void RadiationDefaultSurface::thermal_update()
{
    if(thermal.active)
    {
 ...
 */
void RadiationDefaultSurface::add_thermal_integrator_to(DynBody * dyn_body)
{
