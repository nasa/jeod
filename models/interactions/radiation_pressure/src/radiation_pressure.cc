 ...
 */
void
RadiationPressure::initialize_environment (
   ...
   double real_time)
{
   ...
   ThermalFacetRider::cycle_time =
                           JeodSimulationInterface::get_job_cycle() * scale_factor;


   source.calculate_flux (veh_struc_frame,
                              center_grav);

   if (third_bodies_active) {
      third_body_adjustments(real_time,
                             veh_struc_frame);
   }
...
//    multi_shadows = false;
   for (unsigned int ii_body = 0; ii_body < num_third_bodies; ++ii_body) {
      if (third_bodies[ii_body]->active) {
         illum_factor *= third_bodies[ii_body]->process_third_body( real_time,
                                                                    veh_struc_frame);
      }
   }
    ...
    RadiationThirdBody * third_body_ptr)
{
   ...
   if (third_body_ptr->name.empty()) {
      if (initialized) {
         ...
         __FILE__, __LINE__, RadiationMessages::incomplete_setup_error, "\n"
   ...
   if (third_body_ptr->get_added_to_model() ) {
      ...
      "field.\n",
      third_body_ptr->name.c_str(), third_body_ptr->name.c_str(), third_body_ptr->name.c_str());
   }
 ...
 */
void
RadiationPressure::set_third_body_active (
    const char * third_body_name)
{
   int body_index = find_third_body( third_body_name );
   if (body_index >= 0) {
      if (third_bodies[body_index]->active) {
         MessageHandler::warn (
         __FILE__, __LINE__, RadiationMessages::invalid_function_call, "\n"
 ...
 */
void
RadiationPressure::set_third_body_inactive (
    const char * third_body_name)
{
   int body_index = find_third_body( third_body_name );
   if (body_index >= 0) {
 ...
 */
int
RadiationPressure::find_third_body (
    const char * third_body_name)
{
   for (unsigned int ii_body = 0; ii_body < num_third_bodies; ++ii_body) {
      if (std::strcmp (third_bodies[ii_body]->name.c_str(), third_body_name) == 0) {
         return ii_body;
      }
   }
   MessageHandler::error (
 ...
 */
void
RadiationPressure::set_calculate_forces(
   bool value)
{
