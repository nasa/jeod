...
/******************************************************************************
  Purpose:
    ...
    with facets).)
******************************************************************************/
void RadiationPressure::initialize(DynManager & dyn_mgr, RadiationSurface * surf_ptr, double center_grav[3])
{
 ...
 */
void RadiationPressure::update_facet_surface()
{
    ...
    {
        surface_ptr->incident_radiation(source.flux_mag, source.flux_struc_hat, calculate_forces);
    }
    ...
    for(unsigned int ii_body = 0; ii_body < num_third_bodies; ++ii_body)
    {
        ...
        {
            surface_ptr->interact_with_third_body(third_bodies[ii_body], calculate_forces);
        }
    }
    ...
    if(calculate_forces)
    {
        ...
        Vector3::copy(surface_ptr->force, force);
        Vector3::copy(surface_ptr->torque, torque);
    }
}
