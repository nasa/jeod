   ...
   double center_grav[3])
{
   ...
   for (ii_facet = 0; ii_facet < num_facets; ++ii_facet) {
      ...
      if ((facets[ii_facet]->thermal.emissivity > 1) ||
            (facets[ii_facet]->thermal.emissivity < 0)) {
         MessageHandler::fail (
            __FILE__, __LINE__, RadiationMessages::invalid_setup_error, "\n"
 ...
 */
void
RadiationSurface::allocate_array (
   unsigned int size)
{
 ...
 */
void
RadiationSurface::allocate_interaction_facet (
   Facet* facet,
   ...
   unsigned int index)
{
   ...
   RadiationFacet* temp_radiation_facet =
 ...
 */
void
RadiationSurface::incident_radiation (
   double flux_mag,
   ...
   bool calculate_forces)
{
   ...
   for (ii_facet = 0; ii_facet < num_facets; ++ii_facet) {
      facets[ii_facet]->incident_radiation (flux_mag,
                                            flux_struc_hat,
                                            calculate_forces);
   }
 ...
 */
void
RadiationSurface::interact_with_third_body (
   RadiationThirdBody * third_body_ptr,
   const bool calculate_forces)
{
   for (ii_facet = 0; ii_facet < num_facets; ++ii_facet) {
      facets[ii_facet]->interact_with_third_body (third_body_ptr,
            calculate_forces);
   }
   return;
}
 ...
 */
void
RadiationSurface::accumulate_thermal_sources (
   void)
{
   for (ii_facet = 0; ii_facet < num_facets; ++ii_facet) {
 ...
 */
void
RadiationSurface::thermal_integrator (
   void)
{
   for (ii_facet = 0; ii_facet < num_facets; ++ii_facet) {
      if (facets[ii_facet]->thermal.integrable_object.active) {
         facets[ii_facet]->base_facet->temperature =
            facets[ii_facet]->thermal.integrable_object.get_temp();
      }
      else {
         facets[ii_facet]->base_facet->temperature =
            facets[ii_facet]->thermal.integrate();
      }
   }
   return;
}
 ...
 */
void
RadiationSurface::equalize_absorption_emission (
   void)
{
   for (ii_facet = 0; ii_facet < num_facets; ++ii_facet) {
      facets[ii_facet]->thermal.power_emit =
         facets[ii_facet]->thermal.power_absorb;
   }
   return;
}
...
/**
 * systematically calls the method to wrap up the radiation pressure calculation on each facet and accumulate forces and torques over all facets.
 */
void
RadiationSurface::radiation_pressure (
   void)
{
   Vector3::initialize (force);
   Vector3::initialize (torque);
   ...
   for (ii_facet = 0; ii_facet < num_facets; ++ii_facet) {
      ...
      Vector3::incr (facets[ii_facet]->force,
                     force);
      Vector3::incr (facets[ii_facet]->torque,
                     torque);
   }
   return;
}
 ...
 */
void
RadiationSurface::initialize_runtime_values(
   void)
{
   for (ii_facet = 0; ii_facet < num_facets; ++ii_facet) {
 ...
 */
void
RadiationSurface::add_thermal_integrators_to
(DynBody * dyn_body)
{
