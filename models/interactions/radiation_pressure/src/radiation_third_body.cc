 ...
 */
void RadiationThirdBody::initialize(DynManager * dyn_mgr_ptr)
{
    if(name.empty())
    {
...
void RadiationThirdBody::calculate_shadow()
{
    ...
    if(shadow_geometry == Cylindrical || shadow_geometry == Cyl)
    {
    ...
    else if(shadow_geometry == Conical || shadow_geometry == Con)
    {
        ...
        double delta = radius_x_d - r_perp_x_d + (radius + primary_source_ptr->radius) * r_par;
        if(ang_ratio_2 >= 1)
        { // ThirdBody has a larger angular size
            illum_factor = 1 - generate_alpha(1 / ang_ratio, delta / (2 * primary_source_ptr->radius * r_par));
        }
        else
        {
            illum_factor = 1 -
 ...
 */
double RadiationThirdBody::generate_alpha(double rho_adj, double delta)
{
    // Coefficients are empirically derived to provide close match
    // to circular geometry.  See documentation for origin of
    // these values.
    double a3 = 0.758656 * rho_adj * rho_adj + -0.0637441 * rho_adj - 1.08955;
    double a2 = -1.2205 * rho_adj * rho_adj + 0.61815 * rho_adj + 1.62909;
    double a1 = 0.43723 * rho_adj * rho_adj + -0.52921 * rho_adj + 0.473649;
    double a0 = -0.01242 * rho_adj * rho_adj * rho_adj + -0.0036715 * rho_adj * rho_adj + 0.0150263 * rho_adj +
                -0.0078259;
    return a3 * delta * delta * delta + a2 * delta * delta + a1 * delta + a0;
}
 ...
 */
void RadiationThirdBody::convert_shadow_from_int(int old_shadow_geometry)
{
 ...
 */
double RadiationThirdBody::process_third_body(double real_time, RefFrame & veh_struc_frame JEOD_UNUSED)
{
 ...
 * \param[in] real_time Current time\n Units: s
 */
bool RadiationThirdBody::test_for_state_update(double real_time)
{
    // Always update on the first pass, and thereafter whenever the
    // "update_interval" time has passed since the last update.
    if(force_state_update || std::abs(real_time - last_update_time) > update_interval)
    {
 ...
 */
bool RadiationThirdBody::update_third_body_state()
{
    local_frame_ptr->compute_position_from(*(primary_source_ptr->inertial_frame_ptr), source_to_third_inrtl);
    ...
    Vector3::scale(source_to_third_inrtl, 1 / d_source_to_third, source_to_third_hat_inrtl);
    return true;
}
