 ...
 * Constructor
 */
ThermalFacetRider::ThermalFacetRider (
   void)
{
   d_temperature       = 0.0;
   power_absorb        = 0.0;
   ...
   double surface_area)
{
   ...
   if (emissivity < 0) {
      MessageHandler::fail (
         __FILE__, __LINE__, ThermalMessages::incomplete_setup_error, "\n"
   ...
   void)
{
  ...
  #define pow4(x) pow4_temp = (x) * (x), pow4_temp *= pow4_temp
   ...
   pow4 (T0);
   if (pow4_temp > Teq4) { // above equilibrium
      dt_dir = -1;
   }
   else {
      dt_dir = 1;
   ...
   if (I2 * dt_dir < 0) { // jumped through asymptote.
      MessageHandler::inform (
         __FILE__, __LINE__, ThermalMessages::invalid_integration_operation,
         "\n"
      ...
      power_emit       = power_absorb -
                         (heat_capacity * d_temperature / cycle_time);
      return dynamic_temperature;
   }
   ...
   else { // temperature went right direction, but did it go too far?


      next_temperature = dynamic_temperature + d_temperature;
      pow4 (next_temperature);

      if (dt_dir * (Teq4 - pow4_temp) < 0) { // overshot equilibrium value
         MessageHandler::inform (
            __FILE__, __LINE__, ThermalMessages::invalid_integration_operation,
            "\n"
