 ...
 */
void
ThermalIntegrableObject::create_integrators(
 ...
 */
void
ThermalIntegrableObject::destroy_integrators(
   void)
{
   // nothing to do
}
 ...
 */
void
ThermalIntegrableObject::reset_integrators (
   void)
{
...
ThermalIntegrableObject::integrate (
   double dyn_dt,
   unsigned int target_stage)
{
   double t_eq_pow4 = rider->power_absorb/rider->rad_constant;
   er7_utils::IntegratorResult result =
      integrator.integrate (dyn_dt, target_stage, &temp_dot, &temp);

   t_pow4 = temp*temp;
   t_pow4*=t_pow4;

   if (temp_dot*(t_eq_pow4-t_pow4) < 0.0) { // overshot the asymtotic value
 ...
 */
void
ThermalIntegrableObject::initialize(
   double temperature,
   ThermalFacetRider &associated_rider)
{
   temp = temperature;
 ...
 */
void
ThermalIntegrableObject::compute_temp_dot(
   void)
{
