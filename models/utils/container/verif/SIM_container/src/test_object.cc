...
std::ostream&
operator << (std::ostream& out, const TestObject & obj) {
   return (out << "ival=" << obj.ival << " dval=" << obj.dval);
}

bool
operator < (const TestObject & x, const TestObject & y) {
...
struct PrintContainer {
      ...
      const ContainerType & container)
   {
      typedef typename ContainerType::const_iterator
         const_iterator;
      // Had to remove to prevent error: typedef 'const_reverse_iterator'
      // locally defined but not used [-Werror=unused-local-typedefs]
      // on Trick 17 - Jordan 1/2019
...
TestContainer::shutdown (
   void)
{
   print_container<JeodObjectList<TestObject>::type, TestObject> (
      "object_list", object_list);
   print_container<JeodObjectVector<TestObject>::type, TestObject> (
      "object_vec", object_vec);
   print_container<JeodObjectSet<TestObject>::type, TestObject> (
      "object_set", object_set);

   print_container<JeodPointerList<TestObject>::type, TestObject*> (
      "pointer_list", pointer_list);
   print_container<JeodPointerVector<TestObject>::type, TestObject*> (
      "pointer_vec", pointer_vec);
   print_container<JeodPointerSet<TestObject>::type, TestObject*> (
      "pointer_set", pointer_set);

   print_container<JeodPrimitiveList<double>::type, double> (
      "double_list", double_list);
   print_container<JeodPrimitiveVector<double>::type, double> (
      "double_vec", double_vec);
   print_container<JeodPrimitiveSet<double>::type, double> (
      "double_set", double_set);

   print_container<JeodPrimitiveList<float>::type, float> (
      "float_list", float_list);
   print_container<JeodPrimitiveVector<float>::type, float> (
      "float_vec", float_vec);
   print_container<JeodPrimitiveSet<float>::type, float> (
      "float_set", float_set);

   print_container<JeodPrimitiveList<int>::type, int> (
      "int_list", int_list);
   print_container<JeodPrimitiveVector<int>::type, int> (
      "int_vec", int_vec);
   print_container<JeodPrimitiveSet<int>::type, int> (
      "int_set", int_set);

   print_container<JeodPrimitiveList<bool>::type, bool> (
      "bool_list", bool_list);
   print_container<JeodPrimitiveVector<bool>::type, bool> (
      "bool_vec", bool_vec);
   print_container<JeodPrimitiveSet<bool>::type, bool> (
      "bool_set", bool_set);

   print_container<JeodPrimitiveList<std::string>::type, std::string> (
      "string_list", string_list);
   print_container<JeodPrimitiveVector<std::string>::type, std::string> (
      "string_vec", string_vec);
   print_container<JeodPrimitiveSet<std::string>::type, std::string> (
