...
void
print_expected (
   const ElemType & item1)
{
      std::cout << "Expect: contents = "
                << item1 << "\n";
}
...
template <typename JeodContainerType,
          typename STLContainerType,
          typename ElemType>
struct TestContainer {

   void forward_copy (
      const JeodContainerType & source,
      JeodContainerType & dest)
   {
      typedef typename JeodContainerType::const_iterator IterType;
      dest.clear();
      for (IterType it = source.begin(); it != source.end(); ++it) {
   ...
   void reverse_copy (
      ...
      JeodContainerType & dest)
   {
      typedef typename JeodContainerType::const_reverse_iterator IterType;
      dest.clear();
      for (IterType it = source.rbegin(); it != source.rend(); ++it) {
         dest.insert (dest.end(), *it);
      }
   }

   template <typename Type>
   void summarize_contents (
      Type & container,
      const std::string & id)
   {
   ...
   const
   {
      ...
      for (typename Type::const_iterator it = container.begin();
           it != container.end();
           ++it) {
         std::cout << " " << *it;
      }
      if (reverse) {
         ...
         for (typename Type::const_reverse_iterator it = container.rbegin();
              it != container.rend();
              ++it) {
            std::cout << " " << *it;
         }
   ...
   const
   {
      ...
      std::cout << "  size()=" << container.size() << "\n";
      std::cout << "  forward contents:";
      for (typename Type::iterator it = container.begin();
           it != container.end();
           ++it) {
         std::cout << " " << *it;
      }
      if (reverse) {
         std::cout << "\n";
         std::cout << "  reverse contents:";
         for (typename Type::reverse_iterator it = container.rbegin();
              it != container.rend();
              ++it) {
            std::cout << " " << *it;
         }
...
void test_sequence_container (
   const TestContainer<JeodContainerType, STLContainerType, ElemType> &
      examiner,
   const std::string & id,
   JeodContainerType & jeod_container1,
   const JeodContainerType & jeod_container2,
...
void test_associative_container (
   ...
   JeodContainerType & jeod_container4)
{
   typename JeodContainerType::key_compare key_comp =
      jeod_container1.key_comp();
   typename JeodContainerType::value_compare value_comp =
      jeod_container1.value_comp();
   typename JeodContainerType::const_iterator citer;
   typename JeodContainerType::iterator iter;
   std::pair<typename JeodContainerType::iterator,
              typename JeodContainerType::iterator> range;
   std::pair<typename JeodContainerType::const_iterator,
              typename JeodContainerType::const_iterator> crange;
   int count;
   ...
   std::cout << "Number of elements smaller than 2 via key_comp="
             << count << "\n";
   count = 0;
   iter = jeod_container1.begin();
   ...
   std::cout << "equal_range(2): Found="
             << (crange.first != jeod_container2.end()) << "\n";
   ...
   JeodContainerType & jeod_container4)
{
   ...
   test_sequence_container (
      examiner, id, jeod_container1, jeod_container2, jeod_container4);
   ...
   ++beg_iter;
   jeod_container1.splice (jeod_container1.begin(),
                           jeod_container4, beg_iter, end_iter);
   ...
   JeodContainerType & jeod_container4)
{
   ...
   std::cout << "jeod_container1.capacity()="
             << jeod_container1.capacity() << "\n";
...
template <>
void
TestContainer<JeodPrimitiveVector<double>::type,
              std::vector<double>,
              double>::test_special (
   const std::string & id,
   ...
   const JeodPrimitiveVector<double>::type & jeod_container2)
{
   ...
   test_vector (
      *this, id, jeod_container1, jeod_container2, jeod_container4);
}

template <>
void
TestContainer<JeodPrimitiveList<double>::type,
              std::list<double>,
              double>::test_special (
   const std::string & id,
...
template <>
void
TestContainer<JeodObjectList<Foo>::type,
              std::list<Foo>,
              Foo>::test_special (
   const std::string & id,
...
template <>
void
TestContainer<JeodObjectSet<Foo>::type,
              std::set<Foo>,
              Foo>::test_special (
   const std::string & id,
...
int main ()
{
   ...
   test_container<JeodPrimitiveList<double>::type,
                  std::list<double>,
                  double> (
      "JeodPrimitiveList<double>::type",
       1, 2, 3);

   test_container<JeodPrimitiveList<std::string>::type,
                  std::list<std::string>,
                  std::string> (
      "JeodPrimitiveList<std::string>::type",
      "a", "b", "c");

   test_container<JeodObjectList<Foo>::type,
                  std::list<Foo>,
                  Foo> (
      "JeodObjectList<Foo>::type",
      foo1, foo2, foo3);

   test_container<JeodPointerList<Foo>::type,
                  std::list<Foo*>,
                  Foo*> (
      "JeodPointerList<Foo*>::type",
      &foo1, &foo2, &foo3);


   test_container<JeodPrimitiveVector<bool>::type,
                  std::vector<bool>,
                  bool> (
      "JeodPrimitiveVector<bool>::type",
      0, 0, 1);

   test_container<JeodPrimitiveVector<double>::type,
                  std::vector<double>,
                  double> (
      "JeodPrimitiveVector<double>::type",
      1, 2, 3);

   test_container<JeodPrimitiveVector<std::string>::type,
                  std::vector<std::string>,
                  std::string> (
      "JeodPrimitiveVector<std::string>::type",
      "a", "b", "c");

   test_container<JeodObjectVector<Foo>::type,
                  std::vector<Foo>,
                  Foo> (
      "JeodObjectVector<Foo>::type",
      foo1, foo2, foo3);

   test_container<JeodPointerVector<Foo>::type,
                  std::vector<Foo*>,
                  Foo*> (
      "JeodPointerVector<Foo*>::type",
      &foo1, &foo2, &foo3);


   test_container<JeodPrimitiveSet<double>::type,
                  std::set<double>,
                  double> (
      "JeodPrimitiveSet<double>::type",
      1, 2, 3);

   test_container<JeodPrimitiveSet<std::string>::type,
                  std::set<std::string>,
                  std::string> (
      "JeodPrimitiveSet<std::string>::type",
      "a", "b", "c");

   test_container<JeodObjectSet<Foo>::type,
                  std::set<Foo>,
                  Foo> (
      "JeodObjectSet<Foo>::type",
      foo1, foo2, foo3);

   test_container<JeodPointerSet<Foo>::type,
                  std::set<Foo*>,
                  Foo*> (
      "JeodPointerSet<Foo*>::type",
      &foo1, &foo2, &foo3);
}
