 ...
 */
template<typename State, typename Primer> class GaussJacksonIntegratorBase
{
public:
    // Member data.
     ...
     * Default constructor.
     */
    GaussJacksonIntegratorBase() = default;
     ...
     */
    GaussJacksonIntegratorBase(const er7_utils::IntegratorConstructor & priming_constructor,
     ...
     */
    GaussJacksonIntegratorBase(const GaussJacksonIntegratorBase & src)
        : coeff(src.coeff),
    ...
    GaussJacksonIntegratorBase & operator=(const GaussJacksonIntegratorBase &) = delete;
...
protected:
     ...
     */
    er7_utils::IntegratorResult base_integrate(double dyn_dt,
                                               ...
                                               State state)
    {
        ...
        if(fsm_state == GaussJacksonStateMachine::Operational)
        {
            ...
            {
                rotate_acc_hist();
                er7_utils::integ_utils::copy_array(deriv, size, acc_hist[order]);
            }

            // Integrate state via the Gauss-Jackson integrator.
            passed = integrate_gj(dyn_dt,       // Dynamic time step
     ...
     */
    void swap(GaussJacksonIntegratorBase & other)
    {
...
private:
     ...
     */
    static Primer * create_primer(const er7_utils::IntegratorConstructor & priming_constructor,
                                  unsigned int size_in,
                                  er7_utils::IntegrationControls & priming_controls);
     ...
     * @return Constructed primer.
     */
    static Primer * replicate_primer(const Primer * src);
     ...
     */
    void start_cycle(double dt, const double * ER7_UTILS_RESTRICT acc, const State & state)
    {
        ...
        {
            er7_utils::integ_utils::copy_array(acc, size, acc_hist[history_length]);
            ++history_length;
        }
     ...
     */
    bool edit_point(double dt, const double * ER7_UTILS_RESTRICT acc ER7_UTILS_UNUSED, State & state)
    {
     ...
     * @return The status (time advance, pass/fail status) of the integration.
     */
    er7_utils::IntegratorResult integrate_primer(double dyn_dt,
                                                 unsigned int target_stage,
                                                 const double * deriv,
                                                 State & state);
     ...
     */
    void save_epoch_data(const double * acc, const State & state);
     ...
     */
    void save_comparison_data(const State & state, double * pos_hist_elem);
     ...
     */
    void mid_correct(unsigned int coeff_idx, double dt, State & state);
     ...
     * @param  state  Corrected state.
     */
    void predict(double dt, const double * const * ahist, State & state);
     ...
     * @param  state  Corrected state.
     */
    void correct(double dt, const double * ER7_UTILS_RESTRICT acc, State & state);
     ...
     */
    bool test_for_convergence(const State & state, double * ER7_UTILS_RESTRICT hist_data);
