...
GaussJacksonConfig
GaussJacksonConfig::standard_configuration ()
{
   GaussJacksonConfig result;
 ...
 */
static GaussJacksonConfig set_default_config_values (
   const GaussJacksonConfig & config)
{
   ...
   if (result.initial_order == -1u) {
      ...
      else if ((result.final_order >= 8) &&
               ((result.priming_technique ==
                 er7_utils::Integration::Unspecified) ||
                (result.priming_technique ==
                 er7_utils::Integration::RKFehlberg78))) {
         result.initial_order = 8;
      }
   ...
   if ((result.ndoubling_steps == -1u) &&
      ...
      result.ndoubling_steps =
         std::min (1u, (result.final_order - result.initial_order)/2u);
   }
 ...
 */
static unsigned int validate_config (
   const GaussJacksonConfig & config)
{
   unsigned int error_count = 0;

   // The initial order must be an even number between 2 and 14.
   if (((config.initial_order & 1) != 0) ||
       (config.initial_order < 2) ||
       (config.initial_order > 14)) {
      er7_utils::MessageHandler::error (
         __FILE__, __LINE__,
...
GaussJacksonConfig
GaussJacksonConfig::validate_configuration (
   const GaussJacksonConfig & config)
{
   // Set defaults.
