...
:
   ...
   SecondOrderODEIntegrator (position_size, velocity_size,
                             deriv_funs, controls),
   vel_integrator (primer_constructor, controls,
   ...
   posdotdot (NULL)
{
    posdot =
       er7_utils::alloc::replicate_array<double> (
          state_size[0], src.posdot);
    posdotdot =
       er7_utils::alloc::replicate_array<double> (
          state_size[0], src.posdotdot);
}
...
// Non-throwing swap.
void
GaussJacksonGeneralizedDerivSecondOrderODEIntegrator::swap (
...
GaussJacksonGeneralizedDerivSecondOrderODEIntegrator::create_copy() const
{
