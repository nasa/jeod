...
// FIXME:
// Need to vary the transition table with state machine transitions.
...
// Copy constructor.
GaussJacksonIntegrationControls::GaussJacksonIntegrationControls (
   const GaussJacksonIntegrationControls & src)
:
   er7_utils::Er7UtilsDeletable (),
...
er7_utils::IntegrationControls*
GaussJacksonIntegrationControls::create_copy() const
{
...
// Non-throwing swap.
void
GaussJacksonIntegrationControls::swap (
   GaussJacksonIntegrationControls & other)
{
...
void
GaussJacksonIntegrationControls::reset_integrator ()
{
   fsm_state = GaussJacksonStateMachine::Reset;
   edit_count = 0;
   cycle_stage = 0;
...
unsigned int
GaussJacksonIntegrationControls::integrate (
   double start_time,
   double sim_dt,
   ...
   er7_utils::BaseIntegrationGroup & integ_group)
{
   ...
   if (fsm_state == GaussJacksonStateMachine::Operational) {
      ...
      case GaussJacksonStateMachine::Priming:
         priming_controls->integrate (
            cycle_starttime, cycle_simdt,
   ...
   er7_utils::BaseIntegrationGroup & integ_group)
{
   ...
   if ((edit_count < max_correction_iterations) &&
       ! integ_status.get_passed()) {
...
void
GaussJacksonIntegrationControls::integrate_gj (
   er7_utils::TimeInterface & time_interface,
   er7_utils::BaseIntegrationGroup & integ_group)
{
   unsigned int target_stage = cycle_stage + 1;
   const er7_utils::IntegratorResult & integ_status =
      integ_group.integrate_bodies (cycle_dyndt, target_stage);
   ...
   if (target_stage == 1) {
      ...
      ++edit_count;
      if (integ_status.get_passed() ||
          (edit_count == max_correction_iterations)) {
         cycle_stage = 0;
      }
   }
}
...
void
GaussJacksonIntegrationControls::start_cycle (
   double sim_dt)
{
   ...
   cycle_starttime = reset_time +
                     sim_dt * state_machine.get_cycle_start_time();
