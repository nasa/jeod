...
public:
    ...
    * Destructor.
    */
   virtual ~JeodIntegrationGroupOwner () {}
...
*/
class JeodIntegrationGroup :
   public er7_utils::BaseIntegrationGroup,
   virtual public TimeChangeSubscriber {

JEOD_MAKE_SIM_INTERFACES(JeodIntegrationGroup)
public:
    ...
    * @return Desired flag.
    */
   bool need_first_step_derivatives (void) const
   {
   ...
   bool merge_integrator_result (
      const er7_utils::IntegratorResult & new_result,
      er7_utils::IntegratorResult & merged_result)
   const
   {
      return integ_merger.merge_integrator_result (
                new_result, merged_result);
   }
      ...
      const T & container)
   {
      ...
      for (typename T::const_iterator iter = container.begin();
         ...
         integ_merger.merge_integrator_result (
            (*iter)->integrate (dyn_dt, target_stage),
            status);
      }
      return status;
   }
...
private:
    ...
    * Not implemented.
    */
   JeodIntegrationGroup (const JeodIntegrationGroup &);

   /**
    * Not implemented.
    */
   JeodIntegrationGroup & operator= (const JeodIntegrationGroup &);
};
