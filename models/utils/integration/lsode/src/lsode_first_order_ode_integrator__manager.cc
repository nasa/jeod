 ...
 */
er7_utils::IntegratorResult LsodeFirstOrderODEIntegrator::integrate(double dyn_dt,
                                                                    unsigned int target_stage JEOD_UNUSED,
                                                                    const double * y_dot_in,
                                                                    double * y_in)
{
...
******************************************************************************/
void LsodeFirstOrderODEIntegrator::process_entry_point_cycle_start()
{
        ...
        if(std::fpclassify(cycle_target_time) != FP_ZERO) // Otherwise nothing to do, time is current
        {
            if((cycle_target_time)*control_data.initial_step_size < 0.0)
            {
                er7_utils::MessageHandler::fail(__FILE__,
                                                ...
                                                "initial_step_size.",
                                                control_data.initial_step_size,
                                                cycle_target_time);
            }
 ...
 */
void LsodeFirstOrderODEIntegrator::manager_initialize_calculation_part1()
{
    arrays.allocate_arrays(control_data.num_odes, control_data.corrector_method);
...
***************************************************************************/
void LsodeFirstOrderODEIntegrator::manager_initialize_calculation_part2()
{
    ...
    if(std::fpclassify(control_data.initial_step_size) == FP_ZERO) // inverse go to 180
    {
        ...
        if(control_data.error_control_indicator == LsodeControlDataInterface::CommonAbsSpecificRel ||
           control_data.error_control_indicator == LsodeControlDataInterface::SpecificAbsSpecificRel)
        {
        ...
        if(rel_tolerance <= 0.0) // inverse go to 160
        {
            ...
            {
                if(control_data.error_control_indicator == LsodeControlDataInterface::SpecificAbsCommonRel ||
                   control_data.error_control_indicator == LsodeControlDataInterface::SpecificAbsSpecificRel)
                {
                ...
                {
                    rel_tolerance = std::max(rel_tolerance, abs_tolerance / (std::abs(y[ii])));
                }
            }
        }
        // 160
        ...
        control_data.initial_step_size = std::min(control_data.initial_step_size, baseline_time);
 ...
 * the cycle.
 */
int LsodeFirstOrderODEIntegrator::manager_check_stop_conditions()
{
 ...
 */
void LsodeFirstOrderODEIntegrator::manager_integration_loop_part1()
{
...
// Part 2
/****************************************************************************/
void LsodeFirstOrderODEIntegrator::manager_integration_loop_part2()
{
    ...
    if(magnitude_of_weighted_array(0, arrays.history) > 1 / epsilon)
    {
        ...
        er7_utils::MessageHandler::fail(__FILE__,
                                        ...
                                        stage_target_time,
                                        1 / magnitude_of_weighted_array(0, arrays.history),
    ...
    if(Numerical::compare_exact((stage_target_time + step_size), stage_target_time))
    {
        ...
        {
            er7_utils::MessageHandler::warn(__FILE__,
                                            __LINE__,
                                            er7_utils::IntegrationMessages::internal_error,
...
//  Part 3
/****************************************************************************/
void LsodeFirstOrderODEIntegrator::manager_integration_loop_part3()
{
    ...
    switch(calculation_task)
    {
        ...
        default:
            er7_utils::MessageHandler::fail(__FILE__,
                                            __LINE__,
                                            er7_utils::IntegrationMessages::invalid_request,
                                            "Illegal calculation task requested, calculation_task = %d.",
                                            calculation_task);
            break;
    }
}
 ...
 * as needing a reset.
 */
void LsodeFirstOrderODEIntegrator::reset_integrator()
{
    if(first_pass)
    {
        return;
 ...
 *
 */
void LsodeFirstOrderODEIntegrator::manager_set_calculation_phase_eq_2_reload()
{
    // ##-----------------------------------------------------------------------
    // ## Block G.
