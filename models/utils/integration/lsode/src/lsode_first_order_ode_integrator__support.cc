 ...
 */
void LsodeFirstOrderODEIntegrator::calculate_epsilon()
{
    epsilon = 1.0;
    // 10
 ...
 * Hence a 13x12 array.
 */
void LsodeFirstOrderODEIntegrator::calculate_integration_coefficients()
{
    ...
    switch(control_data.integration_method)
    {
        ...
        case LsodeControlDataInterface::ImplicitBackDiffStiff:
            ...
            for(int nq = 1; nq <= 5; nq++) // do 230
            {
                ...
                test_coeffs_complete[1][nq - 1] = nqp1 / method_coeffs_complete[0][nq - 1];
                test_coeffs_complete[2][nq - 1] = (nq + 2) / method_coeffs_complete[0][nq - 1];
                rq1fac = rq1fac / nq;
                // 230
            }
            break;
 ...
 */
void LsodeFirstOrderODEIntegrator::interpolate_y()
{
    double tp = 100.0 * epsilon * (std::abs(stage_target_time) + std::abs(prev_good_step_size));
    ...
    if((cycle_target_time - tp) * (cycle_target_time - stage_target_time) > 0.0)
    {
        // 90
 ...
 */
void LsodeFirstOrderODEIntegrator::jacobian_prep_init()
{
    double r;
    ...
    switch(control_data.corrector_method)
    {
        ...
        case LsodeControlDataInterface::NewtonIterInternalJac:
            ...
            data_prepj.r0 = 1000.0 * epsilon * std::abs(step_size) * control_data.num_odes * data_prepj.fac;
...
***************************************************************************/
bool LsodeFirstOrderODEIntegrator::jacobian_prep_loop()
{
    int jj;
    ...
    switch(control_data.corrector_method)
    {
        ...
        case LsodeControlDataInterface::NewtonIterInternalJac:
            ...
            {
                // 220
                arrays.lin_alg[jj][ii] = (arrays.accum_correction[ii] - arrays.save[ii]) * data_prepj.fac;
            }
            y[jj] = data_prepj.yj;
 ...
 ***************************************************************************/
bool LsodeFirstOrderODEIntegrator::jacobian_prep_wrap_up()
{
    switch(control_data.corrector_method)
    {
        case LsodeControlDataInterface::NewtonIterUserJac:
 ...
 */
void LsodeFirstOrderODEIntegrator::linear_chord_iteration()
{
    double hl0, phl0;
 ...
 *
 */
void LsodeFirstOrderODEIntegrator::load_ew_values()
{
    switch(control_data.error_control_indicator)
    {
        case LsodeControlDataInterface::CommonAbsCommonRel:
            ...
            {
                arrays.error_weight[i] = control_data.rel_tolerance_error_control[0] * std::abs(arrays.history[i][0]) +
        ...
        case LsodeControlDataInterface::CommonAbsSpecificRel:
            ...
            {
                arrays.error_weight[i] = control_data.rel_tolerance_error_control[i] * std::abs(arrays.history[i][0]) +
                                         control_data.abs_tolerance_error_control[0];
            }
            break;
