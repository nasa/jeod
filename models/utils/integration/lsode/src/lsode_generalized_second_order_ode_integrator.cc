...
LsodeGeneralizedDerivSecondOrderODEIntegrator::LsodeGeneralizedDerivSecondOrderODEIntegrator(
    ...
    : LsodeSecondOrderODEIntegrator(data_in, controls, deriv_funs, position_size, velocity_size)
{
 ...
 */
LsodeGeneralizedDerivSecondOrderODEIntegrator * LsodeGeneralizedDerivSecondOrderODEIntegrator::create_copy() const
{
    er7_utils::MessageHandler::fail(__FILE__,
                                    __LINE__,
                                    er7_utils::IntegrationMessages::internal_error,
...
er7_utils::IntegratorResult LsodeGeneralizedDerivSecondOrderODEIntegrator::integrate(
    double dyn_dt,
    unsigned int,
    ...
    double * ER7_UTILS_RESTRICT position)
{
    ...
    return_val = first_order_integrator.integrate(dyn_dt, 1, y, y_dot);
    for(unsigned int ii = 0; ii < zeroth_derivative_size; ii++)
    {
        position[ii] = y[ii];
    }
