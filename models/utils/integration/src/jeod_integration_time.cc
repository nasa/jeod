 ...
 */
void JeodIntegrationTime::add_time_change_subscriber(TimeChangeSubscriber & subscriber)
{
    if(std::find(time_change_subscribers.begin(), time_change_subscribers.end(), &subscriber) !=
 ...
 */
void JeodIntegrationTime::remove_time_change_subscriber(TimeChangeSubscriber & subscriber)
{
    auto iter = std::find(time_change_subscribers.begin(), time_change_subscribers.end(), &subscriber);
 ...
 */
void JeodIntegrationTime::notify_time_change_subscribers()
{
    // Walk over the list of registered time change subscribers,
    // notifying each that time has changed rate / direction.
    for(std::vector<TimeChangeSubscriber *>::const_iterator iter = time_change_subscribers.begin();
