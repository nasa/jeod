...
IntegrationTestManager::IntegrationTestManager()
{
    ...
    template_items = JEOD_ALLOC_CLASS_POINTER_ARRAY(template_items_size, IntegrationTest);
...
IntegrationTestManager::~IntegrationTestManager()
{
    ...
    JEOD_DELETE_2D(test_items, total_tests, false);
...
*/
void IntegrationTestManager::initialize( // Return: -- Void
    ...
    double sim_dt)                       // In:     -- Sim. dynamic step size
{
    ...
    if(regression_test)
    {
        ...
        {
            template_items[ii]->set_deprecated_rotation_integration(deprecated_rotation_integration);
    ...
    if(integ_constructor == nullptr)
    {
        if((integ_option_int >= 122) && (integ_option_int <= 134))
        {
            ...
            GaussJacksonConfig gj_config = GaussJacksonConfig::default_configuration();
        ...
        {
            integ_constructor = er7_utils::IntegratorConstructorFactory::create(integ_option);
        }
    }

    // Create and initialize the integration group.
    integ_group = JEOD_ALLOC_CLASS_OBJECT(JeodIntegrationGroup,
                                          (*this,
                                           *integ_constructor,
                                           *integ_interface,
    ...
    if(num_tests > 1)
    {
        for(unsigned int iitem = 0; iitem < num_template_items; ++iitem)
        {
            ...
            for(unsigned int itest = 0; itest < num_tests; ++itest)
            {
                ...
                item->set_deprecated_rotation_integration(deprecated_rotation_integration);
    ...
    if(debug)
    {
        state_integrator_id = JEOD_ALLOC_CLASS_OBJECT(std::string,
 ...
 */
void IntegrationTestManager::reset_integrators( // Return: -- Void
...
*/
void IntegrationTestManager::compute_derivatives()
{
    for(unsigned int ii = 0; ii < total_tests; ++ii)
    {
 ...
 */
int IntegrationTestManager::integrate( // Return: -- Integration step number
    ...
    TimeManager & time_manager)        // Inout:  -- Time manager
{
    ...
    if(debug)
    {
        const char isNotStrings[2][7] = {"is", "is not"};
        int index = 1;
        if(step_number == 0)
        {
            index = 0;
...
*/
void IntegrationTestManager::propagate( // Return: -- Void
...
*/
void IntegrationTestManager::shutdown( // Return: -- Void
    ...
    double dyn_time)                   // In:     s  Dynamic time
{
    ...
    else if(sim_time < end_time - 1.5)
    {
        MessageHandler::warn(__FILE__,
                             ...
                             IntegrationTestMessages::internal_error,
                             "Report not generated "
                             "due to early termination at t=%g",
                             sim_time);
    }
