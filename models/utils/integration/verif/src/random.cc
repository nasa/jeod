...
*/
RandomBase::RandomBase(unsigned int nskip, // In:    -- Skip count
...
*/
RandomBase::RandomBase(RandomSeedGenerator & seed_gen, // Inout: -- Random seed generator
...
*/
double RandomBase::random_u01_scalar()
{
    if(!seeded)
    {
        reset();
...
Purpose: (Constructor, with seed set from numbers pulled out of a hat.)
*/
RandomSeedGenerator::RandomSeedGenerator(unsigned int nskip, // In:    -- Skip count
                                         bool initialize)    // In:    -- Complete initialization?
...
*/
void RandomSeedGenerator::get_seed(unsigned short seed[3]) // Out: -- Seed
{
    long lseed = random_long();
    auto * useed = reinterpret_cast<unsigned short *>(&lseed);
    unsigned int idx0, idx1;

    if((sizeof(long) == 8) && ((useed[0] == 0) || (useed[0] == 0xffffU)) && (useed[1] == useed[0]))
    {
        idx0 = 3;
        idx1 = 2;
    }
