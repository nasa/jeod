...
void
RotationTest::validate_integrability (
...
void
RotationTest::create_integrators (
   const er7_utils::IntegratorConstructor & generator,
   ...
   const er7_utils::TimeInterface & time_if __attribute__ ((unused)) )
{
   ...
   if (deprecated_rotation_integration) {
      ...
      if (generator.implements (
         ...
         integ_generator =
            JEOD_ALLOC_CLASS_OBJECT (
               LieGroupQuatInteg, (state_integrator));
      }
   }
   ...
   if (integ_generator != NULL) {
      ...
      active = false;
      MessageHandler::error (__FILE__, __LINE__,
                             IntegrationTestMessages::internal_error,
...
*/
void
RotationTest::pre_initialize (      // Return: -- Void
   void)
{
...
*/
void
RotationTest::post_initialize (     // Return: -- Void
   void)
{
   Quaternion tempquat;
   double ang_mom[3];
   ...
   true_canon_state.Q_inertial_body.left_quat_to_transformation (
      true_canon_state.T_inertial_body);
   ...
   true_integ_state.Q_inertial_body.left_quat_to_transformation (
      true_integ_state.T_inertial_body);

   // Transform the canonical body rate to the propagation body frame.
   Vector3::transform (T_bcanon_binteg, true_canon_state.body_rate,
                       true_integ_state.body_rate);

   // Transform the canonical inertia matrix to the propagation body frame.
   Matrix3x3::transform_matrix (T_bcanon_binteg, true_canon_state.inertia,
                                true_integ_state.inertia);
   ...
   Vector3::transform (true_canon_state.inertia, true_canon_state.body_rate,
                       ang_mom);
   true_canon_state.kinetic_energy =
      0.5 * Vector3::dot (ang_mom, true_canon_state.body_rate);
   true_canon_state.total_energy =
      true_canon_state.potential_energy + true_canon_state.kinetic_energy;

   Vector3::transform (true_integ_state.inertia, true_integ_state.body_rate,
                       ang_mom);
   true_integ_state.kinetic_energy =
      0.5 * Vector3::dot (ang_mom, true_integ_state.body_rate);
   ...
   Matrix3x3::copy (true_integ_state.T_inertial_body,
                    prop_integ_state.T_inertial_body);
   ...
   Matrix3x3::invert_symmetric (
      prop_integ_state.inertia, inverse_inertia);
 ...
 */
void
RotationTest::compute_derivatives (   // Return: -- Void
   void)
{
   ...
   Vector3::transform (prop_integ_state.inertia, prop_integ_state.body_rate,
                       ang_mom);
...
RotationTest::integrate (             // Return: -- Integration status
   ...
   unsigned int target_stage)         // In:     -- Target stage number
{
   ...
   er7_utils::IntegratorResult status =
      state_integrator->integrate (
         dyn_dt, target_stage,
         body_accel,
         prop_integ_state.body_rate, prop_integ_state.Q_inertial_body);

   // Normalize the quaternion.
   prop_integ_state.Q_inertial_body.normalize_integ ();

   // Compute the corresponding transformation matrix.
   prop_integ_state.Q_inertial_body.left_quat_to_transformation (
      prop_integ_state.T_inertial_body);

   // Compute the quaternion derivative.
   prop_integ_state.Q_inertial_body.compute_left_quat_deriv (
      prop_integ_state.body_rate, Qdot_inertial_body);
   ...
   bool update_errors)
{
   ...
   prop_theta.update (
      prop_integ_state.Q_inertial_body, Q_ref, prop_integ_state.body_rate);
   ...
   prop_integ_state.kinetic_energy =
      0.5 * Vector3::dot (ang_mom, prop_integ_state.body_rate);
   ...
   Vector3::transform (
      T_bcanon_binteg, true_canon_state.body_rate, true_integ_state.body_rate);
   ...
   // Compute errors
   rel_energy_error =
      prop_integ_state.total_energy/true_integ_state.total_energy - 1.0;

   Vector3::diff (prop_integ_state.body_rate, true_integ_state.body_rate,
                  rate_error);
   rate_error_mag = Vector3::vmag (rate_error);

   orient_error.update (prop_integ_state.Q_inertial_body,
                        true_integ_state.Q_inertial_body,
                        rate_error);
...
void
RotationTest::shutdown (           // Return: -- Void
...
*/
IntegrationTest *
RotationTestTorqueFreeSphere::replicate ( // Return: -- Constructed object
   unsigned int test_incr)                // In:     -- Test increment
{
   return JEOD_ALLOC_CLASS_OBJECT (RotationTestTorqueFreeSphere,
                                   (*this, test_incr));
}
...
*/
void
RotationTestTorqueFreeSphere::pre_initialize ( // Return: -- Void
   void)
{
 ...
 */
void
RotationTestTorqueFreeSphere::post_initialize ( // Return: -- Void
   void)
{
   ...
   true_canon_state.inertia[0][0] =
   true_canon_state.inertia[1][1] =
   true_canon_state.inertia[2][2] = I_xx;
...
void
RotationTestTorqueFreeSphere::propagate (
   double dyn_time,
   bool update_errors)
{

   true_canon_state.theta = omega * dyn_time;
   true_canon_state.Q_inertial_body.left_quat_from_eigen_rotation (
      true_canon_state.theta, zhat);
...
*/
IntegrationTest *
RotationTestTorqueFreeSymTop::replicate ( // Return: -- Constructed object
   unsigned int test_incr)                // In:     -- Test increment
{
   return JEOD_ALLOC_CLASS_OBJECT (RotationTestTorqueFreeSymTop,
                                   (*this, test_incr));
}
...
*/
void
RotationTestTorqueFreeSymTop::pre_initialize ( // Return: -- Void
   void)
{
 ...
 */
void
RotationTestTorqueFreeSymTop::post_initialize ( // Return: Void
   void)
{
   double fwz;
   ...
   Matrix3x3::initialize (true_canon_state.inertia);
   true_canon_state.inertia[0][0] =
   true_canon_state.inertia[1][1] = I_xx;
...
void
RotationTestTorqueFreeSymTop::propagate (
   double dyn_time,
   bool update_errors)
{
   double wt;
...
*/
IntegrationTest *
RotationTestTorqueFreeSymTopB::replicate ( // Return: -- Constructed object
   unsigned int test_incr)                 // In:     -- Test increment
{
   return JEOD_ALLOC_CLASS_OBJECT (RotationTestTorqueFreeSymTopB,
                                   (*this, test_incr));
}
...
*/
IntegrationTest *
RotationTestSHOSphere::replicate ( // Return: -- Constructed object
   unsigned int test_incr)         // In:     -- Test increment
{
   return JEOD_ALLOC_CLASS_OBJECT (RotationTestSHOSphere,
                                   (*this, test_incr));
}
...
*/
void
RotationTestSHOSphere::pre_initialize ( // Return: -- Void
   void)
{
 ...
 */
void
RotationTestSHOSphere::post_initialize ( // Return: -- Void
   void)
{
...
*/
void
RotationTestSHOSphere::compute_potential (
   void)
{
   prop_integ_state.potential_energy =
      0.5 * k_sho * prop_integ_state.theta * prop_integ_state.theta;
   true_canon_state.potential_energy =
      0.5 * k_sho * true_canon_state.theta * true_canon_state.theta;
}
...
*/
void
RotationTestSHOSphere::propagate (
   double dyn_time,
   bool update_errors)
{
   ...
   true_canon_state.Q_inertial_body.left_quat_from_eigen_rotation (
      true_canon_state.theta, zhat);
   Vector3::scale (zhat, -omega * theta_max * sinwt,
                   true_canon_state.body_rate);
...
*/
void
RotationTestSHOSphere::compute_torque (
   void)
{
   ang_var = prop_theta;
   ang_var.update (
