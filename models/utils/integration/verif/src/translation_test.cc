...
*/
void TranslationTest::pre_initialize()
{
 ...
 */
void TranslationTest::post_initialize()
{
    // Transform the canonical state to the propagation frame.
    Vector3::transform_transpose(T_iinteg_icanon, true_canon_state.position, true_integ_state.position);
    Vector3::transform_transpose(T_iinteg_icanon, true_canon_state.velocity, true_integ_state.velocity);

    // Compute initial specific kinetic energy (potential done by derived class).
    true_canon_state.kinetic_energy = 0.5 * Vector3::vmagsq(true_canon_state.velocity);
 ...
 */
void TranslationTest::compute_derivatives()
{
    double forc_integ[3];

    // Compute external forces and gravity.
    compute_force();
    Vector3::transform_transpose(prop_integ_state.T_inertial_body, forc_body, forc_integ);
 ...
 */
er7_utils::IntegratorResult TranslationTest::integrate( // Return: -- Integration status
    ...
    unsigned int target_stage)                          // In:     -- Target stage number
{
    ...
    return state_integrator->integrate(dyn_dt * time_scale,
                                       target_stage,
                                       dynamic_accel,
                                       prop_integ_state.velocity,
                                       prop_integ_state.position);
}

void TranslationTest::propagate( // Return: -- Void
    ...
    bool update_errors)
{
    ...
    prop_integ_state.kinetic_energy = 0.5 * Vector3::vmagsq(prop_integ_state.velocity);
    ...
    // Compute errors
    rel_energy_error = (prop_integ_state.total_energy - true_integ_state.total_energy) / energy_scale;

    Vector3::diff(prop_integ_state.position, true_integ_state.position, position_error);
    rel_position_err_mag = Vector3::vmag(position_error) / position_scale;

    Vector3::diff(prop_integ_state.velocity, true_integ_state.velocity, velocity_error);
...
void TranslationTest::shutdown( // Return: -- Void
...
*/
IntegrationTest * TranslationTestOrbit::replicate( // Return: -- Constructed object
    unsigned int test_incr)                        // In:     -- Test increment
{
    return JEOD_ALLOC_CLASS_OBJECT(TranslationTestOrbit, (*this, test_incr));
}
...
*/
void TranslationTestOrbit::pre_initialize()
{
 ...
 */
void TranslationTestOrbit::post_initialize()
{
 ...
 */
void TranslationTestOrbit::compute_force()
{
    double rsq;
    double scale;
...
*/
void TranslationTestOrbit::compute_potential()
{
    prop_integ_state.potential_energy = -GMb / Vector3::vmag(prop_integ_state.position);
}
...
*/
void TranslationTestOrbit::propagate(double dyn_time, bool update_errors)
{
...
Purpose:
  (Compute the true state.)
*/
void TranslationTestOrbit::compute_true_state(double dyn_time)
{
    double manom;
    double eanom;
    double err;
...
Purpose:
  (Create a copy of the object.)
*/
IntegrationTest * TranslationTestSpringDamper::replicate( // Return: -- Constructed object
    unsigned int test_incr)                               // In:     -- Test increment
{
    return JEOD_ALLOC_CLASS_OBJECT(TranslationTestSpringDamper, (*this, test_incr));
}
...
*/
void TranslationTestSpringDamper::pre_initialize()
{
 ...
 */
void TranslationTestSpringDamper::post_initialize()
{
    omega_0 = omega;
...
Purpose:
  (Compute forces, including gravitational accel.)
 */
void TranslationTestSpringDamper::compute_force()
{
    double forc_integ[3];

    // The spring and drag exert forces on the object.
    Vector3::scale(prop_integ_state.position, -mass * omega_sq, forc_integ);
    Vector3::scale_incr(prop_integ_state.velocity, -2.0 * mass * lambda_value * omega_0, forc_integ);

    // Transform to body.
    Vector3::transform(prop_integ_state.T_inertial_body, forc_integ, forc_body);
...
Purpose:
  (Compute the integrated state potential energy.)
*/
void TranslationTestSpringDamper::compute_potential()
{
    prop_integ_state.potential_energy = -omega_sq * Vector3::vmagsq(prop_integ_state.position);
}
...
Purpose:
  (Compute the true state and evaluate.)
*/
void TranslationTestSpringDamper::propagate(double dyn_time, bool update_errors)
{
    // Compute the true state.
    compute_true_state(dyn_time);
...
Purpose:
  (Compute the true state.)
*/
void TranslationTestSpringDamper::compute_true_state(double dyn_time)
{
    switch(damping)
    {
        case UnderDamped:
        {
            ...
            true_canon_state.position[1] = true_canon_state.position[2] = 0.0;

            true_canon_state.velocity[0] = -vA * sin(omegat) * expkt;
            true_canon_state.velocity[1] = true_canon_state.velocity[2] = 0.0;
            break;
        }
