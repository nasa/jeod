...
void LvlhFrame::initialize(DynManager & dyn_manager)
{
    ...
    if(subject_frame == nullptr)
    {
        ...
        if(subject_name.empty())
        {
            MessageHandler::fail(__FILE__,
                                 __LINE__,
                                 LvlhFrameMessages::invalid_configuration,
        ...
        subject_frame = dyn_manager.find_ref_frame(subject_name);
        ...
        if(subject_frame == nullptr)
        {
            MessageHandler::fail(__FILE__,
                                 __LINE__,
                                 LvlhFrameMessages::invalid_name,
    ...
    if(planet_centered_inertial == nullptr)
    {
        ...
        BasePlanet * planet;
        planet = dyn_manager.find_base_planet(planet_name);
    ...
    frame.set_name(subject_name, planet_name, "lvlh");
...
void LvlhFrame::update()
{
    ...
    else
    {
        ...
        subject_frame->compute_relative_state(*planet_centered_inertial, subject_rel_state);
 ...
 */
void LvlhFrame::set_subject_name(const std::string & new_name)
{
 ...
 */
void LvlhFrame::set_subject_frame(RefFrame & new_frame)
{
 ...
 * \param[in] new_name new name.
 */
void LvlhFrame::set_planet_name(const std::string & new_name)
{
 ...
 */
void LvlhFrame::set_planet(BasePlanet & new_planet)
{
 ...
 */
void LvlhFrame::compute_lvlh_frame(const RefFrameTrans & rel_trans)
{
    double angmom[3];
    double hmag;
    double rmagsq;
