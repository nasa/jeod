 ...
 */
template<typename ValueType> class JeodMemoryTable
{
    // Types
public:
    /**
...
public:
    ...
    unsigned int find(const std::string & key) const
    {
        ...
        auto iter(string_to_index.find(key));
    ...
    void del(const std::string & key)
    {
        ...
        {
            JEOD_THROW(std::invalid_argument("key not in table"));
        }
    }
    ...
    const ValueType * get(unsigned int idx) const
    {
        ...
        {
            JEOD_THROW(std::out_of_range("invalid index"));
        }
        ...
        if(result == nullptr)
        {
            JEOD_THROW(std::invalid_argument("deleted entry"));
        }
 ...
 */
template<typename ValueType> class JeodMemoryTableClonable : public JeodMemoryTable<ValueType>
{
    // Constructors, destructors, etc.
public:
    /**
 ...
 */
template<typename ValueType> class JeodMemoryTableCopyable : public JeodMemoryTable<ValueType>
{
    // Constructors, destructors, etc.
public:
    /**
...
public:
    ...
    JeodMemoryReflectiveTable()
        : JeodMemoryTableCopyable<std::string>()
    {
        ;
    }

    explicit JeodMemoryReflectiveTable(const JeodMemoryReflectiveTable &) = delete;
