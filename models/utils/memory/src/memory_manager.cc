 ...
 */
JeodMemoryManager::JeodMemoryManager(JeodMemoryInterface & interface)
...
#undef MAKE_DESCRIPTOR
    ...
    else
    {
        MessageHandler::error(__FILE__,
                              __LINE__,
                              MemoryMessages::singleton_error,
...
JeodMemoryManager::~JeodMemoryManager()
{
    ...
    if(Master == this)
    {
        ...
        for(auto & it : alloc_table)
        {
            const void * addr = it.first;
            const JeodMemoryItem & item = it.second;
            const JeodMemoryTypeDescriptor & tdesc = get_type_descriptor_nolock(item);
            if(item.get_is_registered())
            {
                sim_interface.deregister_allocation(addr, item, tdesc, __FILE__, __LINE__);
            }
        }
 ...
 */
void JeodMemoryManager::generate_shutdown_report()
{
    if(debug_level > 0)
    {
        ...
        unsigned int total_size = max_table_size * (telem_size + item_size);
...
Assumptions and limitations:
  ((If the restore doesn't work the sim will be knee deep in alligators.))
*/
void JeodMemoryManager::restart_clear_memory()
{
    void * addr = nullptr;
    JeodMemoryItem item;
    ...
    for(delete_oldest_alloc_entry_atomic(addr, item, type); addr != nullptr;
        delete_oldest_alloc_entry_atomic(addr, item, type))
    {
        // De-register the item with the simulation engine.
        if(item.get_is_registered())
        {
            sim_interface.deregister_allocation(addr, item, *type, __FILE__, __LINE__);
        }

        // Destruct and delete the item consistent with its creation.
        type->destroy_memory(item.get_placement_new(), item.get_is_array(), item.get_nelems(), addr);
 ...
 */
void JeodMemoryManager::restart_reallocate(const std::string & mangled_type_name,
                                           uint32_t unique_id,
                                           uint32_t nelements,
                                           bool is_array)
{
    TypeEntry tentry = get_type_entry_atomic(Typeid_type_name, mangled_type_name);
    ...
    register_memory_internal(addr, unique_id, true, is_array, nelements, tentry, __FILE__, __LINE__);
}
...
Assumptions and limitations:
  ((This method will be invoked via the JEOD memory allocation macros.
   ...
   (The memory is allocated but not constructed. Construction is the
    ...
    the allocated memory via placement new.))
*/
void * JeodMemoryManager::create_memory_internal(
    bool is_array, unsigned int nelems, int fill, const TypeEntry & tentry, const char * file, unsigned int line)
{
    ...
    register_memory_internal(addr, 0, true, is_array, nelems, tentry, file, line);
 ...
 * \param[in] line Line number containing JEOD_ALLOC
 */
void JeodMemoryManager::register_memory_internal(const void * addr,
                                                 uint32_t unique_id,
                                                 bool placement_new,
                                                 bool is_array,
                                                 unsigned int nelems,
                                                 ...
                                                 unsigned int line)
{
    ...
    JeodMemoryItem item(placement_new, is_array, true, tdesc.is_structured(), nelems, tidx, alloc_idx);
    ...
    if(tdesc.get_register_instances() && sim_interface.register_allocation(addr, item, tdesc, file, line))
    {
 ...
 */
bool JeodMemoryManager::is_allocated_internal(const void * addr, const char * file, unsigned int line)
{
    ...
    find_alloc_entry_atomic(addr, false, file, line, found_addr, found_item, found_type);
 ...
 */
void JeodMemoryManager::destroy_memory_internal(void * addr, bool delete_array, const char * file, unsigned int line)
{
    void * found_addr = nullptr;
    JeodMemoryItem found_item;
    const JeodMemoryTypeDescriptor * found_type = nullptr;
    ...
    find_alloc_entry_atomic(addr, true, file, line, found_addr, found_item, found_type);
    ...
    {
        sim_interface.deregister_allocation(found_addr, found_item, *found_type, file, line);
    }

    // Destruct and delete the item consistent with its creation.
    found_type->destroy_memory(found_item.get_placement_new(),
 ...
 */
void JeodMemoryManager::set_mode_internal(JeodSimulationInterface::Mode new_mode)
{
    mode = new_mode;
}
...
Assumptions and limitations:
  ((This is a low-level allocation function. It does not
   ...
   (The returned address should not be released using the C free function
    ...
    Failure to obey this restriction will result in big problems.))
*/
void * JeodMemoryManager::allocate_memory(std::size_t nelems, std::size_t elem_size, bool guard, int fill) const
{
    void * addr = nullptr;
    char * buf = nullptr;
    ...
    if(buf == nullptr)
    {
        MessageHandler::fail(__FILE__, __LINE__, MemoryMessages::out_of_memory, "Out of memory");
        return nullptr;
    }
 ...
 * \param[in] line Line number containing delete
 */
void JeodMemoryManager::free_memory(
    void * addr, std::size_t length, bool guard, unsigned int alloc_idx, const char * file, unsigned int line) const
{
    ...
    if(guard)
    {
        ...
        if((guard_lo[0] != MAGIC0) || (guard_lo[2] != MAGIC0) || (guard_hi[0] != MAGIC0) || (guard_lo[1] != MAGIC1) ||
           (guard_lo[3] != MAGIC1) || (guard_hi[1] != MAGIC1))
        {
            if(alloc_idx == 0)
            {
                MessageHandler::warn(__FILE__,
