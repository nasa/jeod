...
const
{
   ...
   if (mutex_lock_status != 0) {
      MessageHandler::fail (
         __FILE__, __LINE__, MemoryMessages::lock_error,
 ...
 */
void
JeodMemoryManager::end_atomic_block (
   bool ignore_errors)
const
{
...
*/
const std::string &
JeodMemoryManager::get_string_atomic (
   unsigned int idx)
const
{
   if (idx == 0) {
 ...
 */
unsigned int
JeodMemoryManager::add_string_atomic (
...
*/
bool
JeodMemoryManager::get_type_index_nolock (
...
JeodMemoryManager::get_type_entry_atomic (
...
JeodMemoryManager::get_type_descriptor_atomic (
...
const
{
   ...
   try {
      ...
      // This is not keyed, so we need to do a search in this case.
         ...
         for (TypeTable::const_value_iterator iter = type_table.begin();
              iter != type_table.end();
              ++iter)
         {
...
*/
uint32_t
JeodMemoryManager::get_alloc_id_atomic (
   const char * file,
   unsigned int line)
{
   ...
   try {
      ...
      // Check for overflow.
      if ((allocation_number == UINT32_MAX) ||
         (allocation_number == 0)) {
         end_atomic_block (true);
         MessageHandler::fail (
 ...
 */
void
JeodMemoryManager::reset_alloc_id_atomic (
   uint32_t unique_id)
{
 ...
 */
void
JeodMemoryManager::find_alloc_entry_atomic (
   const void * addr,
   bool delete_entry,
   const char * file,
   unsigned int line,
   ...
   const JeodMemoryTypeDescriptor *& found_type)
{
   ...
   try {
      ...
      // the subject address.
      AllocTable::iterator iter = alloc_table.upper_bound (addr);
   ...
   if (address_inside) {
      MessageHandler::warn (
         ...
         (delete_entry ? "JEOD_IS_ALLOCATED" : "JEOD_DELETE"),
         file, line,
         addr, test_type->get_name().c_str());
   }
}
 ...
 */
void
JeodMemoryManager::add_allocation_atomic (
   const void * addr,
   ...
   unsigned int line)
{
   ...
   try {
      ...
      AllocTable::iterator beg_iter (alloc_table.upper_bound (addr));
      AllocTable::iterator end_iter (alloc_table.lower_bound (
                                        tdesc.buffer_end (
                                           addr, item.get_nelems())));
      ...
      if (beg_iter != alloc_table.begin()) {
        ...
        if (addr >= get_type_descriptor_nolock(beg_iter->second).buffer_end (
                       beg_iter->first, beg_iter->second)) {
            ++beg_iter;
         }
      }
      ...
      std::pair<AllocTable::iterator, bool> ret =
         alloc_table.insert (AllocTable::value_type(addr, item));
 ...
 */
void
JeodMemoryManager::delete_oldest_alloc_entry_atomic (
   void *& addr,
   JeodMemoryItem & item,
   const JeodMemoryTypeDescriptor *& type)
{
   ...
   try {
      ...
      // Not done. Find the oldest element, set outputs, and delete.
         ...
         for (AllocTable::iterator iter = alloc_table.begin();
              iter != alloc_table.end();
              ++iter) {
