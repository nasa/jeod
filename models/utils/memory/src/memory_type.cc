 ...
 */
std::string
JeodMemoryTypeDescriptor::initialize_type_name (
 ...
 */
size_t
JeodMemoryTypeDescriptor::pointer_dimension ( // Return: -- Number asterisks
   const std::string & demangled_name)        // In:     -- Type name
{
   ...
   for (pos = demangled_name.find('*', pos);
        pos != std::string::npos;
        pos = demangled_name.find('*', pos+1)) {
      ++naster;
   }
...
JeodMemoryTypeDescriptor::base_type (  // Return: -- Base type name
   const std::string & demangled_name) // In:     -- Type name
{
   ...
   for (pos = base_name.find(" const", start_pos);
        pos < len;
        pos = base_name.find(" const", pos)) {
   ...
   return JeodMemoryManager::get_type_descriptor (
             JeodMemoryManager::Demangled_type_name, base_name);
}
 ...
 */
JeodMemoryTypeDescriptor::JeodMemoryTypeDescriptor (
 ...
 */
JeodMemoryTypeDescriptor::JeodMemoryTypeDescriptor (
   const JeodMemoryTypeDescriptor & src)
:
   obj_id(src.obj_id),
   name(src.name),
   attr(src.attr),
   size(src.size),
   register_instances(src.register_instances)
{ }
 ...
 */
const std::string
JeodMemoryTypeDescriptor::type_spec (
