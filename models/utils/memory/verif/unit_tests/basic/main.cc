...
class MemoryPool {
 public:
   ...
   {
      bool allocated;
      void * test_ptr = (void*)((char*)ptr - 16);

      active = false;
      {
         AllocTable::iterator iter = alloc_table.find (test_ptr);
      ...
      void * ptr)
   {
      ...
      if (active) {
         active = false;
         {
            AllocTable::iterator iter = alloc_table.find (ptr);
            if (iter != alloc_table.end()) {
               if (message_level >= MessageHandler::Debug) {
                  fprintf (stderr,
                           ...
                           __FILE__, __LINE__, ptr, iter->second);
               }
               free (ptr);
               ...
               MessageHandler::fail (
                  ...
                  "Attempt to free unallocated buffer at %p", ptr);
            }
         }
         active = true;
      }
 ...
 private:
   ...
   typedef std::map <const void *, size_t> AllocTable;
   bool active;
   ...
   char ** argv)
{
   ...
   message_level = verbose ? MessageHandler::Debug : MessageHandler::Notice;
   memory_level = verbose ? 3 : 2;
   ...
   i42 = JEOD_ALLOC_PRIM_OBJECT (int, 42);
   if (memory_pool.is_allocated(i42) &&
       (*i42 == 42)) {
      MessageHandler::inform (
         __FILE__, __LINE__, "utils/memory/verif/unit_tests/basic/debug",
   ...
   darr = JEOD_ALLOC_PRIM_ARRAY (3, double);
   if (memory_pool.is_allocated(darr) &&
       (darr[0] == 0.0) &&
       (darr[1] == 0.0) &&
       (darr[2] == 0.0)) {
      MessageHandler::inform (
   ...
   carr = JEOD_ALLOC_PRIM_ARRAY (1, char);
   if (memory_pool.is_allocated(carr) &&
       (*carr == 0)) {
      MessageHandler::inform (
         __FILE__, __LINE__, "utils/memory/verif/unit_tests/basic/debug",
   ...
   if (memory_pool.is_allocated(qarr) &&
       (qarr->are_you_ok())) {
      MessageHandler::inform (
         __FILE__, __LINE__, "utils/memory/verif/unit_tests/basic/debug",
   ...
   if (memory_pool.is_allocated(qobj) &&
       (qobj->are_you_ok())) {
      MessageHandler::inform (
         __FILE__, __LINE__, "utils/memory/verif/unit_tests/basic/debug",
   ...
   if (memory_pool.is_allocated(bobj) &&
       (bobj->are_you_ok())) {
      MessageHandler::inform (
         __FILE__, __LINE__, "utils/memory/verif/unit_tests/basic/debug",
   ...
   MessageHandler::inform (
      __FILE__, __LINE__, "utils/memory/verif/unit_tests/basic/debug",
      "JEOD_DELETE_OBJECT (bobj = %p);", bobj);
   JEOD_DELETE_OBJECT (bobj);

   MessageHandler::inform (
      __FILE__, __LINE__, "utils/memory/verif/unit_tests/basic/debug",
      "JEOD_DELETE_OBJECT (barr[0] = %p);", barr[0]);
   JEOD_DELETE_OBJECT (barr[0]);


   MessageHandler::inform (
      __FILE__, __LINE__, "utils/memory/verif/unit_tests/basic/debug",
      "JEOD_DELETE_ARRAY (barr[1] = %p);", barr[1]);
   JEOD_DELETE_ARRAY (barr[1]);

   MessageHandler::inform (
      __FILE__, __LINE__, "utils/memory/verif/unit_tests/basic/debug",
      "JEOD_DELETE_ARRAY (barr = %p)", barr);
   JEOD_DELETE_ARRAY (barr);

   MessageHandler::inform (
      __FILE__, __LINE__, "utils/memory/verif/unit_tests/basic/debug",
      "JEOD_DELETE_ARRAY (carr = %p)", carr);
   JEOD_DELETE_ARRAY (carr);

   MessageHandler::inform (
      __FILE__, __LINE__, "utils/memory/verif/unit_tests/basic/debug",
      "JEOD_DELETE_ARRAY (darr = %p)", darr);
   JEOD_DELETE_ARRAY (darr);

   MessageHandler::inform (
      __FILE__, __LINE__, "utils/memory/verif/unit_tests/basic/debug",
      "JEOD_DELETE_OBJECT (i42 = %p)", i42);
   ...
   if (overwrite_outside) {
      passed = passed &&
               (sim_interface.get_allowed_count() == 0);
   }
