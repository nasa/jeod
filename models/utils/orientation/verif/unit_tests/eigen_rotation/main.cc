...
bool test_eigen_rotation (
   double psi, const double uhat[3],
   unsigned int icase, bool verbose, ColorText & color);
   ...
   char * argv[])
{
   ...
   for (unsigned icosphi = 0; icosphi <= 16; ++icosphi) {
      ...
      if (icosphi == 0) {
         ...
         ntheta[icosphi] =
            (unsigned int) (2.0 * M_PI * icosphi * sinphi / phi + 0.5);
      ...
      for (unsigned itheta = 0; itheta <= itheta_max; ++itheta) {
         ...
         for (unsigned ipsi = 0; ipsi < 5; ++ipsi) {
            ++icase;
            psi = small_angles[ipsi];
...
bool
test_eigen_rotation (
   double psi,            // In:  r Eigen rotation angle
   ...
   ColorText & color)     // Inout: -- Color text object
{
   ...
   Orientation::compute_eigen_rotation_from_matrix (
      trans_quat, &psi_step2, uhat_step2);
   ...
   if (verbose || ! case_passed) {
      printf ("%4d  %11.8f %11.8f %11.8f  %11.7f  %s  %s\n",
              icase,
              uhat[0], uhat[1], uhat[2], psi*180.0/M_PI,
