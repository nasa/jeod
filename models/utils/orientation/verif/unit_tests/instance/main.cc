...
const char * euler_sequence_name[12] = {"EulerXYZ",
                                        "EulerXZY",
                                        "EulerYZX",
                                        "EulerYXZ",
                                        "EulerZXY",
...
void test_getters(Orientation::DataSource source, const TruthData & truth_data, Orientation & orient)
{
    ...
    switch(source)
    {
        ...
        case Orientation::InputEulerRotation:
        {
            ...
            if((sequence != orient.euler_sequence) ||
               (std::fpclassify(euler_angles[0] - orient.euler_angles[0]) != FP_ZERO) ||
               (std::fpclassify(euler_angles[1] - orient.euler_angles[1]) != FP_ZERO) ||
               (std::fpclassify(euler_angles[2] - orient.euler_angles[2]) != FP_ZERO))
            {
    ...
    TestData test_data(trans_out, quat_out, eigen_rot_out, eigen_axis_out, euler_angles_out);

    test_results(truth_data, test_data);
}

void test_contents(const TruthData & truth_data, Orientation & orient)
{
    double trans_out[3][3];
    Quaternion quat_out;
    double eigen_rot_out;
...
void get_euler_angles(Orientation & orient, double euler_angles_out[12][3])
{
    for(unsigned int iseq = 0; iseq < 12; ++iseq)
    {
        auto sequence = (Orientation::EulerSequence)iseq;
...
void test_results(const TruthData & truth_data, const TestData & test_data)
{
    ...
    Matrix3x3::product_right_transpose(truth_data.trans, test_data.trans, err_matrix);
    ...
    for(unsigned int iseq = 0; iseq < 12; ++iseq)
    {
        ...
        Orientation::compute_matrix_from_euler_angles(static_cast<Orientation::EulerSequence>(iseq),
                                                      test_data.euler_angles[iseq],
                                                      test_matrix);
        Vector3::diff(truth_data.euler_angles[iseq], test_data.euler_angles[iseq], err_vec);
        ...
        if(verbose || !passed)
        {
            ...
            printf("  Euler angle %s test %s\n", euler_sequence_name[iseq], passFailStrings[index]);
        }
        update_results(passed);
    }
}
...
TestData::TestData(const double trans_in[3][3],
                   const Quaternion quat_in,
                   const double eigen_rot_in,
                   const double eigen_axis_in[3],
                   const double euler_angles_in[12][3])
...
void TestData::set_data(const double trans_in[3][3],
                        const Quaternion quat_in,
                        const double eigen_rot_in,
                        const double eigen_axis_in[3],
                        const double euler_angles_in[12][3])
