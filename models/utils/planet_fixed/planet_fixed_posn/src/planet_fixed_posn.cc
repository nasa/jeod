...
/*******************************************************************************
Purpose: (Specify radius below which coordinates should be deemed suspect.
          The current tiny setting protects against division by zero.
          A setting much closer to one might be better.)
*******************************************************************************/
const double PlanetFixedPosition::Small_radius_limit = 1e-60;


/**
 * Construct a PlanetFixedPosition object.
 */
 ...
 */
void PlanetFixedPosition::initialize(
      Planet *planet_in)
{
   planet = planet_in;
 ...
 */
void PlanetFixedPosition::update_from_cart(
      const double cart[3])
{
 ...
 */
void PlanetFixedPosition::update_from_spher(
      const AltLatLongState &spher)
{
   sphere_coords.set_data (spher.altitude,
                           spher.latitude, spher.longitude);
 ...
 */
void PlanetFixedPosition::update_from_ellip(
      const AltLatLongState &ellip)
{
   ellip_coords.set_data (ellip.altitude,
                          ellip.latitude, ellip.longitude);
   ...
   void)
{
   ...
   if (r_local < planet->r_eq * Small_radius_limit) {
      ...
      if (std::isnan(cart_coords[0])) {
         MessageHandler::fail (
            __FILE__, __LINE__, PlanetFixedMessages::domain_error,
   ...
   void)
{
   ...
   double x_ellipse_sq = (cart_coords[0] * cart_coords[0]) +
...
// Solve for elliptic parameters
   get_elliptic_parameters(x_ellipse, z_ellipse,
                          ellip_coords.latitude, ellip_coords.altitude);
   ...
   void)
{
   ...
   cart_coords[2] = ((rc_ellipse * (1.0 - e_sq)) + ellip_coords.altitude) *
                    sin_lat;
...
*******************************************************************************/
int
PlanetFixedPosition::get_elliptic_parameters (/* Return: --
