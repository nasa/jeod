 ...
 */
template<class Links, class Container, class Messages> class TreeLinks
{
    JEOD_MAKE_SIM_INTERFACES(jeod, TreeLinks)
...
public:
    ...
    {
        return std::find(path_to_node_.begin(), path_to_node_.end(), &link) - path_to_node_.begin();
    }
    ...
    Container * nth_from_root(unsigned int index)
    {
        ...
        {
            return &(path_to_node_[index]->container_);
        }
        else
        {
            return nullptr;
    ...
    void make_root()
    {
        ...
        if(parent_ != nullptr)
        {
            MessageHandler::fail(__FILE__, __LINE__, Messages::invalid_node, "Object has non-null linkages");
            // Not reached
            return;
        }
    ...
    void attach(Links & new_parent)
    {
        ...
        if(parent_ != nullptr)
        {
            MessageHandler::fail(__FILE__, __LINE__, Messages::invalid_node, "Object is already attached");
            // Not reached
            return;
        }
    ...
    bool is_progeny_of(const Links & target) const
    {
        ...
        auto link_matcher = [&target](const Links * link)
        {
            return link == &target;
        };

        if(std::any_of(path_to_node_.begin(), path_to_node_.end(), link_matcher))
        {
            result = true;
        }
    ...
    int find_last_common_index(const Links & target) const
    {
        ...
        if((path_length() == 0) || (target.path_length() == 0) || (path_to_node_[0] != target.path_to_node_[0]))
        {
            last_common = -1;
        }
        ...
        else
        {
            ...
            {
                for(last_common = 0; path_to_node_[last_common + 1] == target.path_to_node_[last_common + 1];
                    ++last_common)
                {
    ...
    const Links * find_last_common_node(const Links & target) const
    {
        ...
        {
            return path_to_node_[last_common];
        }
    }
...
protected:
    ...
    void construct_path_to_node()
    {
        ...
        if(parent_ != nullptr)
        {
            ...
            std::copy(parent_->path_to_node_.begin(), parent_->path_to_node_.end(), path_to_node_.begin());
        }
...
private:
    ...
    void detach_internal()
    {
        ...
        if(parent_ != nullptr)
        {
            parent_->children_.erase(std::find(parent_->children_.begin(), parent_->children_.end(), this));
        }
