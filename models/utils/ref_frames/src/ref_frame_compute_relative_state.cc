 ...
 */
void RefFrame::compute_relative_state(const RefFrame & wrt_frame, RefFrameState & rel_state) const
{
    ...
    else
    {
        ...
        for(auto * link : TreeLinksDescentRange<const RefFrameLinks>(wrt_frame.links, common_node_index + 1))
        {
 ...
 * \param[out] rel_state The relative state
 */
void RefFrame::compute_relative_state(const RefFrame & wrt_frame, bool reverse_sense, RefFrameState & rel_state) const
{
    ...
    if(reverse_sense)
    {
        ...
        Vector3::transform(rel_state.rot.T_parent_this, rel_state.trans.position);
        Vector3::transform(rel_state.rot.T_parent_this, rel_state.trans.velocity);

        // Transform relative angular vel. from this frame to the wrt_frame.
        Vector3::transform_transpose(rel_state.rot.T_parent_this, rel_state.rot.ang_vel_this);
        Vector3::transform_transpose(rel_state.rot.T_parent_this, rel_state.rot.ang_vel_unit);
 ...
 * \param[out] rel_state The relative state
 */
void RefFrame::compute_state_wrt_pred(const RefFrame & pred_frame, RefFrameState & rel_state) const
{
 ...
 * \param[out] rel_state        The relative state
 */
void RefFrame::compute_state_wrt_pred(unsigned int pred_frame_index, RefFrameState & rel_state) const
{
    ...
    for(auto * link : TreeLinksAscendRange<const RefFrameLinks>(links, links.path_length() - 1, pred_frame_index + 1))
    {
 ...
 * \param[out] rel_state The relative state
 */
void RefFrame::compute_pred_rel_state(const RefFrame & pred_frame, RefFrameState & rel_state) const
{
    unsigned pred_frame_idx = links.find_path_index(pred_frame.links);
 ...
 * \param[out] rel_state        The relative state
 */
void RefFrame::compute_pred_rel_state(unsigned int pred_frame_index, RefFrameState & rel_state) const
{
 ...
 */
void RefFrame::compute_position_from(const RefFrame & in_frame, double rel_pos[3]) const
{
    int common_node_index;       /* Index of last node in common between
                                    this frame and the wrt_frame */
    ...
    // common node frame.
    for(auto * link : TreeLinksAscendRange<const RefFrameLinks>(links, links.path_length(), common_node_index + 1))
    {
        ...
        {
            Vector3::transform_transpose(link_frame->state.rot.T_parent_this, rel_pos);
        }
    ...
    for(auto * link : TreeLinksDescentRange<const RefFrameLinks>(in_frame.links, common_node_index + 1))
    {
        link_frame = &link->container();
