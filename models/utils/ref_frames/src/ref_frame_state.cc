...
*******************************************************************************/
RefFrameTrans & RefFrameTrans::operator=(const RefFrameTrans & source)
{
    if(this != &source)
    {
        copy(source);
 ...
 * \param[in] source Source state
 */
RefFrameRot & RefFrameRot::operator=(const RefFrameRot & source)
{
    if(this != &source)
    {
        copy(source);
 ...
 * \param[in] source Source state
 */
RefFrameState::RefFrameState(const RefFrameState & source)
{
    copy(source);
}
 ...
 * \param[in] source Source state
 */
RefFrameState & RefFrameState::operator=(const RefFrameState & source)
{
    if(this != &source)
    {
        copy(source);
 ...
 * \param[in] source Source state
 */
void RefFrameState::negate(const RefFrameState & source)
{
    // Relevant equations:
    //   T_B:A =   T_A:B^T
    //   w_B:A = - (T_B:A * w_A:B)
    ...
    {
        Vector3::cross_decr(source.rot.ang_vel_this, trans.position, trans.velocity);

        Vector3::transform(rot.T_parent_this, source.rot.ang_vel_this, rot.ang_vel_this);
    }
 ...
 */
void RefFrameState::incr_left(const RefFrameState & s_ab)
{
    // Relevant equations:
    //   T_A:C = T_B:C * T_A:B
    //   w_A:C = T_B:C * w_A:B + w_B:C
    ...
    if(std::fpclassify(s_ab.rot.ang_vel_mag) != FP_ZERO)
    {
        ...
        Vector3::transform_incr(rot.T_parent_this, s_ab.rot.ang_vel_this, rot.ang_vel_this);
    ...
    if(!Numerical::compare_exact(s_ab.rot.Q_parent_this.scalar, 1.0))
    {
        ...
        if(std::fpclassify(s_ab.rot.ang_vel_mag) != FP_ZERO)
        {
            Vector3::cross_incr(s_ab.rot.ang_vel_this, trans.position, trans.velocity);
        }

        // Transform x_B:C to frame A.
        Vector3::transform_transpose(s_ab.rot.T_parent_this, trans.position);

        Vector3::transform_transpose(s_ab.rot.T_parent_this, trans.velocity);
    }
 ...
 */
void RefFrameState::incr_right(const RefFrameState & s_bc)
{
    // Relevant equations:
    //   T_A:C = T_B:C * T_A:B
    //   w_A:C = T_B:C * w_A:B + w_B:C
    ...
    if(std::fpclassify(rot.ang_vel_mag) != FP_ZERO)
    {
        ...
        Vector3::cross_incr(rot.ang_vel_this, s_bc.trans.position, v_bc_in_b_obs_a);
    ...
    if(!Numerical::compare_exact(rot.Q_parent_this.scalar, 1.0))
    {
        ...
        Vector3::transform_transpose_incr(rot.T_parent_this, s_bc.trans.position, trans.position);
 ...
 * \param[in] s_ab Left addend
 */
void RefFrameState::decr_left(const RefFrameState & s_ab)
{
    // Relevant equations:
    //   T_B:C = T_A:C * T_A:B^T
    ...
    if(!Numerical::compare_exact(s_ab.rot.Q_parent_this.scalar, 1.0))
    {
        ...
        Vector3::cross_decr(s_ab.rot.ang_vel_this, trans.position, trans.velocity);
    }
    ...
    if(std::fpclassify(s_ab.rot.ang_vel_mag) != FP_ZERO)
    {
        Vector3::transform_decr(rot.T_parent_this, s_ab.rot.ang_vel_this, rot.ang_vel_this);
        rot.compute_ang_vel_products();
    }
}
 ...
 */
void RefFrameState::decr_right(const RefFrameState & s_bc)
{
    // Relevant equations:
    //   T_A:B = T_B:C^T * T_A:C
    ...
    if(!Numerical::compare_exact(rot.Q_parent_this.scalar, 1.0))
    {
        Vector3::transform_transpose_decr(rot.T_parent_this, s_bc.trans.position, trans.position);
        Vector3::transform_transpose_decr(rot.T_parent_this, v_bc_in_b_obs_a, trans.velocity);
    }
    // Shortcuts for the case T_A:B is identity.
    else
    {
