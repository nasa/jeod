...
:
   ...
   message_suppress_id(false),
   message_suppress_location(false),
 ...
 */
void
JeodSimulationInterface::configure (
   const JeodSimulationInterfaceInit & config)
{
   MessageHandler::set_suppression_level (
      config.message_suppression_level);

   MessageHandler::set_suppress_id (
      config.message_suppress_id);

   MessageHandler::set_suppress_location (
      config.message_suppress_location);

   JeodMemoryManager::set_debug_level  (
      config.memory_debug_level);
}
...
JeodSimulationInterface::create_integrator_interface (
   void)
{
 ...
 */
double
JeodSimulationInterface::get_job_cycle (
   void)
{
   // Throw a fatal error if the singleton sim interface is not available.
   if (sim_interface == NULL) {
   ...
   const JeodMemoryTypeDescriptor * tdesc)
{
   ...
   // Nominal case: Pass the call on to the sim interface object.
      ...
      return sim_interface->get_memory_interface_internal().
                get_name_at_address (addr, tdesc);
   }
}
 ...
 */
void *
JeodSimulationInterface::get_address_at_name (
...
JeodSimulationInterface::get_checkpoint_writer (
   const std::string & section_id)
{
   // Throw a fatal error if the singleton sim interface is not available.
   if (sim_interface == NULL) {
 ...
 */
void
JeodSimulationInterface::set_mode (
   Mode new_mode)
{
