...
void
JeodDynbodyIntegrationLoop::initialize_integ_loop (
   void)
{
   ...
   integ_group =
      integ_group_factory->create_group (
         *this, **integ_constructor, integ_interface,
   ...
   er7_utils::IntegrableObject & integrable_object)
{
   ...
   for (std::size_t pos = obj_name.find_last_of ('.');
        pos < obj_name.npos;
        pos = obj_name.find_last_of ('.')) {
      obj_name.erase (pos);
      sim_object =
...
void
JeodDynbodyIntegrationLoop::add_integrable_object (
   er7_utils::IntegrableObject & integrable_object)
{
   // FIXME : Think of calling add_sim_object on the containing SimObject
   // if the object is a DynBody.

   // The object must not be a DynBody.
   if (dynamic_cast<DynBody*> (&integrable_object)) {
      MessageHandler::error (
         __FILE__, __LINE__, SimInterfaceMessages::integration_error,
         "Do not use add_integrable_object on DynBody objects.");
      return;
   }
...
void
JeodDynbodyIntegrationLoop::remove_integrable_object (
   er7_utils::IntegrableObject & integrable_object)
{
...
void
JeodDynbodyIntegrationLoop::set_time_to_loop_start ()
{
...
int
JeodDynbodyIntegrationLoop::integrate_dt (
   double beg_sim_time,
   double del_sim_time)
{
   int ipass = 0;
   bool need_derivs =
      integ_group->get_first_step_derivs_flag() ||
      get_first_step_deriv_from_integrator();
...
int
JeodDynbodyIntegrationLoop::add_sim_object (
...
int
JeodDynbodyIntegrationLoop::remove_sim_object (
   Trick::SimObject & sim_obj)
{
...
void
JeodDynbodyIntegrationLoop::add_sim_object_bodies (
   Trick::SimObject & sim_obj)
{
   std::vector<DynBody*> bodies = dyn_manager->get_dyn_bodies ();

   for (std::vector<DynBody*>::iterator iter = bodies.begin();
        iter != bodies.end();
        ++iter) {
      DynBody * body = *iter;
...
void
JeodDynbodyIntegrationLoop::remove_sim_object_bodies (
   Trick::SimObject & sim_obj)
{
   std::vector<DynBody*> bodies = dyn_manager->get_dyn_bodies ();
...
void
JeodDynbodyIntegrationLoop::update_integration_group (
