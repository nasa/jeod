 ...
 */
const std::string
JeodTrick10MemoryInterface::get_trick_checkpoint_file (
   bool checkpoint)
{
   std::string result;
   if (checkpoint) {
 ...
 */
void *
JeodTrick10MemoryInterface::get_address_at_name (
...
// FIXME: Add error message for null pointer if name is not "NULL"?
   return result;
}
...
const
{
   ...
   return trick_checkpoint_agent->ref_string_from_ptr (
            const_cast<void*>(addr), &ptr_attr, pointer_index);
}
...
*/
void *
JeodTrick10MemoryInterface::translate_name_to_addr (
