...
{
    for(OptionTable::iterator iter = option_table.begin(); iter != option_table.end(); ++iter)
    {
        delete iter->second;
        iter->second = nullptr;
    }
 ...
 */
void CmdlineParser::add(const char * option_name, bool * target)
{
 ...
 * \param[in,out] target        The receiver of the default and parsed value.
 */
void CmdlineParser::add_switch(const char * option_name, bool * target)
{
    ...
    {
        SwitchOptionValue * opt_val = new SwitchOptionValue(option_name, target_ref);
 ...
 * \param[in,out] target        The receiver of the default and parsed value.
 */
void CmdlineParser::add_toggle(const char * option_name, bool default_value, bool * target)
{
    // Convert the target pointer to a reference.
    bool & target_ref = *target;
    ...
    if(check_toggle_prereqs(__FILE__, __LINE__, "add_toggle", option_name, negated_name))
    {
        ToggleOptionValue * opt_val = new ToggleOptionValue(option_name, default_value, target_ref);
        NegatedOptionValue * neg_val = new NegatedOptionValue(negated_name, target_ref);
 ...
 * \param[in,out] target        The receiver of the default and parsed value.
 */
void CmdlineParser::add_counter(const char * option_name, int * target)
{
    // Convert the target pointer to a reference.
    ...
    {
        CounterOptionValue * opt_val = new CounterOptionValue(option_name, target_ref);
 ...
 * \param[in,out] target        The receiver of the default and parsed value.
 */
void CmdlineParser::add_string(const char * option_name, const char * default_value, const char ** target)
{
    // Convert the target pointer to a reference.
    ...
    {
        StringOptionValue * opt_val = new StringOptionValue(option_name, default_value, target_ref);
 ...
 * \param[in,out] target        The receiver of the default and parsed value.
 */
void CmdlineParser::add_int(const char * option_name, int default_value, int * target)
{
    // Convert the target pointer to a reference.
    int & target_ref = *target;
    ...
    {
        IntOptionValue * opt_val = new IntOptionValue(option_name, default_value, target_ref);
 ...
 * \param[in,out] target        The receiver of the default and parsed value.
 */
void CmdlineParser::add_double(const char * option_name, double default_value, double * target)
{
    // Convert the target pointer to a reference.
    ...
    {
        DoubleOptionValue * opt_val = new DoubleOptionValue(option_name, default_value, target_ref);
...
int CmdlineParser::parse(int & argc, char ** argv)
{
    ...
    for(OptionTable::iterator iter = option_table.begin(); iter != option_table.end(); ++iter)
    {
        ...
        if(iter->second->takes_argument())
        {
            options[iopt].has_arg = required_argument;
        }
        else
        {
            options[iopt].has_arg = no_argument;
        }
        options[iopt].flag = nullptr;
        options[iopt].val = 1;
        ++iopt;
    }

    // The options array must be terminated with a null entry.
    options[iopt].name = nullptr;
    ...
    while(status == 0)
    {
        ...
        if(optc == '?')
        {
            MessageHandler::warn(__FILE__,
                                 __LINE__,
                                 TestHarnessMessages::invalid_option,
        ...
        {
            OptionTable::iterator iter(option_table.find(options[option_index].name));
            assert(iter != option_table.end());
            if(options[option_index].has_arg == no_argument)
            {
                iter->second->parse_value(nullptr);
    ...
    if(status == 0)
    {
        for(IntList::iterator iter = delete_list.begin(); iter != delete_list.end(); ++iter)
        {
 ...
 * \param[in] option_name  The option name, sans the leading hyphen.
 */
void CmdlineParser::check_timing(const char * file, int line, const char * method_name, const std::string & option_name)
{
 ...
 * \param[in] option_name   The option name, sans the leading hyphen.
 */
bool CmdlineParser::check_registration(const char * file,
                                       int line,
 ...
 * \param[in] option_name   The option name, sans the leading hyphen.
 */
bool CmdlineParser::check_prereqs(const char * file,
                                  int line,
                                  const char * method_name,
                                  const std::string & option_name)
{
 ...
 */
bool CmdlineParser::check_toggle_prereqs(const char * file,
                                         int line,
                                         const char * method_name,
                                         const std::string & option_name,
