...
extern "C" {
    ...
    * \param[in] ... Items to be printed
    */
   static void print_nonfatal_message (
      FILE * file,
      unsigned int level,
      const char * format,
      ...)
 ...
 */
void
TestMessageHandler::add_allowed_code (
 ...
 */
void
TestMessageHandler::add_verboten_code (
   const std::string & code)
{
 ...
 */
void
TestMessageHandler::process_add_suppressed_code (
   const char * msg_code)
{
 ...
 */
void
TestMessageHandler::process_delete_suppressed_code (
   const char * msg_code)
{
 ...
 */
void
TestMessageHandler::set_failure_threshold (
   int threshold)
{
 ...
 */
void
TestMessageHandler::process_message (
   int severity,
   const char * prefix,
   const char * file,
   unsigned int line,
...
const
{
   ...
   if (severity < 0) {
      ...
      print_message_and_exit (
         where, why.str().c_str(), exit_status);
   ...
   if (severity <= failure_threshold) {
      ...
      // The failure isn't thrown here so that the message can be printed.
         ...
         for (CodeList::const_iterator iter = verboten_codes.begin();
              iter != verboten_codes.end();
              ++iter) {
   ...
   if (throw_failure ||
      ...
      if (suppress_id && suppress_location) {
         print_nonfatal_message (
            stderr, JEOD_SEND_HS_LEVEL,
            "\n%s\n", buffer);
      }
