   ...
   char ** argv)
{
   ...
   item_passed = CheckItem::within_tolerance (com_vec, exp_vec, 0.8,
                                              out_vec);
   ...
   item_passed = CheckItem::within_tolerance (com_vec, exp_vec, 0.8, 0.1,
                                              out_vec);
   std::cout << color.pass_fail (out_vec, "%6g", 1, 2, com_vec) << "\n";
   all_passed = all_passed &&
                (! item_passed) &&
   ...
   all_passed = all_passed &&
                (! item_passed) &&
                (! out_mat[0][0]) && out_mat[0][1] && out_mat[0][2] &&
                out_mat[1][0] && (! out_mat[1][1]) && out_mat[1][2] &&
                out_mat[2][0] && out_mat[2][1] && (! out_mat[2][2]);

   std::cout << "within_tolerance, " <<
                "thresholds = 5e-4 absolute, 1e-3 relative\n" <<
                "Expect one failure:\n";
   item_passed = CheckItem::within_tolerance (com_mat, exp_mat, 5e-4, 1e-3,
                                              out_mat);
   std::cout << color.pass_fail (out_mat, "%6g", 2, 2, com_mat) << "\n";
   all_passed = all_passed &&
                (! item_passed) &&
